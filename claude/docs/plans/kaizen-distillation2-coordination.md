...
2. **The producers still holding entries.** All four `teco` chunks are now
   **done** (U42–U45, `117df76`/`3804263`/`8973269`/`97ea756`) — that inbox is
   drained to the frontier that existed at each dispatch. Re-queried live at
   U45's close, **2026-09-10**: `analyst` **10**, `tdd-engineer` **6**,
   `architect` **4**, `coder` **4**, `data-scientist` **4**, `cobb` **2**,
   `teco` **2** (both out-of-scope arrivals from the concurrent model-bench
   coordination, correctly left untouched by U45), plus the 1 orphan — **33**
   in the graph. Next: a chunk per remaining producer, **small → large** per
   the standing decision — `cobb` (2) first, then `architect`/`coder`/
   `data-scientist` (4 each), then `tdd-engineer` (6), then `analyst` (10) —
   and the sweep does not stop there; the stakeholder decision below keeps it
   open-ended rather than closing on this list. **Re-query at each dispatch;
   never dispatch against a figure recorded here** — this list has been wrong
   at every single dispatch it has been read at.
