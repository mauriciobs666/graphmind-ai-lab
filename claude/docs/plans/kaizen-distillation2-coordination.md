...
| U35 | **`qa-engineer` inbox — 1 entry** (`7f3c9a21…`, `TestClient` defaults to `raise_server_exceptions=True`, so an acceptance test on the default can never see the real bare 500). Two open questions handed over, not decided: one section or two in `python-web-quirks`, and whether `SKILL.md:47`'s audience line is stale for omitting `qa-engineer` | `ac2f1a4ba4ac40333` | in-flight | `skills/python-web-quirks/SKILL.md` (steer) · `claude/qa-engineer/kaizen/*` | teco re-derivation → — | — |
