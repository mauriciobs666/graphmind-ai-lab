...
## Six platform failures, all cleanly recovered

Six units were killed mid-run by session rate limits (HTTP 429) — U10, U15,
U19, U20, U41 and U42 — and **not one lost a disposition.** They landed at six
different points, which is what makes them useful together. U42 is the first to
be killed **after** its deliverable was accepted, during a gate-driven
correction, and it is the one that shows the failure mode the other five do not:
a kill can leave the **artifact** corrected and the **record of it** stale.
