...
| U47 | **`qa-engineer`'s inbox — 1 entry** (`a1c2e9d4`, LM Studio's real `/api/v0` server does not JIT-load an unrecognized model id — HTTP 400 "No models loaded" rather than a load attempt), re-queried at dispatch. A fresh arrival, not present at U45's/U46's census — precedented as a standalone single-entry chunk at U35 (`3f769ca`). Routing question left open for cobb: `qa-engineer`'s own `qa-testing-techniques.md` KB (tooling/technique angle) vs. `data-scientist`'s `lm-studio-model-notes.md` (model-behavior angle) — the brief asked it to verify rather than assume. K-030 out of scope | `ac37e105af7b82e96` | in-flight | — | — → — | — |
...
2. **The producers still holding entries.** All four `teco` chunks are done
   (U42–U45), and `cobb`'s own inbox is now done too (U46, `b6b136b`) —
   **drained to 0.** Re-queried live at U46's close, **2026-09-10**:
   `analyst` **10**, `tdd-engineer` **6**, `coder` **4**, `architect` **4**,
   `data-scientist` **4**, `teco` **2** (both out-of-scope arrivals, untouched
   since U45), `qa-engineer` **1** (new — flagged by U46, not present at
   U45's census, whoever dispatches it next should re-verify rather than
   trust this figure), plus the 1 orphan — **32** in the graph. Next, small →
   large: `qa-engineer` (1, precedented standalone at U35), then `teco`'s 2
   new arrivals (out of U45's frontier but not out of the pass — the pass is
   open-ended now, so a fresh `teco` inbox is an ordinary next chunk like any
   other producer's, not a permanent exclusion), then
   `architect`/`coder`/`data-scientist` (4 each), then `tdd-engineer` (6),
   then `analyst` (10). The stakeholder decision below keeps this open-ended
   rather than closing on any fixed list. **Re-query at each dispatch; never
