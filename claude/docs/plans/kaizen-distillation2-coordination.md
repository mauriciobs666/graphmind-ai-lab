...
2. **The producers still holding entries.** All five `teco` chunks are done
   ...
   is done (U50, `40b0daf`), `architect`'s is done (U51, `fa7c906`), `cobb`'s
   self-produced refill is done (U52, `75c0fd1`), and `tdd-engineer`'s is done
   (U54, `1bdb2fa`) — all **drained to 0.** Re-queried live at U54's close,
   **2026-09-10**: `analyst` **10**, plus the 1 orphan — **11** in the graph.
   Next: `analyst` (10, over the ~12 chunk bar so likely one chunk) — but
   **re-verify fresh at dispatch**; every producer count recorded in this pass
   has drifted from concurrent-session or the pass's own refills at least once
   already, several producers (`teco`, `cobb`, `coder`) more than once. The
   stakeholder decision below keeps this open-ended rather than closing on any
   fixed list. **Re-query at each dispatch; never dispatch against a figure
