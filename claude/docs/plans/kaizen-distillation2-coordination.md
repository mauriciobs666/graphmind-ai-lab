...
| U48 | **`teco`'s inbox — 2 entries** (`b7d4e1a9`: an early failing `cd` in a multi-line, non-`&&`-joined Bash call doesn't abort the call, later lines silently still run; `f3a1c9e2`: a shared git working tree can be silently wiped by an unrelated concurrent session's commit/mutation, not just by the coordinator's own git command). Re-queried fresh immediately before dispatch — count confirmed unchanged at 2, same ids. Likely routes to `teco/teco.md` itself; cobb asked to verify rather than assume. | `a9a6a4a95fdadae88` | in-flight | — | — → — | — |
...
2. **The producers still holding entries.** All four `teco` chunks are done
   (U42–U45), `cobb`'s own inbox is done (U46, `b6b136b`), and `qa-engineer`'s
   is done too (U47, `5e54603`) — **drained to 0.** Re-queried live at U47's
   close, **2026-09-10**: `analyst` **10**, `tdd-engineer` **6**, `coder`
   **4**, `architect` **4**, `data-scientist` **4**, `teco` **2** (both
   out-of-scope arrivals, untouched since U45), plus the 1 orphan — **31** in
   the graph. Next, small → large: `teco`'s 2 new arrivals (out of U45's
   frontier but not out of the pass — the pass is open-ended now, so a fresh
   `teco` inbox is an ordinary next chunk like any other producer's, not a
   permanent exclusion), then `architect`/`coder`/`data-scientist` (4 each),
