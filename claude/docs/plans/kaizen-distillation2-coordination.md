...
| U28 | `graph-dba` **code unit** on `skills/joern-cpg/scripts/pipeline.sh`: fix **K-009** (`rq()` returns 0 on a bare runtime-error reply) + confirm and close/re-scope **K-008**. Not a distillation unit | `a71e467eb629d98ad` (resumed once, to correct a case count) | gated (code committed `48882d8`, awaiting Pass 2) | **K-009 fixed by removing the blacklist**, not widening it: success is now recognised positively (reply's last line must begin `Query internal execution time:`), non-query commands refused rc 2. Second defect found and fixed in the same block (a read-back that never ran reported as *did not land*). **K-008 closed**, both facts confirmed by execution. 7 files | `analyst` (diff-scoped, `a8f29fed07dd1847e`) → **approve with suggestions** (`7edf98c`), 2 majors both by execution, both **closed** in `48882d8`; **Pass 2 re-gate in flight**. Review at `docs/reviews/rq-execution-gate.md` | 141.2k + 150.5k + 156.4k tok, 59 tools |
