...
| U10 | `cobb` | `ac832594ecb52b863` | in-flight | Stage 4: repoint `claude/AGENTS.md`'s "Learnings capture" section to `ingest_document`/`ws:agent-team`, piloted with 1-2 agents (rollout order left to `cobb`'s own call per plan closing item (c)), live-verified against the now-wired `falkor-chat-agent-team` MCP entry | analyst → — | — |
