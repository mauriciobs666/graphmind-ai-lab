...
| U5 (fix SKILL.md orphan-sweep: corpus-count guard + collision-safe adopt branch) | `cobb` | `ae77ce414732a6168` | gated | `skills/agent-maintenance/SKILL.md` + `claude/cobb/kaizen/history.md`, commit `b7e39a1` — teco-reverified: read the full edited bullet directly, both fixes close the exact gaps U1a named (count-guard-before-trust; adopt-branch mismatch escalates instead of deleting); `git diff --stat` confirmed only the 2 intended files changed | `analyst` → — (Pass 4, resumed `ac021e93cccda49a0`) | 98.2k tok / 20 tools / 153s |
