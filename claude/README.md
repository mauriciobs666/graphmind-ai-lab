...
| [`graph-dba`](./graph-dba/graph-dba.md) | Graph database administrator & data architect specialized in **FalkorDB** (Redis-module, GraphBLAS sparse-matrix engine; RedisGraph successor; built for GraphRAG). Covers its OpenCypher dialect, modeling, vector/full-text indexing, constraints, multi-graph tenancy, in-memory sizing, replication/clustering, and tuning via `GRAPH.PROFILE`. Fluent in the wider LPG world (Neo4j, openCypher, GQL) for porting. Designs the deployment; container/Compose plumbing → `devops`. Implementer-bound design work hands off as a **design note** at `<component>/docs/plans/<slug>-graph.md` (quick consults stay inline). **Destructive ops are harness-gated:** a `PreToolUse` hook (`graph-dba/hooks/guard-destructive-ops.sh`) escalates `GRAPH.DELETE`/`FLUSHALL`/volume wipes to human approval. | Designing a graph data model, writing/optimizing FalkorDB Cypher, indexes/constraints, FalkorDB deployment (RAM sizing, persistence, replication, Redis Cluster), tuning slow traversals, bulk ingestion/migration, building a GraphRAG/knowledge-graph layer, or FalkorDB ops. |
| [`joern`](./joern/joern.md) | **Code Property Graph (CPG)** specialist; operates the **Joern** toolset locally to turn a source repo into a CPG, query it via **CPGQL** (AST/CFG/CDG/DDG, call graphs, data-flow & taint), and **export/load the code graph into FalkorDB as Cypher** — driving the [`joern-cpg`](../skills/joern-cpg/SKILL.md) skill's parse → export (neo4jcsv) → transform → load scripts (which pin `JOERN_HOME`/`JAVA_HOME`). Ships a default CPG→FalkorDB model (shared `:CpgNode` label + indexed `id`, edge types verbatim); deep FalkorDB modeling/indexing/tuning → `graph-dba`, JDK/toolchain provisioning → `devops`. **Destructive ops are harness-gated:** a `PreToolUse` hook (`joern/hooks/guard-destructive-ops.sh`) escalates `GRAPH.DELETE`/`FLUSHALL`/volume wipes (a CPG reload resets its graph) to human approval. | Generating a CPG for a codebase, running CPGQL/Joern queries (vulnerability, taint, reachability, call-chain analysis), or exporting/loading a repository's code graph into FalkorDB / Cypher. |
...
- `joern/kaizen/` — [plan](./joern/kaizen/plan.md) · [history](./joern/kaizen/history.md)
...
- **Agents:** `~/.claude/agents/<name>` → `claude/<name>` (one symlink per agent folder).
  - **Hooks (`devops`, `graph-dba`, `qa-engineer`, `joern`, `architect`, `teco`, `tico`, `analyst`,
    ...
    globs + escalation message), and the four destructive-ops guards (`devops`, `graph-dba`,
    `qa-engineer`, `joern`) over
