...
This directory (`claude/`) holds custom Claude Code subagents and shared skills. Each agent is a folder: `<name>/<name>.md` (Markdown + YAML frontmatter) plus `<name>/kaizen/{plan,history}.md`. Skills live under `skills/<name>/SKILL.md` (the collection-level skills home, deployed via `~/.claude/skills` → `claude/skills`). See [`README.md`](./README.md) for the human-facing catalog.

## Agents

- **cobb** — agentic-development expert (Claude Code, Kiro, OpenCode agent formats + cross-tool standards). Use for designing/authoring/reviewing/porting/debugging agents, skills, steering docs, hooks, system prompts. Source: `cobb/cobb.md`. Kaizen: `cobb/kaizen/`. Testing standards: `cobb/TESTING.md`. Relies on the **`agent-maintenance`** skill (`skills/agent-maintenance/`, deployed via the `~/.claude/skills` → `claude/skills` symlink) for its kaizen/documentation/drift-audit procedures — the resident prompt keeps only the mandate and points at the skill.
