...
**A live instance is open as of 2026-09-09**, and it is the fourth generation of this same shape:
`skills/joern-cpg/scripts/test-stamp-wiring.sh`'s `rq` call-site check was hardened against four
named mutation shapes and still misses a literal bad command written across a backslash line
continuation, because its mechanism is line-oriented and a shell call site is not. Owned by
`graph-dba`; do not treat that check as a model.
