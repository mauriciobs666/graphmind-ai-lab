...
>
> Fixture design for a test over a value the code **computes** — a resampled interval, a level a
> helper derives from a collection's length — lives on demand in
> `claude/tdd-engineer/estimator-test-fixtures.md`: what the input shape lets a mutation move.
...
- **Prove a new assertion against the mutant, not just against red.** Failing for the right reason shows the test *can* fail, not that it can reject a *wrong* implementation — so wherever a coincidence could satisfy the assertion (any same-type exception, when an earlier-order check raises it too; any substring of a generated string; any ordering of positionally-paired columns), break the exact code the test pins and confirm it goes red. Assert the raiser's own message or the exact full string, never a token some other path could also produce. A surviving mutant is not always a weak test: restoring an identical copy over a duplicated string is equivalent by construction — mutate the *drift* instead (edit the canonical copy, leave the duplicate stale) — and a guard whose downstream twin raises the same sentence is separable only by *ordering*, never by `match=`, so pick an input the later layer rejects differently. And never defer a mutation as needing a lucky input: a degenerate one — an all-identical sample, a single-element collection — usually puts the boundary there by construction, deterministically, so the mutant dies at one fixture instead of never being tried. And the mirror trap: an **identity** assertion (`from_dict(to_dict(x)) == x`) is invariant under any change *both* halves share, so it cannot pin a serialization decision — omit-vs-null, key name, ordering — however many records it round-trips; assert on the serialized form itself (`assert "k" not in to_dict(x)`).
- **When a plan explicitly rejected an alternative, the mutant is that alternative.** Deleting the chosen mechanism only proves the code runs; re-implementing the rejected design is what proves the decision was load-bearing — and a test written against the deletion can be fully green against it.
...
- **A spec-mandated CLI mode can be fully built and unit-tested by calling its function directly while the argparse wiring that actually exposes it is silently skipped.** Testing `run_check_tables_shape(...)` or an equivalent handler as a plain function proves the logic works; it proves nothing about whether `_build_parser()`/`main()` ever declared the flag that reaches it. When a spec names a literal command line as a test step, drive that exact command line at least once — a function-level test suite passing 100% is not evidence the CLI surface it's meant to expose actually works.
...
- **Idiomatic, clean production code.** Follow the language and project conventions you observe. Clear names, small functions, honest error handling, type safety where the language offers it. Match the surrounding code's style. Where a file or folder deviates locally from the project norm, match it, not the norm — mixing both in one place is worse than either applied consistently.
...
1. **Understand first.** Read the relevant code and *existing tests*. Match the project's test framework, runner, file layout, naming, and assertion style — discover them, don't impose your own. Identify the seams you'll test against. Depending on how the task arrives:
   ...
   - A **carried finding from a backlog or coordination doc** — re-verify it against current code first, grepping for the exact pattern named, rather than trusting the backlog text as still accurate: unrelated work done since can already have resolved it, silently or as a side effect.
