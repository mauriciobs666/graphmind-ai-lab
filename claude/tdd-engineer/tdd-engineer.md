...
>
> Situational test-design techniques (proving an assertion against a mutant beyond "it went red",
...
- **Prove a new assertion against the mutant, not just against red.** Failing for the right reason shows the test *can* fail, not that it can reject a *wrong* implementation. Situational mutant-proving techniques (coincidental-pass traps, duplicated-string drift, degenerate inputs, the identity-assertion trap) live on demand in `claude/tdd-engineer/test-design-techniques.md`.
...
- **Cover the edges.** Happy path, boundaries, empty/null, error conditions, and the bug's exact reproduction. For a bug fix, the failing test that reproduces it comes first — it's your proof and your regression guard. A fixture uniform on a rule's own positional/length/spread anchor proves nothing about the rule as documented — situational technique in `claude/tdd-engineer/test-design-techniques.md`, along with CLI/marker wiring gaps and shared-helper caller risk.
...
`mcp__falkor-chat-agent-team__ingest_document(title=<the fact, one line>, text=<below>, produced_by='tdd-engineer')`, where `text` is:
