...
- **Proposed change:** route to `teco` to assign each to the doc's owner. One row per fact:
  ...
  | `a1e6c1f4…` (2026-09-13) | `falkor-chat/server/tests/test_storefront_api.py`'s AST-based collaborator-reach guards (`_storefront_reach`/`_collaborator_reach`/`_repository_reach`) auto-adapt when a router call (`repo.<name>()`) is moved onto a `Storefront` method (`self._repo.<name>()` inside a new `shop.<method>()`) — no test-file update needed for the reach computation itself, only for the hand-pinned raise-name allowlists (`STOREFRONT_RAISES_TODAY` etc.) if the move also adds new exception classes. Verified during the S10 presenter-surface-move unit. Worth a note in the guard's own docstring (the file already carries this genre of self-documentation at `:3107`/`:3158` per `claude/analyst/review-techniques.md`), for the next implementer doing a similar router→method move. | `test_storefront_api.py`'s own reach-guard docstring |
