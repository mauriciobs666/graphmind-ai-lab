...
| K-018 | 2026-08-21 | high | 🔵 | `claude/scripts/guard-destructive-ops.sh` did not fire/escalate for a live `redis-cli GRAPH.DELETE` issued from inside `graph-dba`'s own Bash context when dispatched via the `Agent` tool (M7 rollout, unit G1, `docs/plans/generic-cypher-mcp2-coordination.md`) — despite the regex plainly matching the command text (re-read the script: `GRAPH\.DELETE` with a token-boundary anchor, case-insensitive). Official docs (`code.claude.com/docs/en/hooks`, re-fetched 2026-08-21) state frontmatter/settings hooks fire identically for a subagent whether run as the main session agent or spawned as a nested delegate, with no documented exception — so this is either a genuine harness bug on the Bash/nested-subagent path, or (leading hypothesis, unconfirmed) the G1 dispatch itself silently ran as `general-purpose` rather than `graph-dba` because `subagent_type` was omitted on the `Agent` call — the exact failure mode independently confirmed 2026-08-21 (teco's own `kaizen_team` entry `b1e3a1f0…`, now promoted into `teco.md` Guardrails), which would starve the delegate of `graph-dba`'s frontmatter `hooks:` entirely, not just weaken it. **Next step:** on the next `graph-dba` dispatch that will run a `GRAPH.DELETE`-shaped command, explicitly verify `subagent_type: "graph-dba"` was passed, and confirm the hook fires. If it still doesn't fire with `subagent_type` confirmed correct, escalate as a genuine harness/hook-wiring defect — the content-diff of the 5 actual G1 deletes was independently verified correct, so this is a backstop-reliability gap, not evidence of a bad delete having happened. |
