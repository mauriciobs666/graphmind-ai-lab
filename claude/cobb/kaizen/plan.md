...
| K-030 | 2026-09-09 | **high** | ✅ | **Closed 2026-09-19 (U1, `agent-knowledge-base-strategy5-coordination.md`) — Track 1's own last open item (team-wide write-convention cutover) is done, plus the Stage 9 orphan-doc fix. Full detail moved to `history.md`.** |
...
- **Status:** 🟡 in-progress — Stage 0 (2026-09-17), Track 1 Stages 1-5 (2026-09-18), and all of
  Track 2 — Stage 6 including its final retry pass, Stage 7, Stage 8's full AC-2 gate, and Stage 9
  (all 2026-09-19) — delivered and closed (detail below). **Only open item: team-wide cutover of
  the raw-capture write convention beyond the `cobb`/`teco` pilot** (Track 1's own follow-up, not
  Track 2's). Not moved to history.md: this item isn't done.
- **Priority:** high
- **Stage 0 — delivered 2026-09-17, commit `6d834f0`, closed.** A prompt restructure (moving
  ...
  |---|---|---|---|
  | `teco` | 11,010 → **8,118** (−26%) | 3,105 → **1,874** | `coordination-techniques.md` (new, 37 sections) |
  | `architect` | 2,553 → **1,522** (−40%) | 1,617 → **718** | `plan-authoring-techniques.md` (new, 13 sections) |
  | `data-scientist` | 2,661 → **2,032** (−24%) | 1,247 → **826** | `statistical-method-techniques.md` (added; `lm-studio-model-notes.md` already existed, 9 sections) |
  | `tdd-engineer` | 2,646 → **2,138** (−19%) | 1,502 → **1,240** | `test-design-techniques.md` (added; two guard/fixture KBs already existed, 8 sections) |
...
- **Track 2 Stage 8 (golden-set evaluation) — DELIVERED 2026-09-19**, `data-scientist`/`qa-engineer`
  (not this agent's own unit — logged here for K-030's own completeness). Phase 1 pilot (16 pairs)
  calibrated a 0.42 floor; the full ~45-pair AC-2 gate (Phase 2) pooled to recall@5 0.875
  [Wilson 0.719,0.950] and re-derived the floor to **0.43** after a genuine true positive scored
  0.4201 (over 0.42 by 0.0001). The floor ships as an explicit **interim** value: one row's score
  (R6/h40's second sibling) proved bimodal across sessions (0.4201/0.4405, both reproduced multiple
  times), and full floor-safety at that row is escalated to `data-scientist` rather than resolved
  unilaterally — `skills/agent-kb-retrieval/SKILL.md` states this plainly, not silently. Gate
  verdict: `analyst` approve (Pass 2, after 1 blocker + 2 majors fixed in Pass 1). Full trail:
  `claude/docs/reviews/agent-knowledge-base-strategy4-stage8-phase1.md` and
  `-stage8-phase2.md`.
- **Track 2 Stage 9 (distillation-ingestion hook) — DELIVERED 2026-09-19, this agent's own unit
  (U7), fixed per U7b's `analyst` review (1 blocker, 1 major, 2 minors) and U7d's Pass 2 re-check
  (1 remaining gap in the major's fix) — both rounds 2026-09-19,
  `claude/docs/reviews/agent-knowledge-base-strategy4-stage9.md`.** `skills/agent-maintenance/
  SKILL.md` §5 gained a new step 5, after the existing 4-step numbered procedure (not a
  restructure of it): whenever a KB `.md` file is edited — a fresh distillation promotion, or any
  later revision — sync `ws:agent-team` by `delete_document(oldId)` (if a prior version exists)
  then `ingest_document(newText, title=..., produced_by='cobb')`, same "file stays the authoring
  act, ingestion is a follow-on sync" posture as the plan's §4.5, now with a `verified: "pending"`
  marker written *before* the delete/ingest pair runs (U7b major fix) so a mid-sequence
  `ingest_document` failure after a successful delete leaves a detectable trace instead of a
  silent gap — a real risk, not hypothetical (this backend failed transiently 31 times during
  Stage 6's own migration). **The recovery bullet for a `"pending"` entry found at a fresh pass's
  start now applies the same byte-exact `get_document` discipline the "Verify, then flip" bullet
  already uses (U7d fix)**, not a bare `None`-check — a real-but-stale document (the cycle
  interrupted right after writing `"pending"`, before `delete_document` ever ran) is a different
  prior state from a real-and-current one (the cycle actually completed, only the flip was left
  undone), and only the byte-exact-match case is safe to flip to `verified: true`; both other cases
  re-run the full sequence. **Claim → `documentId` tracking: reused
  `claude/cobb/scripts/kb-claim-manifest.json`** rather than building a `list_documents`+title-match
  scan (the plan's other named option, closing item c) — the manifest already exists and is already
  keyed on file+heading (the same axis an edit is made along); `check_content_loss.py` already
  treats it as canonical (restored to `SKILL.md` itself in U7d after U7b's edit had dropped it).
  (U7b blocker fix: an earlier version of this entry additionally cited Stage 6's own review as
  "recommending" this reuse — false, confirmed by direct `grep`; that review never discusses the
  manifest-reuse question, and the citation is dropped here and from `SKILL.md`/`history.md`,
  standing on the two legs above alone.) Step 5 also now says to refresh a file's stale Stage-6-era
  `_DONE`/`_note` completion narrative after a claim add/remove, and names the manifest's
  no-locking/serialize-concurrent-writers risk (U7b's two minors). **Live smoke-tested**, not just
  documented: ingested a throwaway disposable claim (v1), ran the new delete-old+ingest-new
  sequence to simulate an edit (v2), confirmed the old `documentId` returns `null` and the new one
  round-trips byte-exact via `get_document`, then deleted the v2 artifact too — no leftover test
  document in `ws:agent-team`. `MENTIONS`-equivalent tagging is explicitly named as still open, not
  solved (`ws:agent-team`'s `Document` model has no edge for it; `kaizen_team` stays the fallback
  for a claim that genuinely needs cross-agent credit) — per the plan's own closing item, this was
  flagged, not attempted. `claude/scripts/audit-team.sh` re-run clean after U7, U7b, and U7d (same
  5 pre-existing, unrelated FAILs — personal-identifier leaks elsewhere in the repo — no new ones).
- **Track 1 (Stages 1-5) since delivered, 2026-09-18** — coordination ledger
  ...
  via `Bash`, resolved directly by the user, `docs/reviews/mcp-json-edit-bypass-incident.md`),
  Stage 4 (write-convention repointed, **piloted with `cobb`/`teco` only, not team-wide**), and
  Stage 5 (this agent's own unit — `agent-maintenance` SKILL.md §5's curator review/clear steps
  gained the `ws:agent-team` read/clear hook alongside the unchanged `kaizen_team` shapes, a named
  hook point, not a §5 redesign; live-verified before finalizing, not taken on the plan's word).
  Full detail in `history.md`, 2026-09-18. **One real follow-up still not actioned:** rewrite the
  other 11 agents' own "Learning capture" sections onto the new mechanism (team-wide cutover) —
  Stage 5's §5 text explicitly says only `cobb`/`teco` currently write to `ws:agent-team` and will
  need updating the moment that widens. The embedding-backend-unreachable finding logged here
  2026-09-18 is **superseded** — `devops`'s U1 (`agent-knowledge-base-strategy4-coordination.md`)
  fixed reachability the same day; Stage 6's own smoke test re-confirmed the backend live and
  working before this note was corrected.
- **Track 2 Stage 6 (distilled-knowledge migration) — FULLY CLOSED 2026-09-19, including the final
  retry pass.** Content migration finished across all 13 KB files (final dispatch U3h completed
  `analyst/review-techniques.md`, 54/54 headings, 81 claims, zero `Document.status:"failed"`
  across its own migration). 11 of the 13 files landed fully clean with zero embedding failures
  (`plan-authoring-techniques.md`, `statistical-method-techniques.md`, `test-design-techniques.md`,
  `falkordb-reference.md`, `frontend-quirks.md`, `qa-testing-techniques.md`,
  `estimator-test-fixtures.md`, `coordination-techniques.md`, `lm-studio-model-notes.md`,
  `graph-dba/falkordb-quirks.md`, `analyst/review-techniques.md`). The other 2
  (`ops-quirks.md` 12 claims, `guard-testing-techniques.md` 19 claims = 31 documents) went through
  the single low-contention one-item-at-a-time final retry pass (delete → re-ingest identical text
  → verify), run exactly once per the dispatching brief: **26/31 resolved to `ready`; 5/31
  reproduced the identical `"failed"` signature a second time under confirmed low-contention
  conditions and are now a closed, permanent, named gap** (4 in `ops-quirks.md`: `docker run`
  stdout-clean, cached-build `FROM`-metadata round trip, PID-1 bare-interpreter `SIGTERM`,
  interrupted-build-resumes; 1 in `guard-testing-techniques.md`: the coverage-probe-silence-
  criterion claim) — all 5 confirmed byte-exact in content, only retrievability affected. **Final
  corpus tally: 327 documents `ready`, 5 permanently `failed` (332 manifest-tracked claims across all 13 KB files).** The content-loss-check script
  itself is `coder`'s (U3e, **accepted and closed**, `docs/reviews/kb-content-loss-checker.md`),
  not this agent's. Full state, the manifest (`claude/cobb/scripts/kb-claim-manifest.json`), and
  the full checkpoint/retry history are in `history.md`, 2026-09-18 and 2026-09-19 (two entries).
- **Coordination ledger for the design pass:**
  `claude/docs/plans/agent-knowledge-base-strategy2-coordination.md`. **Coordination ledger for
  Track 2 Stage 6 itself:** `claude/docs/plans/agent-knowledge-base-strategy4-coordination.md`
  (U3).
- **Notes:** Opened 2026-09-09 from U42; rewritten 2026-09-10 after U45; Stage 0 delivered and
  ...
  arithmetic-error correction caught by `teco`'s independent re-verification), Stage 7, Stage 8, and
  Stage 9 all delivered, logged here 2026-09-19 (detail in `history.md`) — **Track 2 (K-030's own
  parent effort) is now fully closed**; only Track 1's team-wide write-convention cutover remains
  open under this item. Entry ids are not the trigger here; no `entryId` dedup applies.
...
- **Live evidence toward the unmeasured question (2026-09-16):** while fixing
  `docs/reviews/commit-granularity.md`'s findings this pass, a one-clause edit to
  `claude/AGENTS.md` (the finding-4 terminal-state mapping) was left uncommitted per the
  delegated-subagent rule — and was nonetheless swept into an unrelated concurrent commit,
  `702704f` ("chore(kaizen): distill frontend-engineer's 8 raw kaizen_team entries... (U2)"),
  whose own diff/message cover only frontend-engineer distillation work. Confirmed via
  `git diff 320f682 -- claude/AGENTS.md`: the terminal-state clause was already present in `HEAD`
  though this session never ran `git commit`. Content landed correct, but under a commit message
  that doesn't describe it — the exact shared-index hazard `claude/AGENTS.md` itself documents,
  observed here from the swept-in side rather than the sweeping side. One data point, not a
  measurement, but it argues the race is live and not merely theoretical for a file this central.
...
### K-033 — 2026-09-13 changes widened the `claude/AGENTS.md` overage K-031/K-032 already flagged, by roughly 9x the figure first logged

- **Status:** 🔵 proposed
- **Priority:** medium (raised 2026-09-16 from low — see below)
- **Rationale:** K-031/K-032 already note `claude/AGENTS.md` sat ~53 words past the root
  `AGENTS.md`-stated ~2,500-word smell as of 2026-09-10. **Corrected 2026-09-16**
  (`docs/reviews/commit-granularity.md` gated the original entry and flagged this figure as
  materially wrong; independently re-measured here, not copied from the review's table):
  `git show 320f682^:<path> | wc -w` vs. `git show 320f682:<path> | wc -w` gives
  `claude/AGENTS.md` **2,783→3,074 words (+291)**, `claude/teco/teco.md` **10,765→10,899 (+134)**,
  `claude/tico/tico.md` **5,623→5,809 (+186)**. The original entry claimed `claude/AGENTS.md`
  +33 (net of an offsetting footer-section trim) and `teco.md` ~450 — the real `claude/AGENTS.md`
  delta is ~9x the claim, and `teco.md`'s is ~3x smaller than claimed; `tico.md`'s ~250 was close.
  `claude/AGENTS.md` now sits at 3,074 words, **574 over** root `AGENTS.md`'s own ~2,500-word smell
  line — a real, current overage, not the near-negligible one the original +33 figure implied.
- **Priority reassessment:** raised low→medium. The original "low, folds into whichever of
  K-030/K-031/K-032 executes first" call was made on a figure that undercounted the actual growth
  by an order of magnitude; at 574 words over the smell line, this is no longer a marginal add-on
  to fold in opportunistically. It does not need its own dedicated unit — the fix is still the same
  trim K-031/K-032 already propose (K-032's ~35-word roster cut, prerequisite to K-031's move) —
  but that trim should be scheduled sooner rather than deferred to "whichever executes first," and
  whoever executes it should size the cut against the corrected 3,074-word baseline, not the
  original 3,074-implied-as-3,041 one.
- **Proposed change:** no separate unit — still folds into K-031/K-032 — but flag to whoever picks
  either up that `claude/AGENTS.md`'s actual current word count is 3,074 (not the ~3,041 implied by
  the original K-033 note) and that the overage is now significant enough to prioritize the K-032
  prerequisite trim rather than let K-031 add more words to the file before that trim lands.
- **Notes:** flagged per the maintenance duty to record smells found even on a narrow, otherwise-
  complete change; not blocking anything on its own. Correction logged in `history.md`,
  2026-09-16.
...
### K-052 — A mid-session `tools:` frontmatter edit doesn't reach a subagent spawned later in the same session

- **Status:** 🔵 proposed
- **Priority:** medium
- **Origin:** 2026-09-19, U1 (`agent-knowledge-base-strategy5-coordination.md`) — its own "close the
  loop" tool-visibility probe. A fresh `security-expert` subagent, spawned *after* adding
  `mcp__falkor-chat-agent-team__ingest_document` to its frontmatter `tools:` line, failed to
  resolve or call that tool twice (once self-reported, once on an actual attempted invocation) —
  and both times also reported `Grep`/`Glob` as absent, which are unambiguously granted in its
  pre-existing frontmatter, ruling out "the edit is wrong" as the explanation. A control probe of
  `teco` (whose `ingest_document` grant predates this session) succeeded on a real call in the
  same session. The deployed file itself is correct (`Read`/`grep`-confirmed against both the repo
  path and its `~/.claude/agents/` symlink target).
- **The fact, stated as a rule:** a subagent's available-tool set appears to be resolved once
  (session start, or first reference to that agent type), not re-read from disk on each `Agent`
  dispatch — so a `tools:` frontmatter change only reaches a subagent spawned in a **new** session
  that starts after the edit, not one spawned later in the same session that made the edit. This
  sharpens a caveat already in `claude/AGENTS.md`'s hook-machinery section ("a frontmatter change
  only takes effect for a freshly-spawned instance") — the unit that must be fresh is the
  *session*, not the dispatch.
- **Proposed change:** if reproduced again, promote this into `agent-maintenance` skill's guidance
  for verifying a `tools:`/hook frontmatter change (§4's certification pass, or wherever a future
  unit is told to "confirm at runtime") — the correct verification is a fresh top-level session,
