...
| K-030 | 2026-09-09 | medium | 🔵 | The always-loaded-prompt compaction backlog is **four agents**, not four separate follow-ups: `tdd-engineer.md:40`, `architect.md:51`, `data-scientist.md` (7 lines >700), and now `teco.md` (**14** lines >1,000 and **27** >700 after U43, longest **1,716**, 8,278 words; +583 at U42, +377 at U43 — and U43 pushed one line *over* the 700 bar rather than only extending lines already past it, which is the item compounding). The common mechanism is having **no knowledge base** — `teco`, `architect` and `data-scientist` have none, so every promotion can only fold onto the resident prompt. Decide the general question (knowledge base vs. restructure) before compacting any one file; `teco`'s answer is a stakeholder call, reserved as such in U42's brief. |
