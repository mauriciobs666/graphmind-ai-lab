...
| K-033 | 2026-09-13 | low | 🔵 | Net across two 2026-09-13 changes (footer enforcement, docs-only-chain commit batching), `claude/AGENTS.md` moved 3,041→3,074 words (+33 net — the footer section was trimmed of narrative the same session, offsetting most of the new commit-batching paragraph) — still past the K-031/K-032-flagged overage, marginally. `teco.md` and `tico.md` also grew (~450, ~250 words) implementing the same commit-batching behavior — real behavioral content, not distillation bloat, but adds to `teco.md`'s already-tracked K-030 growth series. No separate action — folds into whichever of K-030/K-031/K-032 executes first. |
...
### K-033 — 2026-09-13 footer-enforcement addition widens the `claude/AGENTS.md` overage K-031/K-032 already flagged

- **Status:** 🔵 proposed
- **Priority:** low
- **Rationale:** K-031/K-032 already note `claude/AGENTS.md` sat ~53 words past the root
  `AGENTS.md`-stated ~2,500-word smell as of 2026-09-10. The 2026-09-13 no-footer-enforcement
  addition (`history.md`, same date) added roughly 250 more words to the same section (git-commit
  authority), moving the file further from, not toward, the trim K-031/K-032 already propose.
- **Proposed change:** no separate action — fold into whichever of K-031/K-032 executes first; the
  trim those items already scope has room to absorb this addition too (the new prose is dense but
  not itself duplicative of anything, so it isn't its own trim candidate).
- **Notes:** flagged per the maintenance duty to record smells found even on a narrow, otherwise-
  complete change; not blocking anything.
