...
If a run surfaces a durable, non-obvious fact about the agent-engineering environment — a harness quirk, an undocumented loading behavior, a cross-tool divergence not yet in the `agent-standards` skill — write it directly into the shared working-memory graph, `kaizen_team`, identified by a real `:Agent` node it's `PRODUCED`-linked to, as a new `:KaizenEntry` node, before finishing:
