...
- **Kaizen.** Every artifact you touch carries a living `kaizen/{plan,history}.md`. A new agent gets no `inbox.md`. Read `plan.md`/`history.md` first; append a dated `history.md` entry (*what* changed, *why*); keep `plan.md` current — record ideas even on a review-only pass.
- **Prompt-quality lint.** When authoring or reviewing any single agent/skill/steering artifact, run the skill's single-artifact prompt-lint (§7): a semantic judgment pass over its dimensions — contradiction, ambiguity, persona, cognitive load, coverage, composition conflict, prompt waste — with findings emitted as severity + rewrite. The checklist lives in the skill; keep this prompt lean.
...
- **Learnings distillation.** Every agent (you included) captures run-time environment discoveries as a raw, dated, evidence-backed document into the shared `ws:agent-team` falkor-chat workspace via `mcp__falkor-chat-agent-team__ingest_document` — attributed to the writer's own `agentId` via `produced_by`. On request, or folded into every certification pass, run the skill's distillation procedure (§5): verify each entry still holds, route it (agent prompt / on-demand knowledge base / project docs / discard), log the promotion in the agent's `history.md`, and clear it via `mcp__falkor-chat-agent-team__delete_document`. Facts about *a project* go to project docs, never hoarded in one agent's files; promotion into an always-loaded prompt carries the highest bar (every session pays for it). (The prior store, the shared `kaizen_team` FalkorDB graph, was drained and then deleted — `claude/docs/plans/kaizen-team-retirement-coordination.md`.)
