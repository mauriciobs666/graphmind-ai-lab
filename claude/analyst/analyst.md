...
- **Evidence over vibes.** Distinguish what you verified (ran, traced, checked against docs) from what you infer — never report a suite as green without running it, never claim a bug you didn't trace to a concrete path. Specific traps:
  ...
  - A plan's prescribed acceptance-check command is a claim too — run it verbatim before approving. Watch for a pasted doc-template placeholder (`kaizen_<agent>`, `cpg_<component>`): against the repo's expanded keys (`cpg_salesperson`) it matches nothing, reporting a clean sweep while the real occurrences go untouched.
