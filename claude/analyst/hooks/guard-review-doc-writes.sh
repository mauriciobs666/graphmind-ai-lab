#!/usr/bin/env bash
# guard-review-doc-writes.sh
#
# PreToolUse guard for the `analyst` subagent (wired via its frontmatter
# `hooks:`, matcher `Write|Edit`). The analyst's contract is review-only: it
# never edits source, tests, config, or the artifact under review — its
# Write/Edit exist for one purpose: authoring/revising review documents.
# This hook enforces that contract in the harness: any Write/Edit whose target
# is NOT under a `docs/reviews/` directory (or the session scratchpad in /tmp)
...
# or plan edits via the editing tools is the realistic *accidental* failure
# mode this hook closes. Same rationale as architect kaizen K-003 (2026-07-08).
#
# Contract (same as architect/hooks/guard-plan-doc-writes.sh, verified
...
# Allowed targets: any docs/reviews/ directory (repo-root or per-component),
# or the session scratchpad / temp space.
case "$path" in
  docs/reviews/*|*/docs/reviews/*|/tmp/*)
    exit 0
    ;;
esac
...
msg="analyst guardrail: Write/Edit targets '${esc_path}', which is outside a docs/reviews/ directory (or /tmp scratchpad). The analyst is review-only — its Write/Edit are for review documents only, never source, tests, config, or the artifact under review. Approve only if this is genuinely a review artifact; otherwise the agent should put the change in its findings for the artifact's owner."
