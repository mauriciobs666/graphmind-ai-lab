...
- **`b7b3bc96-21fa-4d4c-a2ba-82619bb5d3ad` (2026-09-03) → `analyst.md`, folded into the Findings bullet.** The rule is about *authoring a recommendation*, which is what that bullet governs: when the improvement you suggest is a new guard or assertion, say where it goes — and never into the step it is meant to catch. Authored in the same change it guards, its declared set contains the new value from birth, so it can never redden; that is deleting the guard while keeping its name. Split it — assertion into a round before, obligation into the later step's row. `teco.md:94` already carried the coordinator's half of this (*"a reviewer's suggested fix is a finding to judge"*); the reviewer must not author it in the first place, and that half had no home.
- **`3f8b17d2-6c40-4e93-b1a7-5d29e08c6a44` (2026-09-06) → `review-techniques.md`, folded into §"'Verified by execution' names a level".** That section already covers the *judging* direction generally, and explicitly refuses to be narrowed to one instance. What it lacked is the **self-directed mirror**: *"checked, not guessed"* asserts the **method**, not the **scope**, and scope is where the defect lives. Added with the question that catches it in both directions — *is the rule the exemption STATES broader than the reach the mechanism IMPLEMENTS?* — and the tell, a reason string of the form *"no X reaches Y"*.
- **`f3a91c47-2b60-4d8e-9a15-7c0e4bb2d581` (2026-09-07) → `review-techniques.md`, folded into §"Byte-identity confirmation via an AST line-range hash".** That section covered the single-node technique (hash the node's line range; `ast.dump()` with docstrings stripped as a second pass). The entry scales it to a **whole-file done-condition for a prose-only unit over code** — compare `ast.dump()` **and** the docstring-owner set — with the consequence that makes it worth having (byte-equivalent executable code means every behavioural claim about the untouched mechanism *inherits its prior evidence*, so earlier measurements become proofs) and its bound (the proof covers code left alone; any figure the delegate READ from other source is outside it). `teco`'s U18b explicitly dropped this from `teco.md` and routed it here — *"a narrow unit type, and `analyst` is the agent that gates a prose-only change"*.
- **`b2d64f19-7e35-4a80-93c6-1af7c25b0e88` (2026-09-07), reviewer half → `review-techniques.md`, folded into §"A guard derived from the artifact it guards"** as a new paragraph ahead of that section's "The move". The gate question — *what is the smallest edit to production code that satisfies the docstring and survives the body?* — and, decisively, that an over-claiming docstring admits **two** correct closures (extend the mechanism, or narrow the claim), so a review that only ever recommends widening has pre-decided the cheaper half. The existing section carried the two-axes material and the mutate-the-source move but not the docstring-versus-body framing. **This entry carried two `MENTIONS` edges** (`analyst` and `tdd-engineer`); see the count-and-decide note below.
- **Verification.** Nothing was verified by re-reading an entry. The AST census the neighbouring material rests on was re-derived here under **two** definitions of the same count (CPython 3.12.3, system `python3`): intersecting every `ast.AST` subclass's `_fields` against the 8-name binding-field set gives **27**, against the entry's own 5-name set gives **22** — the 5 dropped are `Global`, `Import`, `ImportFrom`, `MatchMapping`, `Nonlocal`. The 27 matches the figure already in this file. The instrument disagreeing with itself under two definitions is the point: *"the grammar has N binding forms"* is not a fact without its field set beside it, and that caveat is now stated in `tdd-engineer`'s new knowledge base rather than here.
- **Graph:** each read before mutating. `b7b3bc96`, `3f8b17d2`, `f3a91c47` — 0 `PRODUCED` / 1 `MENTIONS` ⇒ `otherRemaining = 0` ⇒ full-node `DETACH DELETE`. `b2d64f19` — 0 `PRODUCED` / **2** `MENTIONS` ⇒ `otherRemaining = 0 + 2 − 1 = 1`, so the `analyst` edge was **resolved alone** and the node deliberately left alive for the `tdd-engineer` edge; cleared only after that second promotion landed.
- **Budget:** `review-techniques.md` **9,284 → 9,867 w** (+583) across three existing sections, **27 → 27** sections. `analyst.md` **2,580 → 2,656 w**, the host bullet 649 → 1,057 characters.
- **Docs touched:** `claude/analyst/{analyst.md,review-techniques.md,kaizen/history.md}`.
