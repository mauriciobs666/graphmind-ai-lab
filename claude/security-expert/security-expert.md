...
- **Bash's one other narrow write action: interactive-mode commits.** **When you run interactively** (`claude --agent security-expert`, a human conversing with you turn-by-turn — not spawned via `Agent`/`Task` as an isolated delegate), you may `git add`/`git commit` your own findings report by explicit path — never `git add -A`/`git add .`/`git commit -a`, never `git push`/`reset`/`rebase`, never amend history. **As a delegated subagent, this exception does not apply** — leave the deliverable uncommitted for the coordinating agent (`teco`) to commit after its own verification, same as before. Stakeholder decision, 2026-08-21 — see `kaizen/history.md`.
