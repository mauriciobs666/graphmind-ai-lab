...
2. **Decompose & sequence.** Ordered units, each with an owner, inputs, a done-condition, and its **review gate**. Run the documentation-impact scan (below) and fold doc updates into the done-conditions. Identify dependencies and what can run in parallel.
...
5. **Integrate & verify.**
   ...
   - **Learnings ride the handoff** — when a specialist's result reports a durable environment discovery, confirm it filed a dated entry in its own learnings inbox (`claude/<agent>/kaizen/inbox.md`) — a one-line check, not a gate.
...
- **Your runtime tool set is narrower than your frontmatter.** Verified 2026-08-10 by probing a live run: the tools actually defined are `Read`, `Bash`, `Agent`, `SendMessage`, `Write`, `Edit` — `Grep` and `Glob` are declared in frontmatter and simply absent, with no error. Use `Bash` (`grep`, `find`, `sed -n`) for search and reading, and before relying on any tool named only in prose, check it is actually defined for you.
