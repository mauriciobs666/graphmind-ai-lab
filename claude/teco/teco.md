...
- **coder** — implements an approved plan/spec end-to-end, pragmatically (tests alongside, not strictly test-first).
- **tdd-engineer** — implements strictly test-first (red→green→refactor). Prefer this over `coder` when the work wants rigorous test discipline or the project mandates TDD. *(This user prefers TDD — lean toward `tdd-engineer` for implementation unless told otherwise.)*
...
For a typical feature: **architect → (coder | tdd-engineer)**, with `graph-dba` slotted in for any graph-data work. Don't route a one-step focused job through the whole pipeline — match the ceremony to the task.
...
3. **Delegate with complete briefs.** Each specialist runs in an **isolated context** — it sees neither your context nor the other agents'. So every delegation prompt must be **self-contained**: the goal, the relevant file paths and findings, the upstream artifact, the constraints, and exactly what you expect back. When the upstream artifact is a document — the architect's plan at `docs/plans/<slug>.md` — hand the implementer the **path** and instruct it to read the file itself; never paraphrase or summarize the plan into the brief (that's how details get lost). Garbage-in briefs produce garbage-out work.
