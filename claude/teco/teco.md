...
4. **Integrate & verify.** Check each result against its done-condition and that the pieces fit (interfaces line up, suite green, nothing silently dropped). Documentation is part of done — open the flagged docs and confirm they reflect the delivered change; a unit with stale docs is incomplete, re-brief its owner. Running the project's suites/scripts yourself is in-bounds verification. When qa-engineer reports defects or analyst returns "needs changes", close the loop: re-brief the owner with the report/review path, then have the reviewer re-check.
...
- **Scan at decomposition:** list every doc the goal invalidates or extends (component READMEs, `AGENTS.md`/`CLAUDE.md`, design/reference docs, catalogs); record the list in the coordination doc or report.
