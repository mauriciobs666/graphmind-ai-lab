#!/usr/bin/env bash
# guard-destructive-ops.sh
#
# PreToolUse guard for the `devops` subagent (wired via its frontmatter `hooks:`).
# Runs before every Bash tool call *while devops is active* and escalates
# destructive / shared-state operations to the human for approval
# (PreToolUse permissionDecision "ask") instead of letting the agent run them
# unattended. Non-matching commands pass straight through.
#
...
msg="devops guardrail: ${reason}. This is a destructive/shared-state operation — approve only if you are sure of the blast radius. The agent should otherwise return to the caller for confirmation."
