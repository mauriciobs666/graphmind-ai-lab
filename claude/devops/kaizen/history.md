...
- **`bf3935da` (2026-09-02, WSL2 `npm`-without-`node`) — promoted to `claude/devops/ops-quirks.md`, generalized.** Re-derived live 2026-09-07 and it **still reproduces exactly**: `node` is not found, `npm` resolves to `/mnt/c/Program Files/nodejs/npm` (11.11.0). Two corrections the re-derivation forced: (1) the entry's "Fixed by installing the official Linux tarball … and prepending its bin dir" reads as settled, but the fix **did not persist** — `~/.local/node/current/bin` exists (Node v24.20.0) and is on no `PATH` and in no shell rc, so a fresh shell reproduces the hazard; the durable resolution that actually shipped is `salesperson/scripts/install_node.sh` plus `salesperson/build.sh` resolving the interpreter itself. (2) The project-specific instance is **already fully documented** in `salesperson/README.md` § Prerequisites and `salesperson/AGENTS.md`, so promoting it as a project fact would have been a duplicate. What is *not* documented anywhere `devops` can see it is the generalized rule — probe for `node`, never `npm`; reject any interpreter under `/mnt/` — and `devops` is user-scoped, so the salesperson docs reach it in exactly one component out of every project it runs in. The knowledge-base entry states the rule and **cites** the worked instance rather than restating it.
- **`a7dfd39c` (2026-09-02, npm 11.19.0 arborist crash) — promoted to `claude/devops/ops-quirks.md`.** Corroborated rather than re-run (re-running the failing install would churn `node_modules` for no added certainty): the installed Linux npm is exactly **11.19.0**, and `salesperson/package.json` carries `vitest ^4.1.11` with **no `@vitest/ui`** — precisely the workaround shape the entry describes. Not documented anywhere in project docs (`grep -i "arborist\|ERESOLVE\|vitest/ui"` over `salesperson/{AGENTS,README}.md` and `docs/` is clean). Version-stamped in the knowledge base: the npm bug will be fixed upstream, but the diagnostic pattern — an internal `TypeError` standing in for a peer conflict — outlives it.
- **`8301b20f` (2026-08-23, `cypher-mcp/server.py`) — discarded; superseded by shipped code.** Pass 1 (`cobb`, 2026-08-25) had already disproven claim (1) — `_CURATOR_CLEAR_RE` does *not* require a literal space after `entryId:` — and tagged the entry `MENTIONS→devops` on the strength of claim (2). Re-deriving claim (2)'s three named targets today shows two of the three **already delivered**, in commit `b616c0a` (2026-08-23 18:32, i.e. hours after the entry was written): `authorize_write()`'s fallback now names the trailing-clause trap specifically and echoes the offending excerpt (`server.py` 482–492), and **two** RETURN-specific regression tests exist (`test_producer_write_with_trailing_return_clause_gives_specific_trailer_message`, `…_trailing_return_k_…`). `cypher-mcp/README.md` documents the trap and the new message (167–174). Only the third target survives: `TOOL_DESCRIPTION`/`SERVER_INSTRUCTIONS` (`server.py` 121–153) still omit the restriction — re-read both blocks end to end, confirmed. **Discarded rather than kept open**, deliberately: both strings are injected into every session and every subagent spawn, so a caveat there is paid always; the trap now self-reports at the exact point of failure with the offending text, at zero cost when not hit; and both live bites on record (`teco`, `cobb`) predate that message. `server.py` is outside `cobb`'s write remit in any case. Recorded here rather than dropped silently so the trade is greppable if it is ever revisited.

- **No `MENTIONS` tags added.** All three are squarely devops-discipline; none turned out to be about a different agent.
- **Graph state after this pass:** edge counts read before each mutation — `bf3935da` 1 `PRODUCED` / 0 `MENTIONS`, `a7dfd39c` 1 / 0, `8301b20f` 0 / 1 (its `PRODUCED` edge resolved by pass 1). `otherRemaining == 0` for all three, so each was cleared with the full-node curator `DETACH DELETE`. `devops` now has zero uncleared entries in `kaizen_team`.
- **Docs touched:** `claude/devops/{ops-quirks.md,devops.md,kaizen/{plan,history}.md}` · `claude/AGENTS.md` · `claude/README.md`.
- **Plan items:** none opened — nothing was kept open. One parking-lot observation recorded (`build.sh`'s `command -v node` fallback branch does not apply the `/mnt/` rejection the knowledge-base rule states).
