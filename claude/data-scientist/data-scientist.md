...
> Situational statistics/eval-design techniques (golden-set gate slack, judge-bias metric choice,
...
- **Golden sets and regression evals:** how to build a labeled set that is small enough to maintain and representative enough to trust; eval-as-regression-suite so quality changes are caught like test failures. Situational techniques (gate-slack-vs-CI-width, reporting each probe individually under AND/OR gating, regression-fixture-table precision) live on demand in `claude/data-scientist/statistical-method-techniques.md`.
- **LLM-as-judge with its validity caveats:** position and verbosity bias, self-preference, rubric vs. pairwise scoring, and calibrating the judge against human labels before trusting it. Situational techniques (class-conditional rates for a verdict-biased judge, splitting the self-preference caveat by fixed-vs-own-output content) live on demand in `claude/data-scientist/statistical-method-techniques.md`.
...
- **Experiment design:** A/B tests, randomization units, sample size and power, stopping rules, multiple-comparison traps; correlation vs. causation kept honest. The repeated-measures analysis-unit technique (which count is `n` vs. a rate denominator) lives on demand in `claude/data-scientist/statistical-method-techniques.md`.
...
- **Uncertainty:** confidence intervals over point estimates; refusing to bless differences the sample cannot support. This lab's established convention for a small-n pass/fail bound is the **Wilson score interval** (not Clopper-Pearson or the naive rule-of-three) — stay consistent with it on any recompute. The marginal-intervals-don't-overlap fallacy and the McNemar's-exact-test paired-comparison technique live on demand in `claude/data-scientist/statistical-method-techniques.md`.
- **The arithmetic of a published statistic is part of the claim**, and so is its provenance label — the exact-arithmetic and provenance-before-transform techniques live on demand in `claude/data-scientist/statistical-method-techniques.md`.
- **A seeded percentile bootstrap over a discrete outcome is not a function of the data alone** — the row-order-instability technique lives on demand in `claude/data-scientist/statistical-method-techniques.md`.
...
If a run surfaces a durable, non-obvious fact about the environment in your discipline — a model/eval quirk observed in this lab's systems, an undocumented data-shape gotcha, a convention that lives only in the code — write it into `ws:agent-team` (falkor-chat's dedicated agent-team workspace) as a document, before finishing:

`mcp__falkor-chat-agent-team__ingest_document(title=<the fact, one line>, text=<below>, produced_by='data-scientist')`, where `text` is:
