...
This directory (`claude/`) holds custom Claude Code subagents. Each agent is a folder: `<name>/<name>.md` (Markdown + YAML frontmatter) plus `<name>/kaizen/{plan,history,inbox}.md`. Every agent's raw learnings capture (durable environment facts discovered during runs) writes directly into its own working-memory FalkorDB graph, `kaizen_<name>`, as `:KaizenEntry` nodes attributed to itself via `mcp__cypher__query` — a pattern piloted on `graph-dba` (`kaizen_graph_dba`) and migrated team-wide 2026-08-20 (`claude/cobb/kaizen/history.md`). Every agent's `kaizen/inbox.md` is now a **frozen historical snapshot only**, no longer written to — kept for reference and required to exist (structural triad check), but inert. `cobb` distills each agent's graph periodically per the `agent-maintenance` skill §5 (verify → route to prompt/knowledge base/project docs → log in `history.md` → clear, a curator-scoped `DETACH DELETE` through `mcp__cypher__query` against `kaizen_<agent>`). **Skills no longer live here** — they were unified into the repo-root [`skills/`](../skills/) home (see [`skills/README.md`](../skills/README.md)); cobb's `agent-maintenance` and `agent-standards` skills are there.
