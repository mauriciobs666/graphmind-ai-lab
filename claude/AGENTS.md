...
yet, and that is the only staging a path-limited commit needs. **A path-limited commit ignores the index for every path it
names:** it takes that path's *whole* working-tree content and rewrites the index to match,
discarding whatever was staged there — a partial hunk, or a blob that never existed in the working
tree at all. A `git diff --cached` beforehand therefore proves nothing about what lands (observed:
it counted one appended section; the commit shipped three), and a concurrent session's edit to the
same file is committed under your message. So never stage a path you then name in one — and never
reach for the no-pathspec `git commit` to get the staged version instead: that is the shared-index
race above, and it sweeps every other session's staged files with it. Other paths' staged entries survive untouched either way.
