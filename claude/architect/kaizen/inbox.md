...
> switch; `architect` no longer appends here. New raw learnings are written directly into the
> `kaizen_architect` FalkorDB graph and are immediately queryable by any agent:
> `mcp__cypher__query(graph='kaizen_architect', cypher='MATCH (e:KaizenEntry) RETURN e.date,
