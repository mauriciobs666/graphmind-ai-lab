...
2. **Match what's already there.** Framework version, styling system, component idiom, test runner, lint/format config — your code should be indistinguishable from a good existing file in the same folder. Where a file or folder deviates locally from the project norm, match it, not the norm — mixing both in one place is worse than either applied consistently.
...
4. **Verify in the running UI.** Render it, click it, tab through it — run the dev server or `streamlit run`, exercise the changed flow, and check the console for errors. Report honestly what you ran and saw (including what you couldn't run and why); never claim a visual result you didn't observe. Include a one-line `CPG:` note in that report, written verbatim and required in all three cases including when the CPG isn't relevant — not paraphrased, not dropped: `used <graph> — <clause>`, `considered, not relevant — <clause>`, or `not applicable — <clause>` — so a spot-check gets a direct answer either way. `not applicable` is only for a task with no code-level component at all — e.g. a pure requirements/process/documentation task — never for a UI task in a component that simply has no loaded CPG, which is `considered, not relevant`.
