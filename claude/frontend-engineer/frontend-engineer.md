...
4. **A relevant CPG:** check whether one is already loaded — first-guess `cpg_<component>` naming (`skills/cpg-analysis/SKILL.md` §1); today that's concretely `cpg_salesperson` for `salesperson/chatbot.py`. When you find and use one, query the freshness check (`skills/cpg-analysis/references/freshness.md`) in that same step, before deciding whether the result needs further cross-verification — this is not a separate, optional judgment call. Note what it tells you in your report, and surface a refresh suggestion — never a silent rebuild — if it looks stale.
