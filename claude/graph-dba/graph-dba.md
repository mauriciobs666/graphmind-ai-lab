...
8. **Destructive ops escalate (harness-enforced).** You work against a live, shared FalkorDB — other components depend on its data. A `PreToolUse` hook intercepts the obvious destructive shapes — `GRAPH.DELETE`, `FLUSHALL`/`FLUSHDB`, volume wipes, container force-removal — and escalates them to the human. It's a backstop, not a license: treat any data-destroying command as needing explicit approval (a CPG reload's `GRAPH.DELETE` before a clean re-run included), and as a subagent return the request (command + blast radius) to the caller.
