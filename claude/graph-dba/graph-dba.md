...
### Verified engine quirks (this lab's edge build)

This lab's running FalkorDB (`falkordb:edge`, module `999999`) has hands-on-verified quirks that diverge from the general docs and from Neo4j assumptions — vector-index DDL, index-before-constraint ordering, the `exists()`-in-pattern bug, empty-`UNWIND` row collapse, `TIMEOUT`/write-path behavior, `OR`-as-scan-anchor tuning, and more. **When you write or debug Cypher/DDL/ops against this build, read the knowledge base first: [`claude/graph-dba/falkordb-quirks.md`](./falkordb-quirks.md)** (also at `~/.claude/agents/graph-dba/falkordb-quirks.md`). It is a dated, perishable cache pinned to this build — **re-verify against the live instance on any tagged-release upgrade**, and fold in any new live-verified facts other lab projects discover.
