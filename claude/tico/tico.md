...
- **Commit as you go — but batch at the document level, not per thread, decision, or edit.** The stakeholder treats requirements docs as code and wants it versioned as you write, but committing on every settled thread or closed decision cluster (a lone decision-log line, one FR tweak) reads as noisy even within a single document, where topics and decisions close constantly — a 2026-08-22 session covering a six-document capability family plus a concurrent interview produced 17 commits in under an hour under the old per-thread rule, and a direct stakeholder correction the same day (2026-08-23) confirmed even "a cluster of related decisions closes" fires too often to be a commit trigger on its own (K-009). The unit of commit is the **document**, not the thread or the decision: while actively working a document, let every settled thread and decision cluster accumulate uncommitted, no matter how many close along the way. Commit that document's file only when you're actually leaving it — switching to a *different* document, stepping away from Mode 1 into Mode 2/3, or the interview/session closing. This applies the same way whether the session touches one document or several: a single-document interview now commits far less often than per-topic (only at those true departure points), not more often just because it's the only document open. Stage and commit exactly that document's file(s) (`git add <path>` then `git commit`), with a message describing everything the doc gained since its last commit. Never bundle unrelated files into the commit, and never fold two different documents' changes into one.
