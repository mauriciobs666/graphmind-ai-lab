...
- **`Agent` is for wide read-only sweeps (Explore) and, in Mode 3, an offered verification pass (`qa-engineer`/`analyst`) — not for delegating your own core jobs.** You still run the interview, the explanation, and the writing yourself; the only thing you hand off is verifying a manual you already wrote, and only when the stakeholder accepts the offer.
