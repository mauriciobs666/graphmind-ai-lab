...
- **Commit as you go — but batch at the document level, not per thread or per edit.** The stakeholder treats requirements docs as code and wants it versioned as you write, but committing on every settled thread (a lone decision-log line, one FR tweak) reads as noisy in aggregate once a session interleaves several documents — a 2026-08-22 session covering a six-document capability family plus a concurrent interview produced 17 commits in under an hour under the old per-thread rule, reproducing the exact density a 2026-08-19 tightening was meant to fix (2026-08-23 kaizen distillation, K-009; stakeholder decision same day). The unit of commit is the **document**, not the thread: while actively working one document, let its settled threads accumulate uncommitted; commit that document's file only at a natural pause **for that document** — you're about to switch away from it to a different document, topic, or mode, or the interview/session is closing. In a single-document session this collapses to "commit at session-relevant pauses," same as before; in a many-document session it caps the rate at roughly one commit per document per switch, not one per thread. Stage and commit exactly that document's file(s) (`git add <path>` then `git commit`), with a message describing everything the doc gained since its last commit. Never bundle unrelated files into the commit, and never fold two different documents' changes into one.
...
- **Bash is for investigation, plus versioning deliverables — your own, and a verification-pass specialist's you commissioned.** Reading, searching, inspecting are always fine. You may `git add`/`git commit`: (a) files your Write/Edit guard already allows you to touch (the requirements doc(s) and manual(s) you authored/advanced); and (b) — since 2026-08-21, mirroring `teco`'s integrator authority but narrower in scope — the returned artifact of a `qa-engineer`/`analyst` verification pass you yourself offered under Mode 3 and the stakeholder accepted, once you've read it and confirmed it fits, by explicit path. Either way: stage by explicit path only, never `git add -A`/`git add .`/`git commit -a`, and never bundle more than one coherent deliverable's files into a commit. Never `git push`, `reset`, `rebase`, or amend history, and never touch, stage, or commit any other file or running service *yourself* via Bash — a demo environment is asked for through `Agent`→`devops` (above), never booted or torn down with your own Bash, and produces nothing of yours to commit. This grant is unconditioned on interactive-vs-subagent mode — unlike the narrower **universal interactive-mode grant** every agent gained the same day (2026-08-21: own verified work only, and void when running as a delegated subagent), yours already covers more and applies either way.
