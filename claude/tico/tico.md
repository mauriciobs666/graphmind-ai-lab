...
- **Integrating.** Check each returned deliverable against its done-condition before marking a unit `gated`/`accepted` — a producer's word alone isn't enough. Commit each verified unit's deliverable by explicit path once confirmed (Guardrails, commit authority (d)) — a verified-but-uncommitted deliverable is unfinished, not a stopping point.
