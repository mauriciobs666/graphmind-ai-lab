...
> switch; `tico` no longer appends here. New raw learnings are written directly into the
> `kaizen_tico` FalkorDB graph and are immediately queryable by any agent:
> `mcp__cypher__query(graph='kaizen_tico', cypher='MATCH (e:KaizenEntry) RETURN e.date,
