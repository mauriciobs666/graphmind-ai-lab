...
- **`7f3c9a21-5e84-4b1d-9c62-0a8f16d4b3e7` (2026-09-09) — promoted to `skills/python-web-quirks/SKILL.md`,** one new section placed immediately after the existing `TestClient`-teardown section.
- **Re-derived by execution, not by citing the entry or falkor-chat's own test** — a standalone Starlette app (no falkor-chat code, no server, no FalkorDB), one route raising an unhandled `RuntimeError`, a `/ok` control returning 200 in every arm. Observed: `TestClient(app)` raised `builtins.RuntimeError` **into the calling test** with no response object; `TestClient(app, raise_server_exceptions=False)` returned `500` / `text/plain; charset=utf-8` / `'Internal Server Error'`; and — the half the entry only asserted — a real `httpx.Client` against the same app under a live uvicorn server received **byte-identical** `500` / `text/plain; charset=utf-8` / `'Internal Server Error'`. starlette 1.6.0, httpx 0.28.1, uvicorn 0.52.1, CPython 3.12.3.
- **The re-derivation sharpened the entry rather than merely confirming it.** Reading the mechanism (`starlette/testclient.py:348-363`) showed the flag does not *construct* the 500 — `ServerErrorMiddleware` sends it before re-raising, and the default discards the already-captured response. So the client-parity is the middleware's, not the flag's. Probed the boundary: against a **bare ASGI callable** with no `ServerErrorMiddleware`, `raise_server_exceptions=False` falls to the transport's synthesized fallback and yields `content-type = None`, `body = ''`, where the live server still returns `text/plain; charset=utf-8` / `Internal Server Error`. The entry's advice ("pass the flag when the question is what a client gets") is therefore true for a real Starlette/FastAPI app and **false as an absolute** — promoted with that bound stated, so nobody asserts on a body outside the middleware's reach.
- **The `fastapi` half verified separately,** since fastapi is installed in no venv here: `fastapi/testclient.py` read out of the fastapi 0.141.1 wheel is the single line `from starlette.testclient import TestClient as TestClient`, so the starlette-only reproduction covers both import paths.
- **Home: the skill, not this agent's own `qa-testing-techniques.md`.** That knowledge base's header scopes it to *"this lab's environment"* — WSL2 browser stack, `tmux`, this repo's MCP surface — and every section in it is machine- or repo-specific. This fact is portable and version-pinned, which is `python-web-quirks`' stated charter verbatim. Routing it here would have made a fifth copy of a fact that belongs in one place.
- **Two sections, not one merged `TestClient` section.** The neighbour is about the portal's *teardown* cancelling tasks; this is about *exception propagation* — different mechanisms, different consequences, different fixes (assert on app-produced state vs. pass a constructor flag). They are bound instead by a shared opening line naming the class (`TestClient` is not a real client; each convenience is separately opt-outable). The accretion risk is real and is tracked as `cobb` K-026, not left to chance.
- **Audience ruling — the list was stale, and it is the only routing surface left.** `SKILL.md` named `coder, tdd-engineer, architect, analyst`; `qa-engineer` produced this entry from its documented core mode (black-box driving of a running app). Decisive evidence: `git log -S` shows `6b0a401 chore(claude): trim agent frontmatter descriptions` removed the reciprocal `python-web-quirks` routing clause from **all four** consumer prompts, and `grep` confirms no agent prompt names the skill today — so the skill's own `description` is now the sole signal any router sees. An agent absent from it has no route to the file. The list was stated in **four** places (`SKILL.md` description, `skills/README.md`, root `AGENTS.md`, and per-agent sentences in `claude/README.md`); all four now name `qa-engineer` — checking a bound means checking every place it is stated. **Caveat found while verifying that fix:** Claude Code truncates the skill listing at 1,536 chars and this description is 4,584, so the frontmatter half of the fix sits past the cut and does not reach a router; the three prose catalogs are unaffected and carry it. Filed as `cobb` K-028.
- **Graph:** 1 `PRODUCED` (`qa-engineer`) / 0 `MENTIONS` ⇒ `otherRemaining = 1 + 0 - 1 = 0` ⇒ full-node `DETACH DELETE`, run only after this entry landed.
- **Docs touched:** `skills/python-web-quirks/SKILL.md`, `skills/README.md`, `AGENTS.md`, `claude/README.md`, `claude/qa-engineer/kaizen/history.md`, `claude/cobb/kaizen/{history.md,plan.md}`.
- **Plan items:** none opened for `qa-engineer` (the fact is fully promoted and needs no follow-up here).
...
- **Gate (a) inventory — all preserved:** grant scope and objects (test plan/report or authored test file, explicit path), full never-list, delegated-subagent carve-out + audit check-8 tokens, freshness rule both branches, per-agent variation "(defects belong in the test report, not here)", Cypher template + call line verbatim, "raw capture: `cobb` promotes; never edit your own definition".
