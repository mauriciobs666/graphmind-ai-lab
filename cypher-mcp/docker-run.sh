#!/usr/bin/env bash
set -euo pipefail
# docker-run.sh — launch the `cypher` MCP server in a container, speaking MCP over stdio.
#
...
#     The one call without the redirect is `cypher_mcp_image_tag`, a sourced shell function
...
#
# Env (all optional): FALKORDB_HOST/_PORT, CYPHER_MCP_MAX_ROWS/_MAX_CELL/_MAX_CHARS/
# _TIMEOUT_MS — forwarded into the container if set, else the image defaults apply.
#   CYPHER_MCP_IMAGE=<ref>       run this exact image, bypassing the hash gate entirely
#   CYPHER_MCP_NO_AUTOBUILD=1    never build here; fail with "run cypher-mcp/build.sh"
...
if ! command -v docker >/dev/null 2>&1; then
  echo "cypher-mcp/docker-run.sh: docker not on PATH. Fall back to cypher-mcp/run.sh (host venv)." >&2
  exit 1
fi
if ! docker info >/dev/null 2>&1 </dev/null; then
  echo "cypher-mcp/docker-run.sh: Docker daemon not reachable (check 'docker context ls'). Fall back to cypher-mcp/run.sh." >&2
  exit 1
fi

# --- resolve the image: content hash unless the caller pinned one ------------------
PINNED=0
if [ -n "${CYPHER_MCP_IMAGE:-}" ]; then
  IMAGE="$CYPHER_MCP_IMAGE"
  PINNED=1                              # "the caller knows what they are running"
else
  # shellcheck source=image-tag.sh
  source "$HERE/image-tag.sh"
  if ! cypher_mcp_image_tag; then          # sets CYPHER_MCP_TAG; writes nothing to stdout
    echo "cypher-mcp/docker-run.sh: cannot compute the image tag (see above). Fall back to cypher-mcp/run.sh." >&2
    exit 1
  fi
  IMAGE="${CYPHER_MCP_IMAGE_REPO:-cypher-mcp}:$CYPHER_MCP_TAG"
fi
...
if ! docker image inspect "$IMAGE" >/dev/null 2>&1 </dev/null; then
  if [ "$PINNED" -eq 1 ]; then
    # CYPHER_MCP_IMAGE bypasses the gate, so autobuilding here would build the CONTENT-HASH
    ...
    echo "cypher-mcp/docker-run.sh: CYPHER_MCP_IMAGE=$IMAGE is not present locally." >&2
    echo "  CYPHER_MCP_IMAGE bypasses the content-hash gate, so nothing will be built for it." >&2
    echo "  Either build/load that image yourself, or unset CYPHER_MCP_IMAGE to use the gate" >&2
    echo "  (cypher-mcp/build.sh), or fall back to the host venv: cypher-mcp/run.sh." >&2
    exit 1
  fi
  if [ "${CYPHER_MCP_NO_AUTOBUILD:-0}" = "1" ]; then
    echo "cypher-mcp/docker-run.sh: image $IMAGE not built and CYPHER_MCP_NO_AUTOBUILD=1. Run: cypher-mcp/build.sh" >&2
    exit 1
  fi
  echo "cypher-mcp/docker-run.sh: $IMAGE not present — building (first run after a change may take ~15 s)." >&2
  ...
  #
  # CYPHER_MCP_NO_PULL=1: this autobuild must stay local-only. Without it, build.sh
  ...
  # image store (an interactive `cypher-mcp/build.sh` put it there), so this changes
  # nothing when it matters and removes the unbounded pull when it doesn't. The
  # explicit `cypher-mcp/build.sh`, run by hand without this override, stays the thing
  # that refreshes the base image. See docs/reviews/cpg-mcp-containerization.md M-8.
  if ! CYPHER_MCP_NO_PULL=1 "$HERE/build.sh" --runtime-only >&2 </dev/null; then
    echo "cypher-mcp/docker-run.sh: build of $IMAGE FAILED (BuildKit output above)." >&2
    echo "  Offline? A build needs the network unless python:3.12-slim is already in the local image store." >&2
    echo "  Fall back to the host venv: cypher-mcp/run.sh (see cypher-mcp/README.md), or retry with more" >&2
    echo "  startup budget: MCP_TIMEOUT=60000 claude" >&2
    echo "  ...or run cypher-mcp/build.sh once with a network connection to refresh the local image store." >&2
    exit 1
  fi
fi
...
for _v in FALKORDB_HOST FALKORDB_PORT \
          CYPHER_MCP_MAX_ROWS CYPHER_MCP_MAX_CELL CYPHER_MCP_MAX_CHARS CYPHER_MCP_TIMEOUT_MS; do
