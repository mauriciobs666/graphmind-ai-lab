...
| [`cpg-analysis`](./cpg-analysis/SKILL.md) | **Consumer** counterpart to `joern-cpg`: queries an already-loaded CPG in FalkorDB with Cypher (read-only for CPG graphs — the tool's narrow kaizen-scoped write path can never match one) through the **`mcp__cypher__query` MCP tool** (one tool, two parameters: `graph`, `cypher`; `EXPLAIN` supported, `PROFILE` refused), with **`redis-cli GRAPH.QUERY` kept as the documented fallback**. Lean `SKILL.md` core (query surface + shared CONTAINS→CALL / `REACHING_DEF` / symbol def-ref idioms + the topology gotchas) plus five copy-adaptable `references/` recipes — impact analysis, root-cause analysis, code review (taint), test-gap, and the freshness check (`teco`'s dispatch-time gate on whether a graph is current enough to trust). Cites `joern-cpg/references/cpg-model.md` as the one schema source; live-verified against a `pysrc2cpg` CPG. ⚠ **The MCP wiring is Claude-Code-only** (repo-root `.mcp.json`); under OpenCode/Kiro the `redis-cli` fallback is the only path — see the portability note below. | Impact / root-cause / taint / test-gap questions over a loaded CPG — when analyst, architect, qa-engineer, coder, tdd-engineer, or frontend-engineer need call-graph or data-flow answers instead of reading files. Building/loading a CPG routes to `graph-dba`. | `graph-dba` (M2) |
| [`python-web-quirks`](./python-web-quirks/SKILL.md) | Live-verified Python gotchas, version-pinned per entry — mostly web/async-framework, plus several general pytest/import-timing traps that surfaced in the same codebases: `asyncio.create_task` fire-and-forget GC-safety, Starlette/FastAPI `BackgroundTasks`' bounded thread-pool concurrency (anyio's default limiter, `total_tokens` 40, settable but only from inside a running event loop — and silently accepting `0`, which deadlocks every sync call the app offloads) vs. an unbounded raw `threading.Thread`, uvicorn defaulting `proxy_headers=True` so `ProxyHeadersMiddleware` rewrites `scope["client"]` from `X-Forwarded-For` — falsifying any "this request came from localhost" gate, FastAPI/pydantic `response_model_exclude_unset` silently dropping defaulted fields on **nested** models, FastAPI's four built-in doc routes (and which constructor kwargs suppress them) falsifying any "this app registers only routes X" claim while an *included* router's routes are absent from `app.routes` altogether (and `include_router(prefix=…)` never reaches the inner `.path`), so a route-table assertion can be unfalsifiable rather than merely wrong, a FastAPI `GET` route not matching `HEAD` at all (starlette's `Route` adds it, `APIRoute` does not) so `HEAD` falls through to any path-matching `Mount` — reading as a **404** where every other method reads as a `405` indistinguishable from a real method rejection, `responses={...}` being keyed by status code alone so two error bodies at one status collapse into one declaration — while an `x-` extension inside a status entry rides through verbatim onto `route.responses` and into `app.openapi()`, which is how a finer-than-status discriminator can still be asserted, FastAPI/Starlette's exception-handler registry (MRO lookup, the default keyed on `starlette.exceptions.HTTPException` and not `fastapi.HTTPException`, and a second `add_exception_handler` on one type replacing the first silently but readably), stdlib `urllib`'s `HTTPError`/`URLError`/`TimeoutError` connect-phase exception taxonomy plus a read-phase gap one layer down — `http.client.IncompleteRead` is not an `OSError` subclass, a `try/except` wrapped only around `urlopen()` leaves `resp.read()` unguarded outside it, and `HTTPError` is itself a response object whose own `exc.read()` can fail independently of the connect-phase guard — alongside redis-py's *sibling* `TimeoutError`/`ConnectionError` — both falsifying a handler map claimed total by exception type, `json.loads` parsing bare `NaN`/`Infinity`/`-Infinity` by default so a numeric-coercion boundary must reject non-finite floats via `math.isfinite` explicitly, a `typing.Protocol`'s declared methods being derivable from `vars(cls)` as plain callables for a coverage-equals-declared-set test with no hand-written method list, pytest's default capture making an in-test `input()` call raise `OSError` (`DontReadFromInput`) rather than block or read — worth recognizing when a mutation-testing kill shows up with that traceback shape instead of `AssertionError`, an OpenAI-compatible local server's HTTP-200 error envelope on a missing `/v1` prefix, a fence-fragile `json.loads` LLM-judge parser, an env var set *after* import — a `monkeypatch.setenv` or a plain script's own `os.environ` assignment inside `main()` — as a no-op against an import-time-frozen constant, a bare `pytest.raises` with no `match=` passing on an unrelated same-type raise from an earlier check in the same validator, a whole-module-only test that gates itself on `config.option.keyword` instead of on what `request.session.items` collected — so a node id, `--lf` and `--deselect` all slip past the guard, a function-local deferred import re-resolving fresh on every call vs. a def-time-bound default argument, a one-way circular import failing in every load order unless the deferred import sits in a function body, and `TestClient`'s teardown cancelling every still-running task regardless of the app's own lifespan while its default `raise_server_exceptions=True` re-raises an unhandled server exception into the calling test — so the bare `text/plain` 500 a real client receives cannot be asserted at all, and the client-parity that `False` buys is `ServerErrorMiddleware`'s rather than the flag's, since outside that middleware `TestClient` synthesizes a headerless, empty-bodied 500 where the wire still carries `Internal Server Error`, anything a **lifespan** assigns to `app.state` being absent until the `with TestClient(app)` block is entered — an `AttributeError` out of `State.__getattr__`, not a stale value — so a monkeypatch on it must go *inside*, and whether it then reaches the route is decided by the router's closure shape, bounding a call whose deadline the code under test computes with a **daemon thread + `join(timeout)`** rather than an elapsed-time assert a hang never reaches — stamping the start instant *inside* the thread body, since one taken before `Thread.start()` absorbs the scheduling gap and passes on a call that did nothing, and an application lock held across `ThreadPoolExecutor.submit()` delaying interpreter exit by exactly the lock's hold time rather than deadlocking it — nothing ever holds `_global_shutdown_lock` across a join, so the plausible three-party cycle does not exist — while `submit()` enqueues the work item *before* it grows the pool, so its `can't start new thread` `RuntimeError` leaves the job running where the same-typed post-shutdown `RuntimeError` does not, and one compensating `except` branch cannot serve both. | Writing, reviewing, or planning asyncio fire-and-forget scheduling, background-task dispatch, a FastAPI response model relying on `exclude_unset`/`exclude_none`, an HTTP client against `urllib`/an OpenAI-compatible endpoint, an LLM-as-judge JSON parser, an assertion over an app's route table or its `responses={...}` declarations, a method-matching or static-mount-shadowing question, a test that must bound a possibly-hanging call, an exception-handler/error-envelope map over a web app, a lock-ordering or shutdown-deadlock question over a thread pool, a pytest fixture/monkeypatch touching an env var or a deferred/circular import, or an acceptance assertion on what a client actually receives from an unhandled server error — for `coder`, `tdd-engineer`, `architect`, `analyst`, or `qa-engineer` in a Python codebase. | `cobb` (analyst + coder inbox distillations, 2026-08-09 / 2026-08-11) |
