...
| [`python-web-quirks`](./python-web-quirks/SKILL.md) | Live-verified Python web/async-framework gotchas, version-pinned per entry: `asyncio.create_task` fire-and-forget GC-safety (real per the docs, didn't reproduce under stress on the version tested), Starlette/FastAPI `BackgroundTasks`' bounded thread-pool concurrency vs. an unbounded raw `threading.Thread`, and FastAPI/pydantic `response_model_exclude_unset` silently dropping defaulted fields on **nested** models. | Writing, reviewing, or planning asyncio fire-and-forget scheduling, background-task dispatch, or a FastAPI response model relying on `exclude_unset`/`exclude_none` — for `coder`, `tdd-engineer`, `architect`, or `analyst` in a Python web/async codebase. | `cobb` (analyst inbox distillation, 2026-08-09) |
