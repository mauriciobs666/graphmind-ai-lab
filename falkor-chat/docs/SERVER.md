...
| `FALKORCHAT_STOREFRONT_LOCALES` | `en,pt-BR,es` | The languages a participant may join in, in the UI's offer order. **Setting it changes two delivered answers today**: the set `POST /shop/api/session` validates `language` against (anything else is `422 validation_failed`) and the list `GET /shop/api/health` reports — the SPA locale chooser that reads that list, and the `i18next` bundles behind it, are delivered (S12c). Not a `Literal`: `JoinIn.language` is a length-bounded string and the route checks membership against this per-deployment list, raising the same `RequestValidationError` Pydantic would. A blank or all-separator value falls back to the default rather than yielding an empty set, which would reject every language a participant could pick |
...
carries the tripwire against reintroducing the second. `scripts/start_demo.sh` (S11, delivered)
pins the one variable to a dedicated value — from-cold-box bring-up, `FALKORCHAT_WS_ID=demo`,
never `config.py`'s `"acme"` dev default.
