...
| `FALKORCHAT_STOREFRONT_QUIESCE_S` | `30` | **How long either reset waits for an in-flight turn to drain before giving up and changing nothing.** S9 wired `Storefront.set_turn_state` into the turn executor, so the turn map is populated for every turn and this wait now has something to act on, measured both ways: a short window (`0.4 s`) against a held turn answers `503 {"error":"quiesce_timeout"}` after ~0.41 s and resets nothing — message count and the original `Thread` unchanged; a window that comfortably covers the turn (`5.0 s` against a 0.8 s turn) instead waits it out (~0.77 s) and then resets, dropping the old `Thread`. While a turn is live, a second post from the same participant answers `409 {"error":"turn_in_progress"}` and is not written, and `GET /shop/api/state` reports `{"state": "thinking", "queuePosition": 0}` rather than the idle payload. **Setting the value changes the outcome in both directions** — raise it and a reset outlasts the turn instead of timing out. The **stop-intake** designed to precede the wait is still S10's |
