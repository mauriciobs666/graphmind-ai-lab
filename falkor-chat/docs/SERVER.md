...
| `FALKORCHAT_STOREFRONT_TURN_WORKERS` | `4` | **`max_workers` of the storefront's own bounded turn executor** (§4.4 measure 1), which is where every agent turn runs: `POST /shop/api/messages` writes the message, calls `Storefront.enqueue_turn` and answers, and `trigger.maybe_trigger` → `services.start_workflow_run` runs on a worker named `storefront-turn_*`. **Setting it changes two observable things** — how many turns run at once, and therefore the `turn.queuePosition` `GET /shop/api/state` reports, a position being how many accepted turns were still unfinished when this one arrived. Sized to LM Studio's configured parallelism, which is the reason for the default: it tracks the model server, not the web tier. Turns never touch anyio's thread limiter, so a deep queue costs the 2 s poll nothing |
| `FALKORCHAT_STOREFRONT_QUIESCE_S` | `30` | **The wait is delivered but has nothing to wait for — S9.** It is how long either reset waits for in-flight turns to drain before giving up and changing nothing (`503 quiesce_timeout`), comfortably under the 180 s agent timeout. But **nothing populates the turn map**: `Storefront.set_turn_state` (`storefront.py:632`) has no caller anywhere in `falkorchat/` — S9 adds one with the turn executor — so `turn_in_flight` is `False` for every participant. Both drains (`Storefront._await_quiesce`, `presenter_reset_all`) therefore pass on their **first** check, neither `503 quiesce_timeout` nor `POST /shop/api/messages`'s `409 turn_in_progress` is reachable, `GET /state`'s `turn` block is always the idle payload, and **setting the value changes nothing observable**. The **stop-intake** designed to precede the wait is S10's |
