...
| U29g | `coder` or `tdd-engineer` (resume U27, fresh if over context threshold) | — | queued | re-run `run_nlq_golden_set_eval.py`, updated report | `data-scientist` (methodology) → — | — |
...
| U31-fix | `tdd-engineer` (resume U29f) | `a1a0c4b3b0c42fecf` | in-flight | fix both MAJORs (2+ column tuple-distinct test gap; duplicate-`returns` uniqueness check) + 2 MINORs | `analyst` (re-gate) → — | — |

**Note:** this delegate's cumulative context across 3 rounds (U29f 197k + U29f-2 238k + this resume) is now well past the ~250k fresh-dispatch threshold — resumed anyway since the fix is small/self-contained and it already holds the file's full context; if this round's own usage comes back very large or the result reads as strained, dispatch U31-fix's own re-gate/any further fix fresh rather than resuming again.
