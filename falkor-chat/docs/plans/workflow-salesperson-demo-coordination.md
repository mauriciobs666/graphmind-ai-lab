...
| U34 | `qa-engineer` (fresh) | `af03abc75900195dd` | in-flight | K-055 live acceptance (AC-1..AC-5) → `docs/test-plans/workflow-nl-query-generation2.md` + `docs/test-reports/workflow-nl-query-generation2-report.md` ("2" ordinal — collides in kind+topic+role with the existing gated golden-set harness report, per collision rule 5) | self → — | — |
| U35 | `qa-engineer` (same dispatch as U34, sequenced after it) | `af03abc75900195dd` | in-flight | M6-closing combined e2e, all four capabilities in one `salesperson@v4` conversation → `docs/test-plans/workflow-salesperson-demo.md` + `docs/test-reports/workflow-salesperson-demo-report.md` (new topic slug, matches the existing coordination doc's own slug) | self → — | — |
