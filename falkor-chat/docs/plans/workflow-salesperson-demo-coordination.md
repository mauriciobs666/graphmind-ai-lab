...
| U33-fix | `tdd-engineer` (fresh) | `a37ecf420c2dce5f4` | in-flight | one-line `_require` allowlist check on `QueryFilter.op` in `compile()` + one `.model_construct()`-bypass regression test, per U33's suggested improvement | `security-expert` (resume U33) → — | — |
