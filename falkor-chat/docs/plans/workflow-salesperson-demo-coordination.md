...
| U29g | `tdd-engineer` (fresh) | `ac2265eb82434a950` | delivered | Re-ran `run_nlq_golden_set_eval.py` against the fixed mechanism; `nlq_eval_results.json` + `docs/test-reports/workflow-nl-query-generation-report.md` rewritten with current numbers. **Overall 33/39=84.6% (CI [70.3,92.8]) vs ≥85% — FAIL by exactly 1 pair; knowledge_base 13/19=68.4% (CI [46.0,84.6]) vs ≥75% — FAIL; not-found 0/6=0% PASS. Every one of the 6 misses is exactly `nlq-34..39` — the two structurally-by-design-unanswerable shapes — 100% (33/33) on every shape the mechanism is designed to handle.** Teco-verified independently: git status matches (2 files); re-tallied `nlq_eval_results.json` myself — 33/39, 13/19, 0/6, failing ids exactly `[nlq-34..39]` — exact match; offline suite independently re-run (**2289 passed, 14 deselected** — matches); restored `reference`/`ws:acme` after, all 3 verify scripts `OK`. | `data-scientist` (methodology) → — | 129k tok / 40 tools |

**Significant, favorable result — but the gate as literally defined still fails.** The RCA-anticipated scenario (ml note §6: a mechanism that structurally can't express a shape should show a named 0% there, not be silently narrowed) has occurred exactly as predicted, at 100% on everything else. This is a genuine methodology/scope question, not just a number to accept or reject.

| U32 | `data-scientist` (resume `ac9e47f85f5801d9a`, the RCA author) | `ac9e47f85f5801d9a` | accepted | `docs/plans/workflow-nl-query-generation-ml.md` (v2, §5 exclusion-rule revision). **Conclusion: the original §5 gate pooled a permanent, by-design 0% into the denominator, making it mathematically unattainable regardless of mechanism quality — an oversight, now corrected.** Revised rule: a shape excluded from the pooled Overall/AC-2 denominators only if *structurally and permanently* incapable by the shipped design's own approved scope (not merely low-scoring) — still authored, run, and reported every time as a named permanent gap. Explicitly distinguished from the numeric-threshold-revisit clause (a different kind of gap) and confirmed it doesn't retroactively bless the original failing run (18/39 → 18/33 in-scope = 54.5%, still fails). **Under the corrected formula: Overall 33/33=100% ≥85% PASS, `knowledge_base` 13/13=100% ≥75% PASS, not-found 0/6=0% PASS — K-055's FR-4/AC-4/AC-2 are satisfied.** `relationship-traversal`/`conflicting-facts` remain a permanent, named, reported structural gap (extending the DSL to support them is flagged as a separate, future scope decision, not resolved by this correction). Teco-verified: diff read in full, arithmetic independently reconfirmed (33/39=84.6%, 13/19=68.4%, 6 excluded pairs = 4+2, both in-scope recomputations and the original-run non-retroactive-bless check all correct). | self → — | 166k tok / 5 tools |
| U30 | `coder` (fresh) | `ad2b9b5d3d6b5b620` | accepted | K-055 cluster 4: `proof_defs.py` (v4 — `query_graph_data` added to `config.tools`, `systemPrompt` extended, `config.model` carried forward), `seed_salesperson.sh`/`verify_salesperson.sh` defaults, `AGENTS.md` narrative, `test_salesperson_scaffold.py` (+1 schema stub, version/tool-count assertions bumped). Teco-verified: full diff read (all 5 files), independently re-ran all 3 verify scripts (`verify_workflows.sh acme`/`verify_catalog.sh`/`verify_salesperson.sh acme` — all `OK`, `salesperson@v4` in sync, topology unchanged 2-step/1-transition), independently re-ran `test_salesperson_scaffold.py` (6 passed). No fresh `git diff` surprises — matches delegate's report exactly. | `qa-engineer` (U34/U35) → — | 137k tok / 52 tools |
...
| U33 | `security-expert` (fresh — dispatched via `Agent`, not `SendMessage`, so it's a new instance despite naming the prior reviewer's context in its brief) | `a6ebccba4219bf9db` | accepted | `docs/reviews/workflow-nl-query-generation-security.md` `## Pass 3` — live Groups A-E adversarial run against the real, shipped `query_graph_data`. **Verdict: approve with one new finding (MAJOR, not blocker).** Finding: `QueryFilter.op` is the one DSL field with zero independent `compile()`-level recheck (every other splice-worthy field has one) — a `.model_construct()`-bypass can splice a write-clause chain into the generated Cypher; live-reproduced, but Layer 2 (`GRAPH.RO_QUERY` engine refusal) caught it every time, zero mutation (probe-graph node count unchanged, `verify_catalog.sh` 15/15 OK after). Not reachable via any current production caller (the model-driven path always goes through full Pydantic validation). Pre-existing gap, not a regression from this session's accuracy-fix diff (confirmed by direct code reading — the `where_clauses.append` line predates the RCA fixes). Reviewer recommends closing it as a fast follow-up before treating K-055 as fully closed (mirrors U31's own MAJOR-2 disposition). Teco-verified: read the full Pass 3 section: methodology (disposable `ws:sec-u33-probe` graph, node-counted before/after, deleted at end, `reference`/`ws:nlq-eval`/`ws:acme` never touched), the live B2b reproduction, and the C1-C3/D1-D3 audit are internally consistent and evidenced; no `git status` check needed (review doc is a revise-in-place `reviews/` edit, no code touched by this unit). | self → — | 147k tok / 30 tools |
| U33-fix | `tdd-engineer` (fresh) | `a37ecf420c2dce5f4` | delivered | one-line `_require(filt.op in {...})` allowlist check in `compile()` + `test_compile_rejects_invalid_op_from_hand_constructed_filter` (mirrors the review's exact live payload), `docs/HISTORY.md` entry. Teco-verified: diff read in full (4 lines in `querygen.py`, matches sibling-check pattern exactly); independently reproduced RED (via `git stash` of just the fix, re-ran the new test — `DID NOT RAISE ValueError`) then GREEN after restoring; full offline suite independently re-run (**2290 passed, 14 deselected** — matches, +1 over pre-fix 2289); restored `reference`/`ws:acme` after, all 3 verify scripts `OK`. | `security-expert` (resume U33) → — | 95k tok / 24 tools |
| U33-regate | `security-expert` (resume `a6ebccba4219bf9db`) | `a6ebccba4219bf9db` | accepted | `docs/reviews/workflow-nl-query-generation-security.md` `## Pass 4` — independently re-ran the exact B2b live payload against current code, confirms it now raises before any engine call. **Overall mechanism verdict: approve, no open finding** — across 4 passes (2 design-time, 2 live), every splice-worthy DSL field now has both Pydantic-level + independent `compile()`-level checks. Teco-verified: diff read in full (49 lines, one new section only), evidence internally consistent with U33-fix's own independent verification. **K-055's FR-3/FR-3a/AC-3 security gate is now fully closed.** | self → — | 154k tok / 6 tools |
| U34 | `qa-engineer` (fresh) | `af03abc75900195dd` | accepted | K-055 live acceptance → `docs/test-plans/workflow-nl-query-generation2.md` + `docs/test-reports/workflow-nl-query-generation2-report.md`. **Verdict: PASS WITH DEFECTS.** AC-1/AC-2/AC-3/AC-5 hold live with ground-truth Cypher; AC-4 met under the corrected exclusion-rule gate (100% in-scope). One MAJOR defect (DEF-01): intermittent (1/2 attempts) self-contradictory/incomplete answer on a compound category+price filter question — evidence points to `ministral-3-3b` orchestration, not the `querygen` DSL (golden-set harness independently measures 100% on this exact shape). Routed as a non-gating follow-up, not fixed in this unit. Teco-verified: `git status` clean (only the 4 new doc files), read both reports in full, independently re-ran all 3 verify scripts (OK). | self → — | 251k tok / 96 tools |
| U35 | `qa-engineer` (same dispatch as U34, sequenced after it) | `af03abc75900195dd` | accepted | M6-closing combined e2e → `docs/test-plans/workflow-salesperson-demo.md` + `docs/test-reports/workflow-salesperson-demo-report.md`. **Verdict: PASS, no defects.** All four capabilities (K-052/053/054/055) fired correctly on the right tools in one continuous conversation on the real 11-tool `salesperson@v4`; profile + post-order cart state both proven correct together in an independent second thread. **Closes M6's own combined-demo closing condition.** Teco-verified: read report in full, cross-checked claims against ground-truth Cypher statements cited, no inconsistencies found. | self → — | (shared cost above) |
