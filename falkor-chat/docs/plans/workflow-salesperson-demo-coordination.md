...
| U29e | `graph-dba` | `a30a62b591c18c6ba` | in-flight | confirm `DISTINCT`+`ORDER BY`-on-non-returned-column dialect behavior on this build, before the C fix ships (RCA §4.1.3) | — | — |
| U29f | `coder` or `tdd-engineer` | — | queued | implement RCA Priority 1 (DSL fixes A/B/C) + Priority 2 (prompt hardening D) in `querygen.py`/`tools.py`, gated on U29e's dialect confirmation for C | `analyst` → — | — |
| U29g | — | — | queued | re-run `run_nlq_golden_set_eval.py`, updated report | `data-scientist` (methodology) → — | — |
