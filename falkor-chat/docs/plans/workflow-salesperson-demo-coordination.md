...
| U33-fix | `tdd-engineer` (fresh) | — | queued | one-line `_require` allowlist check on `QueryFilter.op` in `compile()` + one `.model_construct()`-bypass regression test, per U33's suggested improvement | `security-expert` (resume U33) → — | — |
