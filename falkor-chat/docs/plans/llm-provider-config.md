...
> **Status:** active · **Owner:** `architect` · **Tracks:** K-042 (M4) · **Version:** 3 · **Reviews:** `docs/reviews/llm-provider-config.md`
...
- §7 L2-1 — `FallbackClient` sets `ChatResult.fallback = (index of the successful element > 0)` at
  ...
  `None`/`False` on a length-1 chain.
