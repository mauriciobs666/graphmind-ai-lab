...
**Revision note (v2, 2026-08-15):** narrow addendum only — adds an accepted, sign-off-gated
exception to this note's "never the 4B-under-test judging itself" guidance (Layer 2) for K-026's
Unit 3 baseline specifically, per a stakeholder-directed, hardware-constrained decision relayed by
`teco` and documented in `docs/plans/graphrag-eval.md` v4 §3 D1. The general guidance below is
**unchanged** for future work — this is a named exception for one baseline, not a retraction. See
the new paragraph at the end of "Layer 2" below, and the full methodology review at
`docs/reviews/graphrag-eval-ml.md`.
...
**Addendum (2026-08-15) — accepted same-model exception for K-026's Unit 3 baseline only.** Per a
stakeholder-directed, hardware-constrained decision (the local hardware cannot run a second, larger
judge model alongside the 4B agent-under-test for this workload, even though larger models are
registered/reachable on the same LM Studio instance), K-026's Unit 3 collapses judge and
agent-under-test onto the same model instance (`qwen/qwen3-4b-2507`) for this specific baseline.
This is a named, sign-off-gated exception — **not** a retraction of the guidance above, which still
applies as the default whenever a distinct, sufficiently strong judge model is genuinely runnable.
Two consequences to hold onto (detailed in `docs/reviews/graphrag-eval-ml.md`):
- The **judge-calibration sub-pass** (judge vs. human agreement on fixed, non-self-generated
  question/context/answer triples) is largely unaffected by self-preference bias — the judged
  content wasn't generated by the model under test, so this remains a legitimate, if small-N
  (~10 examples), signal of the judge's general rubric-following reliability.
- The **generation sub-pass** (faithfulness/relevance over the model's own live-generated answers)
  is where self-preference bias is squarely in play — a high score there is a same-model directional
  signal, not independent validation, and must never be read as if a distinct judge produced it.
  **Calibration passing does not license trusting the generation numbers — they are two different
  validity claims.** Gross/obvious failures (flat contradiction of retrieved context, answering the
  wrong question) likely remain catchable even under this configuration; the bias risk concentrates
  in borderline/subjective calls, where it becomes indistinguishable from genuine quality.
