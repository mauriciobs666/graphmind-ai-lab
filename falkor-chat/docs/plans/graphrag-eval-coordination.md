...
| U-qa | `qa-engineer` | — | **queued — unblocked** (all three remaining gates accepted, no outstanding "needs changes"; out of scope for this dispatch per this session's brief, not started) | `docs/test-reports/graphrag-eval-report.md` | — |
| U-docs | (folded into implementer/qa units) | — | queued (out of scope for this dispatch, not started; candidate `BACKLOG.md` follow-up items now on record: root `server/tests/conftest.py`'s identical write-mode-query pattern flagged by U2b-fix, `generate_report.py` test-coverage gap flagged by U3-gate's M-1) | `HISTORY.md`, `BACKLOG.md` K-026 flip | — |
