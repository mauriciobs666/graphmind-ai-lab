...
| U-bug | **unassigned — next session** | — | **queued (blocking)** | Fix the `conftest` module-name collision: `server/tests/eval/conftest.py` (Unit 2b) and `server/tests/conftest.py` collide under Python's bare-name import caching once both exist — confirmed by teco, `cd server && .venv/bin/python -m pytest -q` (repo-wide default, no path) fails collection (`ImportError: cannot import name 'TEST_EMBEDDING_DIM' from 'conftest'`) on `tests/test_graphrag.py`/`tests/test_tools.py`. `server/tests/eval` alone is green (157 passed per Unit 3's report); this only manifests repo-wide. Likely fix: package-ize `tests/eval/` (`__init__.py`) or switch the colliding bare imports to package-qualified ones — **not diagnosed further, this is next session's first task**, routes to `tdd-engineer` (safety-net-shaped fix) with an RCA from `analyst` first if the fix isn't obvious on inspection. **This blocks calling the plan's own "keep `pytest -q` green" done-condition met** — must close before U-qa. | — |
| U-qa | `qa-engineer` | — | queued (blocked on U-bug) | `docs/test-reports/graphrag-eval-report.md` | — |
| U-docs | (folded into implementer/qa units) | — | queued | `HISTORY.md`, `BACKLOG.md` K-026 flip | — |
