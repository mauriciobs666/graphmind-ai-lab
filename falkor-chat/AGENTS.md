...
| `./scripts/seed_salesperson.sh [<wsId>]` | Publishes + materializes the shared `salesperson@v1` demo-agent `WorkflowDef` (K-052 M6 — the scaffold `docs/plans/workflow-catalog-lookup.md` §2.3-§2.5 also names as canonical for K-053/K-054/K-055) into `reference` + `ws:<id>`, imported from `falkorchat.proof_defs.SALESPERSON_DEF`. Mirrors `seed_workflows.sh`'s shape/idempotence-probe pattern. **Edited in place** by each sibling capability to bump the def's version (`v1`→`v2`→`v3`→`v4`) — same evolving-artifact convention `seed_workflows.sh` itself follows. It is a `conversation`-kind, chat-triggered def like `triage` — **not** startable via a bare `POST /workflow-runs` (no trigger message ⇒ no thread for `post_message` to post into); exercise it live by pointing `FALKORCHAT_TRIGGER_DEF_KEY`/`_VERSION` at it for a run and `@mention`ing the demo agent. Run after `bootstrap_schema.sh` + `seed_demo.sh` + `seed_catalog.sh`. |
| `./scripts/verify_salesperson.sh [<wsId>]` | Read-only check that `reference` and `ws:<id>` agree on `salesperson@v1` — present, in sync, exactly one start key, and the expected 2-step/1-transition topology. Exit `0`/`1`, same contract as `verify_workflows.sh`, scoped to this one def. |
