...
| `./scripts/seed_workflows.sh [<wsId>]` | Publishes + materializes the two proof workflow defs (`triage@v1`, `access-request@v1`) into `reference` + `ws:<id>`. Run after `bootstrap_schema.sh` + `seed_demo.sh`. **⚠️ Create-only, not update**: re-running after editing a def is a silent no-op, and `reference`/`ws:<id>` can drift out of sync independently (split-brain — the workspace snapshot is what actually executes). Landing a def edit needs an explicit delete+republish or a key/version bump. Use `verify_workflows.sh` (or the `/diff` route) to detect drift rather than reasoning about it. |
