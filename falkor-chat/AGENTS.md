...
- Labels: `PascalCase` — `User`, `Channel`, `Thread`, `Message`, `Agent`, `ReadCursor`,
  `Customer`, `Cart`, `CartItem`, `Order`, `OrderLine` (K-053 M6 — the `salesperson` demo's
  durable cart/order state, `ws:{workspaceId}`-scoped, `docs/QUERIES.md` §16 /
  `docs/plans/workflow-cart-and-totals-graph.md`)
...
| `./scripts/bootstrap_schema.sh <wsId> …` | Creates all indexes + constraints for `reference` + workspace(s). Idempotent. **Always touches `reference` too, even when given only a probe workspace ID** — but DDL only, no `MERGE`/`CREATE (n)`/`DELETE` (`scripts/bootstrap_schema.sh:37-70`), so bootstrapping a throwaway probe workspace is safe for `reference`'s *data*. For a real, live-served workspace use `create_workspace.sh` instead (below), which also pins the embedding-model override, FR-2 — this script alone leaves that unset. |
| `./scripts/create_workspace.sh <wsId> …` | Canonical entry point for a **real** workspace (§3.2 Option B, `docs/plans/embedding-migration.md`): `bootstrap_schema.sh "$@"` then FR-2's pin operation (`pin_workspace_embedding_model.sh`) per given id, so every real workspace gets an explicit `embeddingModelOverride` at birth with no separate operator step. Not for a throwaway/offline-suite workspace (`ws:test`, `ws:nlq-eval`) — those call `bootstrap_schema.sh` directly, deliberately. |
| `./scripts/test_queries.sh` | End-to-end test suite against the live instance; must pass before any schema change is committed. **⚠️ Deletes the `reference` graph at teardown** (`GRAPH.DELETE` — indexes and constraints go too, not just node data), wiping both published workflow defs, so `@mention`-to-start silently breaks (workspace snapshots survive). Re-run `bootstrap_schema.sh <wsId>` → `seed_demo.sh` → `seed_workflows.sh` afterward — `seed_workflows.sh` alone is not enough — or check first with `verify_workflows.sh <wsId>`. |
...
| `./scripts/seed_agent_team.sh [<wsId>]` | Registers every `claude/` subagent (`claude/AGENTS.md`'s roster — a point-in-time copy baked into the script, not read live) as an **Agent** node in `ws:agent-team`, so `ingest_document`'s `produced_by` (K-030, `claude/docs/plans/agent-knowledge-base-strategy.md` §4.1/§4.2) has a real node to resolve each agent against. Idempotent; run after `EMBEDDING_DIM=1024 ./scripts/bootstrap_schema.sh agent-team`. **Whoever next updates `claude/AGENTS.md`'s roster-touch rule should add this script to it** — not wired automatically. |
| `./scripts/seed_workflows.sh [<wsId>]` | Publishes + materializes the two proof defs (`triage@v1`, `access-request@v1`) into `reference` + `ws:<id>`. Run after `bootstrap_schema.sh` + `seed_demo.sh`. **⚠️ Published defs are topology-immutable (K-034)**: re-running after editing a def's steps/transitions/start fails loudly (`409`) on the *materialize* half — publish a new version instead of editing this one. Property-only edits (name, step config, guard text) silently no-op. `reference` and `ws:<id>` can drift independently (split-brain; the workspace snapshot is what executes) — detect it with `verify_workflows.sh` or the `/diff` route rather than reasoning about it. |
...
| `./scripts/seed_catalog.sh [<wsId>]` | Seeds the fixed ~15-product electronics catalog into the GLOBAL `reference` graph only — `<wsId>` is accepted for CLI parity with the other seed scripts but unused (the catalog is workspace-independent, FR-6). Idempotent: `productId` is a deterministic slug of the product name, not `uuid4()`, so a re-seed after `reference` is wiped reconstructs byte-identical data. Run after `bootstrap_schema.sh` (needs the `Product` index/constraint pair it creates). |
| `./scripts/verify_catalog.sh` | Read-only (`GRAPH.RO_QUERY` only) check that the product catalog is present as expected — total count plus two named products' category/price. Exit `0`/`1` with a printed diagnosis, same contract as `verify_workflows.sh`. |
| `./scripts/seed_salesperson.sh [<wsId>]` | Publishes + materializes two defs into `reference` + `ws:<id>`, imported from `falkorchat.proof_defs`: `salesperson@v8` (`conversation`-kind demo agent) and `order-fulfillment@v1` (`process`-kind). Run after `bootstrap_schema.sh` + `seed_demo.sh` + `seed_catalog.sh`. Three traps below. |
| `./scripts/verify_salesperson.sh [<wsId>]` | Read-only check that `reference` and `ws:<id>` agree on **both** `salesperson@v8` and `order-fulfillment@v1` — present, in sync, exactly one start key, and each def's own expected topology (salesperson: 2-step/1-transition; order-fulfillment: 4-step/3-transition) — **plus every stored step `config`, on both sides, against the shipped `falkorchat.proof_defs` constant**. That last check is the only one that can see a version whose `config` was stored from an earlier or uncommitted tree: the def↔snapshot diff compares the two sides against *each other*, so both agree, the topology is right, and the version name silently denotes something `proof_defs.py` can no longer produce (exactly the burned-`v6` shape in `ws:acme`). `config` is create-only, so re-seeding cannot repair it — the failure names the drifted fields and says to bump the version. Exit `0`/`1`, same contract as `verify_workflows.sh`, scoped to these two defs. |
| `./scripts/start_demo.sh` | From-cold-box bring-up of the `salesperson` storefront demo (`docs/plans/salesperson-ui.md` S11): FalkorDB → schema → seed demo/catalog/salesperson → catalog/salesperson preflight → `salesperson/build.sh` → uvicorn as the **storefront** deployment (trigger pinned to `salesperson@v8`, responder fall-through off, `--reload` off). **Pins `FALKORCHAT_WS_ID=demo`**, never `config.py`'s `"acme"` default. Fails loudly, before uvicorn starts, when FalkorDB, Node, the built bundle, a def, the catalog, or the demo Agent is missing — env overrides in the script's own header comment. Skips `seed_workflows.sh`. |
...
**Three traps when bumping `SALESPERSON_DEF`.** (1) `config.tools`, `systemPrompt` and
