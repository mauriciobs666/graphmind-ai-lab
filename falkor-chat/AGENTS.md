...
| `./scripts/seed_catalog.sh [<wsId>]` | Seeds the fixed ~15-product electronics catalog (K-052 M6) into the GLOBAL `reference` graph only — `<wsId>` is accepted for CLI convention parity with the other seed scripts but unused (the catalog is workspace-independent, FR-6). Idempotent: `productId` is a deterministic slug of the product name, not `uuid4()`, so a re-seed after `reference` is wiped (`test_queries.sh`'s teardown, or a default `pytest` run's `wf_repo` fixture) reconstructs byte-identical data. Run after `bootstrap_schema.sh` (needs the `Product` index/constraint pair it creates). |
...
| `./scripts/seed_salesperson.sh [<wsId>]` | Publishes + materializes **two** defs, mirroring `seed_workflows.sh`'s two-def pattern: the shared `salesperson@v4` demo-agent `WorkflowDef` (K-052 M6 bumped to v2 by K-053 M6, to v2.1 by K-056's Ministral re-point, to v3 by K-054 M6's durable-profile tools, then to v4 by K-055 M6's NL query generation — the scaffold `docs/plans/workflow-catalog-lookup.md` §2.3-§2.5 names as canonical for all four) and the new `order-fulfillment@v1` process def (K-053 M6, FR-6/FR-9's LLM-free order-lifecycle flow) — both into `reference` + `ws:<id>`, imported from `falkorchat.proof_defs.SALESPERSON_DEF`/`ORDER_FULFILLMENT_DEF`. **`SALESPERSON_DEF` is edited in place** by each sibling capability to bump its version (`v1`→`v2`→`v3`→`v4`) and add tools/prompt guidance — same evolving-artifact convention `seed_workflows.sh` itself follows; **topology stays byte-identical across versions** (only `config.tools`/`systemPrompt` change), so a version bump never hits the K-034 409 topology-conflict path. `v2.1` (K-056) was a different-shaped bump on the same rule: tools/prompt unchanged from v2, only `config.model` added to re-point the `assistant` step at `mistralai/ministral-3-3b` (`qwen/qwen3-4b-2507`'s confirmed near-certain tool-call-skipping defect on long conversations, `docs/BACKLOG.md` K-056) — `config.model` is create-only exactly like `config.tools`/`systemPrompt` (`proof_defs.py`'s module docstring), so this needed its own version too. `v3` (K-054) is back on the capability sequence: adds `get_profile`/`save_profile` to `config.tools` and extends `systemPrompt` (`docs/plans/workflow-durable-profile.md` §3.2) — and, not documented in either design doc (both predate `v2.1`), **carries `config.model` forward unchanged**, since it too is create-only and a version that omitted it would silently fall back to the shared role default and undo K-056's fix. `v4` (K-055) adds `query_graph_data` to `config.tools` and extends `systemPrompt` to reach for it on a question that doesn't match a fixed catalog lookup (`docs/plans/workflow-nl-query-generation.md` §4 step 7) — and, same discipline as `v3`, carries `config.model` forward unchanged. `triage`/`access-request` are unaffected — they still resolve through the shared `step`/`agent`-role defaults, still `qwen/qwen3-4b-2507`. `salesperson` is a `conversation`-kind, chat-triggered def like `triage` — **not** startable via a bare `POST /workflow-runs` (no trigger message ⇒ no thread for `post_message` to post into); exercise it live by pointing `FALKORCHAT_TRIGGER_DEF_KEY`/`_VERSION` at it for a run and `@mention`ing the demo agent. `order-fulfillment` is a `process`-kind def, started over REST like `access-request` — its own steps have no side effect on the `Order` node they manage (advancing `Order.status` is a separate `services.advance_order` call the caller makes alongside `submit_workflow_input`, not wired to a REST route in this milestone). Run after `bootstrap_schema.sh` + `seed_demo.sh` + `seed_catalog.sh`. |
| `./scripts/verify_salesperson.sh [<wsId>]` | Read-only check that `reference` and `ws:<id>` agree on **both** `salesperson@v4` and `order-fulfillment@v1` — present, in sync, exactly one start key, and each def's own expected topology (salesperson: 2-step/1-transition; order-fulfillment: 4-step/3-transition). Exit `0`/`1`, same contract as `verify_workflows.sh`, scoped to these two defs. |
