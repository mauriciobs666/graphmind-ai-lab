#!/usr/bin/env bash
# verify_salesperson.sh — check that `reference` and ws:<id> agree on the
# `salesperson@<version>` def (K-052 M6). The one-command form of
# seed_salesperson.sh's own re-seed discipline, scoped to just this one def
# (mirrors verify_workflows.sh, which covers `triage`+`access-request`).
#
# Usage:
...
# nothing. When the def is missing it prints the `seed_salesperson.sh` command
# and exits non-zero; running that command is the operator's decision, not this
# script's. A DIVERGENCE is not repaired by re-seeding either (a create-only
# re-publish cannot overwrite anything, and deleting a snapshot breaks live
# WorkflowRuns via OF_DEF/AT_STEP) — report it instead.
#
...
#
# Checks, for `salesperson@<version>` (default version `v1`, override with
# FALKORCHAT_SALESPERSON_DEF_VERSION — same knob seed_salesperson.sh reads):
...
#   5. the topology is exactly what the plan specifies: 2 steps (`assistant`
#      type `agent`, `ended` type `decision`), 1 transition.
#
...
#   FALKORCHAT_SALESPERSON_DEF_VERSION (default: v1)
...
echo "── verifying workflow def '${SALESPERSON_DEF_KEY}@${SALESPERSON_DEF_VERSION}': reference vs ws:${WS_ID} ──"
...
for side, reader in sides:
    ...
    if side.startswith("ws:"):
        snap_structure = structure
...
# Topology sanity — the plan's exact shape (§3.3): 2 steps, 1 transition.
if snap_present:
    snap = services.get_snapshot(ctx, key=KEY, version=VERSION)
    step_types = {s["key"]: s["type"] for s in snap["steps"]}
    expected_steps = {"assistant": "agent", "ended": "decision"}
    ...
    if snap["start_key"] != "assistant":
        print(f"    ⚠ start key: {snap['start_key']!r} (expected 'assistant')")
        failures.append(f"{label}: unexpected start key {snap['start_key']!r}")
    if len(snap["transitions"]) != 1:
        print(f"    ⚠ transition count: {len(snap['transitions'])} (expected 1)")
        failures.append(
            f"{label}: unexpected transition count {len(snap['transitions'])}"
        )
    else:
        print(f"    {'topology':<{width}} : 2 steps (assistant/agent, ended/decision), "
              f"1 transition — OK")
...
if failures:
    ...
    for f in failures:
        print(f"  ✗ {f}")
    print("")
    print("If the def or snapshot is MISSING, re-seed:")
...
print(f"RESULT: OK — {label} in sync between `reference` and ws:{ctx.ws}")
PY
