...
| FastMCP emits `outputSchema` + duplicates payload as `structuredContent` for a plain `-> str` tool; `structured_output=False` suppresses it | Installed `mcp` 1.28.1 in `cypher-mcp/.venv`; wrote and ran a throwaway `FastMCP` tool, called `list_tools()`/`call_tool()` | Confirmed exactly — `outputSchema` present, `call_tool` returns `(TextContent, {'result': 'hello'})` duplicating the payload; `structured_output=False` → `outputSchema: None`. Version (1.28.1) and the `structured_output: bool \| None = None` parameter both matched the doc's citation verbatim |
...
| Stale "joern agent" string gone from `cypher-mcp/server.py` | `grep -rn "joern agent" cypher-mcp/` | No hits; live string reads "the graph-dba agent's job" |
