...
`claude/README.md`, `cpg/mcp/README.md`, `cpg/mcp/server.py`, `skills/joern-cpg/SKILL.md`,
...
graph through `mcp__cpg__query` (see evidence below) rather than trusting the docs alone.
...
```
mcp__cpg__query(graph="cpg_this_graph_should_not_exist_probe", cypher="MATCH (n) RETURN count(n)")
...
`cpg/mcp/server.py:327` hard-codes "the joern agent's job" — a name that no longer exists in the
...
a fix for `cpg/mcp/server.py`'s error string (and the design doc) to whoever owns that component
...
`cpg/mcp/README.md` both document that `redis-cli GRAPH.QUERY`/`GRAPH.RO_QUERY` is the documented
...
an agent, or see `cpg/mcp/README.md` for the exact command."
...
- **The MCP guarantees are correct and live-verified**, not just doc-sourced: read-only enforcement
  ...
  display-only with the true row count always reported — all confirmed both in `cpg/mcp/README.md`
  and by directly exercising `mcp__cpg__query` against the live `cpg_falkorchat` graph during this
...
- **Destructive-reload framing is accurate and appropriately non-technical**: "refuses to run...
  ...
  containerization details in `cpg/mcp/README.md` (no mention of `docker-run.sh`,
...
- The stale "joern agent" string in `cpg/mcp/server.py`/`docs/plans/cpg-query-access.md` (M1) is
