...
2026-07-30: FalkorDB (`falkordb-dev`, port 6379) up and reachable, `cypher` MCP tool connected
...
`skills/joern-cpg/SKILL.md` and `cypher-mcp/README.md`.
...
source (`cypher-mcp/README.md`) accounts for. See TP-008 / DEF-001.
...
| TP-002 | **Pass** | `claude mcp list` → `cypher: ... - ✔ Connected` |
| TP-003 | **Pass** | `GRAPH.LIST` → `cpg_falkorchat`, `cpg_salesperson`, `ws:acme`, `reference`, `ws:test` |
| TP-004 | **Pass** | `mcp__cypher__query(cpg_falkorchat, "MATCH (m:METHOD) RETURN count(m) AS n")` → `graph=cpg_falkorchat · rows=1 · 73.9ms`, `n=1968` |
| TP-005 | **Pass** | Callers-of-`post_message` recipe → 21 rows incl. `falkorchat/api.py:...post_message` (line 144) and `falkorchat/mcp.py:...send_message` (line 53) — matches the exact example in `cypher-mcp/README.md`'s sample output (same two callers, close line numbers — expected drift since the README's is a snapshot from an earlier build) |
...
is always the true one."* The same claim, worded almost identically, is in `cypher-mcp/README.md`:
...
1. `redis-cli -p 6379 GRAPH.CONFIG GET RESULTSET_SIZE` → `10000` (FalkorDB's own server-level
   default cap on any query's result set, independent of the `cypher` MCP tool).
2. `mcp__cypher__query(graph="cpg_falkorchat", cypher="MATCH (n) RETURN count(n) AS total")` →
   `total = 110048` (the true node count).
3. `mcp__cypher__query(graph="cpg_falkorchat", cypher="MATCH (n) RETURN n")` → notice line says
   `showing 132 of 10000 rows`, and the stats line says `rows=10000`.
4. Confirmed the FalkorDB-level cap, not the tool's own `CYPHER_MCP_MAX_ROWS`, is what binds: an
...
a layer *beneath* the `cypher` MCP tool and not mentioned in either document), the reported `rows=`
...
`cypher-mcp/README.md` carries the identical overclaim and is the manual's own cited source, this is
...
- The `redis-cli` fallback path end-to-end for the query walkthrough (used only for the
  ...
  querying walkthrough is written around `mcp__cypher__query`, which is what was exercised as the
  primary path.
...
1. **Fix DEF-001's wording** in the manual (see recommendation above) — this is the main
   actionable finding.
2. **Consider flagging the same overclaim in `cypher-mcp/README.md`** to whoever owns that document;
