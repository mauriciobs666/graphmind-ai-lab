...
**Shape A — 9 turns, read-only (A-03: 10 turns as of 2026-09-16 — the agent pre-check's Finding 1
split its former turn 4 into two single-call turns; see A-03's own row and §3.3's citations
below).**
...
| A-03 (10 turns as of 2026-09-16 — see note) | (1)-(2) two boundary-adjacent price filters (one just under `maxPrice`'s confusable value, one just over `minPrice`'s); (3) exact-name lookup with an explicit `finalReplyMustContain` price assertion; (4)-(5) a "which is cheaper, X or Y" product-comparison question answered across two consecutive single-call turns, each a distinguishable `lookup_product_fact` call with its own `finalReplyMustContain` price — split from the original single compound turn 4 by the agent pre-check's Finding 1 (`model-bench/docs/reviews/small-model-benchmarking-s6-precheck.md`): the compound form was a guaranteed argument-correctness failure under the real scorer, not the spurious/duplicate risk the outline originally named, and neither new turn is a compound two-call turn any longer; (6) abstention (a product not in the catalog); (7)-(8) two ordinary category filters across different categories; (9) exact-name lookup; (10) exact-name lookup with `finalReplyMustNotContain` guarding against a fabricated price on a near-miss product name | both `boundaryRule` arguments stressed together; a two-turn product-comparison sequence (no longer a spurious/duplicate opportunity post-split — see §3.3's corrected (e) row); explicit fabrication guard via `finalReplyMustNotContain` |
...
| (e) spurious/duplicate | cross-turn duplicate opportunities: A-01 (t7-9, t3-4), A-04 (t8-9), C-03 (t3-4); the ministral-pattern re-probe: B-02 (t1, t4) — corrected 2026-09-16 from an earlier `(t3-4)` citation naming turn 3 (`view_cart`, uninvolved in the duplicate-signature check) instead of turn 1 (the re-probe's own baseline call), per `model-bench/docs/reviews/small-model-benchmarking-s6-precheck.md` Finding 3; the compare-two-products spurious risk citation (A-03, formerly t4) is removed 2026-09-16 — Finding 1 of the same review found this turn could only ever register as an argument-correctness failure, never as spurious/duplicate, and the fix split it into A-03's now-single-call turns 4-5 (§3.2), neither of which is a compound two-call turn |
