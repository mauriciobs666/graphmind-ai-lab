...
| U7-fix | `cobb` | *(dispatching)* | queued | Apply U8's 2 minors + 1 nit: add "tool call/" to `frontend-engineer.md`'s freshness sentence; tighten the pronoun-ambiguity clause across all six files per U8's suggested rewrite; swap "freshness check" → "freshness marker" in the query clause. Same-delegate resume via SendMessage (content fix on already-delivered U7, not a platform failure). | — |
