...
| ID | Disposition | What changed |
|---|---|---|
| **M-4** — hash list & `--verify-inputs` under-specified for the one directory `COPY` | **Accepted in full, settled before S2** | §4.3 is now **directory-driven, not glob-driven**: a new `cpg_mcp_input_dirs` names the walked directories (`tests`), `cpg_mcp_input_files` walks each with **two of `.dockerignore`'s three exclusions** (`**/__pycache__` and `**/*.pyc`; **not** `**/.pytest_cache` — corrected here after Part III's m-21, which is filed on C-321), and `--verify-inputs` has **one rule per operand kind** (file → exact membership; directory → "the enumeration walks it"). §4.1's Dockerfile header and §4.2's `.dockerignore` both state the coupling. **Demonstrated, not asserted:** a `.json` fixture dropped under `tests/` moves the hash (`ba910c48571d` → `f33f9f5273c6`) and moves it back on removal — v2's `*.py` glob would not have noticed it; `__pycache__` does *not* affect the hash. `--verify-inputs` was also shown to **fail** on both operand kinds (an uncovered `COPY <file>` and an uncovered `COPY <dir>`), which is S3's done-condition. **Post-delivery (Part III, M-7):** the check itself had a hole — its Dockerfile parse was line-oriented, so a `\`-continued `COPY` was invisible to it and it answered "OK". Fixed by joining continuations before parsing, with a checked-in regression at `cpg/mcp/tests/test_build_inputs.py`; the same pass stopped `COPY --from=<stage>` being misreported as a missing build-context file (m-27). |
