...
> **Status:** active · **Owner:** `architect` · **Tracks:** — (M<n> TBD) · **Version:** 1.27 · **Reviews:** `docs/reviews/salesperson-ui.md`, `docs/reviews/salesperson-ui-impl.md`
...
waiting line** — equivalently, how many *other* accepted turns must start before theirs. It is
**`0` whenever `state` is `idle` or `thinking`**, where it is a constant rather than a position: an
idle participant is in no line, and a running turn has already left it. **`0` on a `queued` turn is
ordinary and load-bearing** — *first in line, waiting for a worker to free up* — so S13 renders it
as a queue, never as the absence of one.
...
under-count, never an over-count, and it self-corrects; §5.1's per-booking token is what keeps this
window merely optimistic rather than corrupting, since those workers then clear nothing that is not
theirs.
  ...
  *domain* — the cross-cutting `504` row, which said "every route that writes" — without extending
  ...
  - after **`POST /shop/api/messages`** → **`GET /shop/api/messages` *and* `GET /shop/api/state`, and
    ...
    *ahead* of the write and **releases it on exactly this failure**, which is what leaves
    `turn.state` `idle` here rather than a `queued` turn nobody will ever run, i.e. what makes the
    reconciliation below decidable at all. Re-reading only `/messages` shows the message
