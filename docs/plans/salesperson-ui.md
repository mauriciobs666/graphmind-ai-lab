...
> **Status:** active · **Owner:** `architect` · **Tracks:** — (M<n> TBD) · **Version:** 1.30 · **Reviews:** `docs/reviews/salesperson-ui.md`, `docs/reviews/salesperson-ui-impl.md`
...
(`docs/reviews/salesperson-ui-impl.md` `## Pass 20`, P20-1). In normal operation neither costs
anything, and that is what keeps the asymmetry cheap: the queued item runs and clears its own
booking, while the refusals that genuinely queue nothing all mean the pool is stopped — so no
leaked booking survives to sit in the line inflating the positions behind it.
