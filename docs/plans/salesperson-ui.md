...
*2026-09-03 — v1.21: three carry-forwards from the **delivered** S8b (`18b675a`), none of them a re-design — §5.3 gains the one `(route, response)` S8b's path-scoped `ServiceError` re-shaper adds (`POST /shop/api/messages` · `503 demo_not_seeded` · C9, `UnknownMemberError` raised **before any write**), and the generation rule names that handler's *measured* route set as its third producer; §4.9's "the route table contains **only**" is corrected for FastAPI's own four documentation routes, which the storefront deployment does carry and the delivered assertion exempts by construction (`docs/reviews/salesperson-ui-impl.md` `## Pass 10`, P10-12), with R6's "no browsable surface" clause narrowed to match; and §5.1's **S10** row records that S8 already delivered the three presenter routes (same pass, ruling 1), agreeing with `_STEP_10_INTERIM`'s **two of the four** private reads (P10-6). S10 is the only step row that moves.*
*2026-09-03 — v1.22: `docs/reviews/salesperson-ui-impl.md` `## Pass 11` (approve with suggestions, 0 blockers) — §5.2's `POST /shop/api/messages` row gains the `503 demo_not_seeded` v1.21 gave §5.3 and gave §5.2's *join* row only (P11-5, the generation rule's own first violation), and **the `401` omission is licensed once in §5.2's preamble rather than listed per route** (chosen over ten repeated cells across eight rows: the credential refusals come from the `get_participant`/`get_presenter` dependencies rather than from any route body, and §5.3's per-route table already rules every one of them, so the prose copies could only drift); and **S9's row decides that the trigger call runs inside the turn-queue worker, never on the request thread**, which turns P11-1's three workflow exceptions into a measured exemption rather than three new rows, and takes two obligations from S8c's guard. S9 is the only step row that moves.*
*2026-09-03 — v1.23: the stakeholder's answer to v1.22's one open question — a turn that dies on the worker becomes visible as **`lastTurn: 'failed' | null` alongside `turn.state`**, not as a fourth state, so `in_flight` stays `state != idle` and the `409 TurnInProgress` gate S9 is building is untouched; §5.2 gains the field and its lifecycle (set by the worker, cleared when the next turn is accepted and by either reset, process-local), C6a gains the render rule and the argument for why this takes **no** completeness-table row, and S9, S12a, S13, S15 plus §6.1/§6.4 inherit it. S12b/S12c/S12d and S10 deliberately do not — reasons in each place.*
*2026-09-07 — v1.24: §5.1's S9 row names `WorkflowRunNotFoundError` for the absent start anchor (`services.py:2085`) and states, per exception, which `INHERITED_HANDLERS` excuse the S8c/S8e guard can measure at S9 and which S9 must argue (`docs/reviews/salesperson-ui-impl.md` `## Pass 14`, P14-3).*
*2026-09-07 — v1.25: the trigger spelling is settled (`teco`), so §5.1's S9 row replaces the "S8c goes red" done-condition with the reach staying green as a service-surface tripwire, and names the response-boundary measurement — plus the executor-side raises it also covers — as the sole mechanism for the three excuses.*
*2026-09-07 — v1.26: §5.1's S9 row now **cites** the guards' own statement of their reach — `falkor-chat/server/falkorchat/storefront_api.py` above `INHERITED_HANDLERS` and `falkor-chat/server/tests/test_storefront_api.py` above `SERVICE_LAYER_REACH_TODAY`, both final as of `b720bd3` — instead of re-describing it, so the mechanism has one home and the row keeps only S9's obligations, its done-conditions, the reversal trigger and the **do-not-restore-a-reddens-at-S9** prohibition (now cited to that statement as its evidence). Two corrections fall out of the narrowing: §5.3's preamble still carried the reddens-at-S9 claim v1.25 had removed from the S9 row and never swept here — and the row's per-exception counterfactual (what the *rejected* spelling's walk would have reported) was falsified when P14-1's `Services`-sibling closure landed, `_reached_methods` now closing a seeded `start_workflow_run` over `self._require_executor`; the counterfactual is dropped rather than re-derived, since no S9 obligation depends on a spelling this row rejects.*
*2026-09-08 — v1.27: `docs/reviews/salesperson-ui-impl.md` `## Pass 17`'s two majors are plan defects and are corrected here — §4.4 measure 1a and §5.1's S9 row make the `409` single-flight an **atomic reserve-then-write** carrying a per-booking token, released on every path that never reaches a worker (P17-1, P17-3), and new §5.2 *The queue position* defines `queuePosition` as the waiting-line index **derived on read**, with S9's interface cell (which also picks up the `lastTurn` v1.23 left out of it), S9's done-condition, §5.3 C6b's `504` reconciliation, §6.1 and S13 swept to match (P17-2).*
*2026-09-08 — v1.28: revised against `docs/reviews/salesperson-ui-impl.md` `## Pass 18` (needs changes, 0 blockers) — the submit-under-lock prohibition keeps its rule and loses its false deadlock mechanism (P18-1: `_python_exit` joins outside every lock, so the cost is a delayed exit, not a hang), the booking ordinal is pinned as a process-global never-reset counter now that it both orders the line and owns the slot (P18-2), the ownership condition covers **every** map write including the release (P18-3), §5.2 defines the line by booking order so its definition and the row's reorder concession agree (P18-4) and states the `turn_in_flight` half of its one bound as accepted (P18-5), the `504` consequence is corrected from *undecidable* to *decides wrongly* in both homes, and S9 picks up the two delivered prose blocks no plan sweep can reach (P18-6) plus the linchpin test's mechanism (P18-7).*
*2026-09-08 — v1.29: `docs/reviews/salesperson-ui-impl.md` `## Pass 19`'s two nits — the booking ordinal is scoped **per-`Storefront`** rather than to the process, the looser scoping having named a second wrong implementation (a module-level counter) while closing the first, and the submit-under-lock prohibition gains the reversal trigger its one contingent leg earns, shaped like S10's: it names the condition (§5.2 ceasing to define the line by booking order) **and** redirects the remedy away from the lock.*
*2026-09-08 — v1.30: `docs/reviews/salesperson-ui-impl.md` `## Pass 20`'s P20-1 is a plan defect and is corrected here — §5.1's S9 row keeps the rule (a reservation that reaches no worker must not survive) and replaces the false mechanism printed beside it, since a thread-exhaustion refusal queues the work item **before** it fails (re-read at `/usr/lib/python3.12/concurrent/futures/thread.py:164-180` and re-run in this revision): the release moves **ahead of** `submit`, behind a `shutdown_turns()`-set flag, rather than conditioning an `except` on an exception identity that cannot discriminate — with a tombstone over the replaced mechanism, the accepted leak derived from `submit`'s own pre-queue raises, a done-condition for the held booking, and §5.2 stating which direction of `turn_in_flight` error the plan takes where the two are indistinguishable.*
*2026-09-08 — v1.31: `docs/reviews/salesperson-ui-impl.md` `## Pass 21`'s P21-4 and P21-5 — §5.2 stops re-summarising the residue and cites §5.1's S9 row for it (tombstoned), the row itself now names the two doors its own précis had generalised past (the `MemoryError` pair leaks on a healthy pool; a queued item does not run on a cold one, both re-measured here) and states the early-read placement's own cost — a post refused in the shutdown window that would previously have been drained — as accepted.*
*2026-09-09 — v1.32: the three plan rulings out of S9's first **acceptance** pass (`falkor-chat/docs/test-reports/salesperson-ui-s9-report.md`, PASS 31/31, the first execution-level gate after six static ones) — **D-2** is ruled a *code* defect rather than a missing table row and the shutdown-window refusal is typed as **`503 turn_not_scheduled`** with its own client rule **C14**, because the table's `(route, response)` key cannot separate a designed refusal from the unmapped-`5xx` residue sharing its status, and because C9's action is a retry that is safe only where nothing changed (obligation on **S9e**; S8's own *no bare `500`* assertion is narrowed to the reach its mechanism arms, and the lost turn stays the accepted price v1.31 set); **D-3** is ruled **key present, value `null`**, delivered behaviour with the document as the defect, and §5.2 gains the *absent versus null* convention the `incomplete` rule was the only instance of — §4.8's F8 clause and S7's and S10's done-conditions swept, and the block named per route, reset-all's being the roster rather than state (source-read in this revision, not measured by that pass); and S9's poll-latency done-condition, which named no threshold and so could not fail, is replaced by a two-sided bound the qa pass's own substitute is quoted into.*
*2026-09-09 — v1.33: `docs/reviews/salesperson-ui-impl.md` `## Pass 23` (needs changes, 1 blocker), every finding of which was executed — **P23-1**, the blocker, is v1.32's replacement poll-latency bound passing on both designs its own sentence named (measured 1.00× for the turn on the request thread, which never approaches `THREAD_LIMIT`, and a 4.18 ms median against a 14 900 ms single sample for the contended lock, which the median discards): the bound gains a **max** clause and an assert-saturation-still-holds precondition, loses the request-thread claim to a new first clause of the same done-condition that actually asserts it, and states what no statistic here reddens; **P23-2** is C14 stating its condition wider than its mechanism — the read→submit race raises a *plain* `RuntimeError` a subclass mapping cannot catch — so §5.2's row, the completeness row and C14 are **qualified** rather than the mapping widened (which would put the token on the exhaustion shape, where the turn does run), and both untyped shapes are named in C13's residual; **P23-3** retires v1.32's claim that the eighth residue was invisible to static reading — it was recorded and **excused by a written allowlist reason** at `d776ca8`, present at the commit the acceptance pass tested, so it moves to the *mis-ruled* bucket and S9e's obligation grows from three sites to seven, three of them in the suite; **P23-6** is `unresolved`, the fifth optional field, found inside Ruling B's own first case — the discriminator is restated over **co-varying groups** rather than keys; **P23-5** and **P23-7** land as an S9e obligation and an S10 clause. Ruling B's reset-all half is now **executed** rather than source-read (Appendix S §2). P23-4 is `coder`'s, not this document's. **This is the last static touch on this surface**: the plan lane closes here, and Pass 24 is warranted only if triggered by an execution or by S9e landing.*
*2026-09-13 — v1.34: `falkor-chat/docs/reviews/salesperson-ui-s12a.md`'s Major finding — §5.0 named `routes.tsx` as S12a-only while S13's/S12d's/S14's real views have no authorized way to reach it — is closed with two different mechanisms, chosen per whether the file already exists: **S13** and **S12d** each gain one narrow, additive edit right on the *delivered* `routes.tsx` (swap their own inline placeholder — `ChatScreen`; `PresenterKeyScreen`+`PresenterRoster` — for a real import, nothing else); since both land in the same parallel stage and target the same file, `teco` **serializes only that one swap apiece**, while the rest of S13's and S12d's own subtrees stay fully parallel. **S14** gets the zero-touch fix already proven on this file tree (`App.tsx`'s i18n/layout slots, §5.0's `main.tsx`/`App.tsx`/`index.css` row): S12b — not yet started, so this is a forward addition to unbuilt scope, not a redo — seeds four no-op placeholders inside S14's own subtree (`views/{Cart,Order,Profile,Catalog}Panel.tsx`) and wires them into its own sheet chrome before S14's stage starts, so S14 never edits a file it doesn't already own and needs no serialization with anyone. §5.0 gains two new rows (`routes.tsx`; the four seed placeholders) and the S12b/S12d/S13/S14 rows in §5.1 are swept to cite them.*
...
1. **A dedicated bounded turn executor, keyed by participant.** `storefront.py` owns its own
   ...
   position rather than an indefinite spinner. **What that number means is defined in §5.2**,
   under *The queue position*: the waiting-line index, derived on every read — a position fixed at
   booking is stale by its second poll.
   ...
   **1a — at most one in-flight turn per participant, enforced server-side.** This is not
   ...
   deliberate: a written message with no reply would sit in the transcript forever. **And
   *enforced* means atomic:** the check and the booking are one indivisible step, never a read on
   the request thread followed by a write with a FalkorDB round trip in between. Check-then-act
   admits two turns for one participant on a double-tap or a second tab (§5.3's own cross-tab
   case), and the turn map — one slot per participant — then reports that participant **`idle`
   while a turn is still running on a worker**, which is the state `_await_quiesce` exists to make
   impossible and the reason this is a corrupted invariant rather than a duplicated unit of work
   (`docs/reviews/salesperson-ui-impl.md` `## Pass 17`, P17-1, reproduced). §5.1's S9 row spells
   the sequence that satisfies this; this measure is the invariant it has to satisfy. The client
...
- **F8 — a reset that times out on the way to FalkorDB means *unknown*, not "nothing changed",
  ...
  **The re-read is allowed to fail too, and it is the *likelier* fault.** It is another query
  ...
  catch it and still return **`504 reset_state_unknown`**, carrying **the evidence key with a
  `null` value** rather than dropping it — §5.2's *Absent versus null on the wire* is the rule and
  this is one of its cases. **The key is named per route and it is not `state` on both**:
  reset-mine carries `state` (the re-read `get_state` payload) and reset-all carries
  `participants` (the re-read roster), each one read inside its own route body and passed as
  `None` when that read failed too. The
  participant-facing meaning is identical either way — *unknown*, never "nothing changed"; the
  block is a courtesy the response carries when it can, not the contract — **which is exactly why
  its *shape* may not vary with whether the courtesy could be paid**: a client branching on the
  key's presence rather than on its value renders an absent block as a real one
  (`falkor-chat/docs/test-reports/salesperson-ui-s9-report.md` D-3, which measured reset-mine at
  `'state' key present=True, non-null=False`; reset-all's half is a source read taken in this
  revision, not measured by that pass). **This is delivered behaviour and the document was the
  defect — no code changes**, and §5.3 **C4** carries the client half. Both orderings
...
1. **The storefront deployment does not mount the unauthenticated surfaces at all.**
   ...
   With `FALKORCHAT_STOREFRONT_ENABLED=1` everything this application registers is: `GET /health`
   ...
   **The one exemption, stated here because the claim is otherwise false:** FastAPI registers its
   own documentation routes on *every* app it builds — `/openapi.json`, `/docs`,
   `/docs/oauth2-redirect`, `/redoc` — and they are present and answer `200` on the storefront
   deployment too (`docs/reviews/salesperson-ui-impl.md` `## Pass 10`, P10-12; re-verified at
   v1.21 by enumerating `create_app(storefront=True, dev_surface=False).routes`, which carries all
   **four** — P10-12 named three, omitting `/docs/oauth2-redirect`). They disclose **this
   deployment's own eleven-route schema and nothing else** — no graph read, no participant data,
   and with the legacy router un-mounted they cannot describe it — so B2's hole stays closed and
   the residual is schema disclosure on a controlled LAN, already inside R6. Not closed here, and
   the mechanism if it ever must be is `create_app`'s `docs_url=None, redoc_url=None,
   openapi_url=None` on the storefront branch, **not** a route-table edit. One reason to keep them:
   S8's `x-storefront-tokens` extension keys survive into `/openapi.json` intact, which makes
   §5.3's completeness table readable off a running server.
...
- **Route-table assertion, keyed on the `storefront` *parameter*, not on `config.STOREFRONT_ENABLED`.**
  ...
  **What the delivered assertion actually says**, since move 1's exemption lives in the test rather
  than in the prose: `_registered_paths` (`falkor-chat/server/tests/test_app.py`) subtracts
  `_FASTAPI_BUILTIN_PATHS` — *the paths a bare `FastAPI()` registers*, computed at import rather
  than written out — and then asserts **set equality** on the remainder: exactly the eleven
  `/shop/api` routes plus the bare `GET /health`, with the `Mount` list exactly `["/shop"]`. It
  differs from move 1's claim in both directions, deliberately. **Stronger:** it is an exact set,
  over paths that carry their mount prefix, so a router that landed at `/` rather than `/shop/api`
  fails it — which no 404 probe can see. **Weaker:** the exemption is *derived*, not enumerated, so
  a future FastAPI version that adds a default route would exempt it silently. That trade is the
  right way round (a framework upgrade must not red-fail an assertion about this app), but it means
  the exemption's exact statement is **"whatever a bare `FastAPI()` registers"** — the four paths
  named in move 1 are today's value of that expression, not the rule.
...
### 4.11 `App.tsx`'s provider order and the `LayoutShell`/router seam (S12b, v1.36)

`falkor-chat/docs/reviews/salesperson-ui-s12b.md`'s Major finding, echoing the delegate's own
report, traced the actual render tree: `App.tsx` nests `<LayoutShell>` **outside**
`<QueryClientProvider>`/`<SessionProvider>`/`<RouterProvider>` (they are `LayoutShell`'s own
`children`), while `Shell.tsx` renders `<Header/>` and all four `*Sheet` components as **siblings**
of that wrapped `children`, not descendants of it. Nothing `layout/**`/`components/sheets/**`
renders can reach `useSession()`, `useQuery()`/`useMutation()`, or `useNavigate()`/`useLocation()`.
S12b's own (uncommitted, "needs changes") interim for the one place this bit —
`ResetControl` needing `SessionContext`'s `setPendingLanguageStep` after a successful reset — is a
module-level singleton bridge (`layout/sessionBridge.ts`) spliced in by a `cloneElement`-based
structural-assumption helper (`layout/injectBridge.tsx`'s `spliceIntoGrandchild`). §5.0's v1.34 seed
row already commits S14's four panels to mount **inside these same sheets**, and every one needs
`useShopState()`/`useCatalog()` at minimum, with the order panel's actions needing
`useAdvanceOrder()` (itself needing `useNavigate` transitively via `useErrorEffects`) — at least
three more bespoke bridges in S14's shape, one of them (`QueryClient`, over cached/keyed data
rather than one primitive value) materially harder to make safe than a single setter, or S14 cannot
deliver its scope without an unauthorized `App.tsx` edit.

**Decision: reorder, via a pathless layout route — not a provider-order swap alone.** Traced
concretely against the actual code and the actual framework API, not assumed:

1. **`LayoutShell` needs nothing from the router today.** `Header.tsx` calls only
   `useSheetState()` (this subtree's own context, `layout/SheetContext.tsx`); nothing in
   `Shell.tsx`/`Header.tsx` calls `useLocation()` or any other router hook for icon highlighting or
   any other chrome. The reorder is not blocked by an existing router dependency in the chrome
   itself.
2. **A bare provider-order swap — move `<LayoutShell>` to wrap `<RouterProvider>`'s output from
   inside `<SessionProvider>`, keeping today's `children`-prop shape — is not merely inferior, it
   is structurally impossible for the router half of the fix.** `RouterProvider`'s own type
   (`react-router`'s `dom-export.d.ts`: `RouterProviderProps = Omit<RouterProviderProps$1,
   "flushSync">`; the underlying shape is `{ router, flushSync, onError, useTransitions }`) takes
   **no `children` prop** — `<RouterProvider router={router} />` renders its own matched-route tree
   internally, with no JSX slot to put `Header`/the sheets "inside" it other than as part of that
   matched tree. A swap that only reorders `<LayoutShell>` relative to the three providers would
   fix `useSession()`/`useQuery()`/`useMutation()` reachability (`SessionProvider` and
   `QueryClientProvider` *are* plain `{children}` passthroughs, confirmed by reading
   `session/SessionContext.tsx` and `@tanstack/react-query`) but would leave
   `useNavigate()`/`useLocation()` exactly as unreachable as today, because `Header`/the sheets
   would still be siblings of `<RouterProvider>`, never part of its rendered output.
3. **The fix that actually reaches all three is a pathless (layout) route** — react-router v7's
   standard mechanism for wrapping every matched route in shared chrome (current framework docs:
   a parent route object with `element`/`Component` + `children` and no `path`; the matched child
   renders via `<Outlet/>`, still exported from `react-router-dom`; unchanged from v6's data-router
   API). Make `LayoutShell` **itself** the router's top-level route element, with the existing
   three routes (`APP_PATHS.participant`, `APP_PATHS.presenter`, the `*` catch-all) as its
   `children`, and have `LayoutShell` render `<Outlet/>` where it renders `{children}` today.
   Because `LayoutShell` is then rendered *by* the router as part of its own matched-route tree,
   everything it renders — not just `<Outlet/>`'s content — sits inside the router's context
   (`NavigationContext`/`LocationContext`, established once by `RouterProvider` around its whole
   rendered output): the ordinary "layout + sidebar" pattern react-router ships as its reference
   shape, not a special case. That is what gets `Header`/the four sheets/S14's future panels
   genuine `useNavigate()`/`useLocation()` access, which a bare provider swap cannot.
4. **`I18nProvider` stays outermost, unchanged.** It is a plain `{children}` passthrough over
   `I18nextProvider` with no dependency on session/query/router state (`i18n/Provider.tsx`), so its
   position is inert to this fix; nothing in S12a's or S12c's delivered code assumes a specific
   position for it relative to the other three.
5. **`spliceIntoGrandchild`/`SessionBridge` become dead code.** Once `LayoutShell` is a genuine
   router/session/query descendant, `ResetControl` can call `useResetMine()` (`api/hooks.ts:304`)
   directly instead of the bridge: that hook already implements the full C1–C14 dispatch
   (`resolveErrorAction` via `useErrorEffects`), the C7 navigate-to-language-step, and the C4/C9
   `queryClient.invalidateQueries` on a `state` reread — which **also resolves the S12b review's
   Blocker (dropped C3/C4/C6b/C13) and Minor (no post-reset cache invalidation) findings for
   free**, as a consequence of the simplification rather than a separate patch.
   `layout/sessionBridge.ts`, `layout/injectBridge.tsx` and `layout/injectBridge.test.tsx` are
   deleted; `Shell.tsx` drops the splice call and the `children` prop entirely.
...
`LayoutShell` is no longer imported here — it moves into `routes.tsx` as the layout route's
element. `routes.tsx`'s router construction:
...
`layout/Shell.tsx`'s `LayoutShell` drops its `{ children }` prop and the
`spliceIntoGrandchild`/`SessionBridge` machinery, imports `Outlet` from `react-router-dom`, and
renders it where `{withBridge}` sits today. This preserves today's actual behaviour exactly — both
the participant and presenter routes already render inside `LayoutShell`'s chrome (it wraps the
whole `<RouterProvider>` today) and continue to after the fix, now correctly nested inside the
providers rather than outside them; whether the presenter route *should* carry the shopper's
header/sheet chrome at all is a pre-existing question this fix does not reopen.

**Ripple.** `Shell.test.tsx` (constructs `LayoutShell` with an explicit `children` tree today) is
rewritten to mount `LayoutShell` as a layout route over a `createMemoryRouter` child route.
`ResetControl.tsx` drops its `resetMine`/`ApiError`/`router`/`APP_PATHS`/
`getPendingLanguageStepSetter` imports in favour of `useResetMine()`, and its `catch` block is
replaced by switching on `resetMine.action.kind` — mirroring the Blocker finding's own recommended
fix (`clearParticipant` → clear + redirect, `reread`/`nothingChangedRetry` → distinct 503/504 copy,
`unscopedAlarm` → its own copy, `unhandled` → a visible failure) — which closes the Blocker as part
of the same edit, not a second pass. No test imports `App.tsx` directly (`main.tsx` is the only
importer), so no other test file is affected.

**Alternative rejected: accept the bridge pattern as ongoing precedent for S14.** Rejected on the
S12b review's own cost argument, independently confirmed here — a `QueryClient` bridge for cached,
keyed query results is materially harder and riskier to build correctly than a single-setter
singleton, and S14 would still need to invent it plus a third bridge for navigation: three bespoke,
no-op-degrading mechanisms carrying real behavioral risk (a `QueryClient` bridge that silently
degrades to "no cache" on a structural-assumption break is a far worse failure mode than losing one
UI nicety), versus one pair of narrow, verified, precedented (`routes.tsx` v1.34/v1.35) exceptions
to two files' ownership.

**Ownership.** `App.tsx` is S12a's owned file ("no later step edits it," `salesperson/AGENTS.md`);
reopening S12a for a fix this far from session/API/routing plumbing would be the wrong unit, and
`routes.tsx` already carries the identical precedent (S12c/S13/S12d each earned one narrow,
additive edit right rather than S12a's row being reopened). The step that owns the adjacent
semantic work here — `layout/**`, `components/sheets/**`, and the bridge machinery being retired —
is **S12b**, already due back for its own Blocker fix in the very same `ResetControl.tsx`/
`Shell.tsx` files and not yet accepted. §5.0/§5.1 grant S12b's revision one narrow, additive edit
right on each of `App.tsx` (the provider reorder) and `routes.tsx` (the layout-route wrap) — the
same shape as the `routes.tsx` grants, assigned to the step whose own subtree the fix is inseparable
from rather than to a freshly minted step that would just have to touch the same two files a second
time.

**The secondary symptom (stale post-reset cache) needs no separate follow-up** — §4.11 point 5
above: `useResetMine()`'s `onError`/`onSuccess` already own the `queryClient` invalidation the
review's Minor was missing.

### 4.12 The welcome turn's missing path — `SessionContext`/`hooks.ts` grant to S13 (v1.37)

**The gap, confirmed real and already named, not inferred.** §5.1's S13 row commits to "**welcome
turn — the join response's `welcome` line (§5.2), never a greeting the client composes**"; §5.2's
*The join greeting* section specifies the server-minted, per-locale `welcome` line `POST
/shop/api/session` returns; §5.3's credentials table already rules **`welcome` out of stored
state** ("the `POST /shop/api/session` response **minus `welcome`**, which is a one-shot greeting
rather than session state") — `session/types.ts`'s own header comment says the same thing about
`ParticipantSession`. S13's delivered `ChatView.tsx` (own comment, lines 12–22) documents the
resulting hole directly: `routes.tsx`'s `JoinScreen` calls `useJoin()`, whose `onSuccess`
(`api/hooks.ts`) reads `data.welcome` only long enough to *not* pass it to `setParticipant(...)`,
and no other path carries the line from there to `ChatView.tsx`. `session/SessionContext.tsx` and
`api/hooks.ts` are both S12a's, with no later-step arrow (`salesperson/AGENTS.md`'s ownership
table) — unlike `routes.tsx`, which already carries four later grants (§5.0).

**Design: one ephemeral context field, mirroring `pendingLanguageStep` exactly.**
`SessionContextValue` gains `welcomeMessage: string | null` and `setWelcomeMessage(message: string
| null): void`, built the same way `pendingLanguageStep`/`setPendingLanguageStep` already are —
plain `useState` inside `SessionProvider`, included in the context's `value`/`useMemo` dependency
list, **never** passed to `saveParticipantSession`/`storage.ts` and **never** added to the
`ParticipantSession` interface. That non-persistence is what makes §5.3's "not session state" rule
structural rather than a convention the client happens to follow: a page reload always starts
`welcomeMessage` at `null`, the same way it always starts `pendingLanguageStep` at `null`.

- **Set once** — `hooks.ts`'s `useJoin()` `onSuccess`, one line added beside the existing
  `setPendingLanguageStep(null)`/`setParticipant(...)` calls: `setWelcomeMessage(data.welcome)`.
  `JoinResponse.welcome: string` is already typed (`api/endpoints.ts`) — no type change needed.
- **Cleared once, by the consumer** — `ChatView.tsx` (already S13's own file, unrestricted by any
  later-step lock) reads `welcomeMessage` from `useSession()`, renders it once, and calls
  `setWelcomeMessage(null)` having done so (e.g. capture it into local state on first render, then
  clear the context field) — never re-shown on a later `ChatView` mount within the same join,
  which is the correct reading of "the greeting belongs to the join moment": it is a one-time
  event, not a value that happens to reset on reload. This is the only clearing site; no other
  hook or component may call `setWelcomeMessage`.

**Rejected alternative 1 — clear it inside `useResetMine()`'s `onSuccess`, symmetric with how
reset *sets* `pendingLanguageStep`.** Rejected: by the time `useResetMine()` fires, `ChatView` has
already consumed and cleared `welcomeMessage` in the ordinary case (the participant reached chat
before resetting), and in the edge case where they somehow haven't, the *next* `useJoin()` call
overwrites it with the new join's own line before `ChatView` mounts again — the same overwrite
`pendingLanguageStep` itself relies on `useJoin()`'s `setPendingLanguageStep(null)` for. A second
clearing site would duplicate that guarantee without covering anything the consumer-clears-it rule
doesn't already cover, and it multiplies the surface a future edit has to keep in sync.

**Rejected alternative 2 — thread `welcome` through router/location state from `routes.tsx`'s
`JoinScreen` instead of through context.** Rejected on architectural grounds, not merely style:
`ParticipantRoute` (`routes.tsx`) already decides between `JoinScreen`/`ChatView` purely by reading
`participant`/`pendingLanguageStep` off `useSession()` — there is no `navigate()` call and no
router state in this flow at all, so a location-state design would be a second, parallel
hand-off mechanism next to the one already proven for `pendingLanguageStep`, for no benefit. It
would also mean editing `JoinScreen`'s body inside `routes.tsx` itself, a file whose ownership
model (§5.0) depends on every later step's edit right being *one narrow, additive swap and nothing
else* precisely so five steps (S12a/S12b/S12c/S13/S12d) can share it without collision; adding new
logic to `JoinScreen` would break that discipline for a case the existing context mechanism already
handles.

**Explicitly out of scope for this grant** — no edit to `session/types.ts` (`ParticipantSession`
correctly excludes `welcome` already, and stays that way), `session/storage.ts`, `api/endpoints.ts`
(`JoinResponse` is already typed), `routes.tsx`, or any hook in `hooks.ts` other than `useJoin()`.
...
| **S8** | **The `/shop/api` router + mounts.** `storefront_api.build_storefront_router(...)`, the `get_participant()`/`get_presenter()` dependencies, size-bounded Pydantic models mirroring `schemas.py`, **the error map — which this step makes *total by type*, the plan's first totality guard**: today `_register_error_handlers` (`app.py:80`) maps `ServiceError` and the workflow errors only, and **`FalkorDBUnreachableError` (`db.py:47`) has no handler at all**, so a query-time `redis.exceptions.TimeoutError` escapes as a bare `500` — on `/state` and `/messages`, for **every polling participant at once**, in exactly the scenario §4.8's F8 exists for. S8 registers **typed** handlers, in this codebase's own stated idiom of typed handlers "without a blanket handler masking real bugs" (`app.py:136-137`): `FalkorDBUnreachableError` and `redis.exceptions.ConnectionError` → **`503 graph_unavailable`** (nothing was sent — the precedent is `api.py:63` / `app.py:345`); a query-time `redis.exceptions.TimeoutError` → **`503 graph_read_timeout`** on a `reads-only` route and **`504 <op>_state_unknown`** on a `writes` one, with `<op>` fixed per route (`join` / `post` / `order` / `reset`) by §5.3's cross-cutting table. **The classes are §5.3's route-class table, which this step is built against** — five `writes`, four `reads-only`, and **two routes (`GET /shop/api/health`, `POST /shop/api/presenter/session`) that issue no query at all** and therefore can produce **none** of the three; a handler firing on either is a defect, not a row. **Precedence, stated so it cannot be built the other way round:** the two reset routes catch the timeout themselves and answer F8's named `504 reset_state_unknown` (S7/S10); the typed handler is the backstop for the other **seven graph-touching** routes, and must not pre-empt them. Also **maps `RequestValidationError` to the storefront's own stable `{error: "validation_failed", field: "<name>"}`** rather than exposing FastAPI's `loc` shape, because §5.3 C11 dispatches on `field` and a framework-detail body is not a contract — **selection rule pinned: the *first* entry of `exc.errors()` (declaration order for a single request model), and `field` is the last element of its `loc`** (`displayName`, never `body.displayName`), so a multi-violation request produces one deterministic, client-facing name (§5.3 C11). Plus `schemas.py:256-257`'s reserved-key list, which still omits `timerFired`** (`docs/reviews/salesperson-ui-impl.md` F-6 — routed here because S8 edits `schemas.py` anyway), and the `create_app` wiring: include the router and mount `FALKORCHAT_STOREFRONT_DIR` at `/shop` **inside `create_app`** (`/` is a catch-all registered last and Starlette matches in registration order, so a mount added after `create_app` returns is unreachable). **`create_app` must pass its `storefront_dir` explicitly into the `Storefront` it constructs, and mount that same one value at `/shop`** — S7 shipped `Storefront(..., storefront_dir=None)` falling back to `config.STOREFRONT_DIR` (`falkorchat/storefront.py:435`), so a `create_app` that takes the parameter for the mount and lets the `Storefront` fall back to config builds the manifest from one tree while serving assets from another: every `imageUrl` `null` in the benign case, and keyed on the wrong tree in the worse one. §4.7 is written about exactly this failure, and AC-11's negative branch masks it. Also **call `build_image_manifest()` in `_lifespan`**, beside the preflight — §4.7's "built at startup only" is stated nowhere in a step until here, and leaving it to `list_catalog`'s first call makes the first participant's catalog fetch list the directory. It is **not** a preflight condition: an empty manifest is a legitimate deployment (the text-only card variant), so log the count and start. Plus §4.9's **route-table assertion** and the **readiness preflight** in `_lifespan`. Plus **a per-route `responses={…}` declaration on every one of the eleven routes**, naming exactly that route's own returns (its status codes and their bodies — *not* the three cross-cutting ones, which come from the handler set): FastAPI keeps them on the route object, so they are machine-readable and turn the second half of the gate below from a reading exercise into an assertion, at the cost of one dict per route. | `falkor-chat/server/falkorchat/storefront_api.py` (new), `.../schemas.py`, `.../app.py`, `falkor-chat/server/tests/test_storefront_api.py` (new), `.../test_app.py` | `create_app(..., storefront: bool = False, storefront_dir: Path \| None = None, dev_surface: bool = True)` — **`storefront_dir` is forwarded, one value, to both `Storefront(storefront_dir=…)` and the `/shop` mount**; routes per §5.2; `401` bad/absent token, `403` bad presenter key, `404` unknown order, `409` stale CAS / turn in flight / `unscoped_participant`, **`422 validation_failed` + `field`** from the Pydantic bounds on all five input-taking routes (§5.2, §5.3 C11), `503` quiesce timeout, **`503 graph_unavailable`** / **`503 graph_read_timeout`** / **`504 <op>_state_unknown`** from the typed handlers, **`504 reset_state_unknown`** on a reset's own FalkorDB socket timeout, and a `Thread` UNIQUE violation on **reset-mine** propagating as `5xx`, never retried (§5.2) | `TestClient` contract tests for every route, incl. the **auth matrix** and **the cross-participant probe**: with A holding cart items, messages and an order, every route called with B's token returns only B's data. **A stubbed repository raising `redis.exceptions.TimeoutError` from a reset is called exactly once** — the *application* layer never retries (§4.8's stated premise; the library layer is beyond this test's reach by construction, which is why the premise is written down rather than only asserted). **The gate is decidable, not a reading exercise, and it has two halves.** *(i) Handler half:* **`{registered handlers} × {routes-that-can-raise-them} ⊆ §5.3's completeness table`** — enumerate the handlers actually registered on the app object, cross them with the route table **filtered by §5.3's route classes** (a `503 graph_read_timeout` is expected on the four `reads-only` routes, a `504 <op>_state_unknown` on the five `writes`, and **neither on the two `no graph access` routes**), and assert every resulting `(route, response)` has a row; a handler with no row, or a row with no producer, fails the step. Without the class filter this half is not computable and its symmetric side falsely fails on `/health` and `presenter/session` — the classification is the input, not a nicety. *(ii) Declaration half:* **`{declared in each route's `responses={…}`} ∪ {handler-produced} == §5.3's table`**, read back off `app.routes`; every route must carry a declaration, so an omission fails loudly rather than silently shrinking the set. **What neither half closes** — a handler that returns a `JSONResponse(status_code=…)` directly never raises, so it is invisible to the handler set, and a declaration is itself an enumeration that can be wrong (a declared response nobody produces, or a produced one nobody declared). The declaration half narrows that residue and does not remove it; the per-route contract tests then prove each declared entry is actually producible, so the two disagree loudly instead of agreeing by omission — and **C13 is the backstop for whatever all three miss**, which is the reason the client guard exists rather than being redundant with this one. **Asserted by execution, not inspection:** a stubbed repository raising `redis.exceptions.ConnectionError` returns `503 graph_unavailable` on `/state`, and one raising `redis.exceptions.TimeoutError` returns `503 graph_read_timeout` on `/state` but `504` on `POST /messages` — **and no route answers a bare `500` on any fault this gate arms**. *Stated unqualified through v1.31, and in that form it is measurably false: the acceptance pass drove `POST /shop/api/messages` inside the shutdown window and read `500`, `content-type: text/plain`, body `Internal Server Error` — no envelope, no token, from no handler at all (`falkor-chat/docs/test-reports/salesperson-ui-s9-report.md` D-2/TP-012b). **The pass supplied the ruling, not the discovery**: that same response was already recorded, and excused by a written reason, in the suite's own `NON_FAMILY_RAISES` allowlist before it ran (§5.3 C13's residual paragraph; `docs/reviews/salesperson-ui-impl.md` `## Pass 23`, P23-3). The gate's mechanism is innocent: it arms repository faults, and that refusal comes from the turn layer past every one of them, which is the sense in which S8 is structurally blind to it — the same residue this row already names under "What neither half closes". So the claim is narrowed to the reach it actually has, and the escape it cannot see is closed where it is produced rather than by widening this assertion: S9e types that refusal (§5.3 **C14**, §5.1's S9 row).* **Two source tripwires carried from S6's gate** (`docs/reviews/salesperson-ui-impl.md` `## Pass 6`, answers 1 and 4): `storefront_api.py` **never calls `.lookup(`** — `lookup` and `resolve_token` return the identical `ParticipantRecord`, so a router authenticating through the read-through cache would be indistinguishable from one authenticating against the graph; and `create_app` constructs the `Storefront` **without passing `id_gen`** — S6's participant-id collision argument rests on no caller ever pinning it, and S8 is the first caller. `/shop` shadows nothing. **The two `no graph access` routes are asserted negatively**: with the repository stubbed to raise on *any* call, `GET /shop/api/health` and `POST /shop/api/presenter/session` still answer their normal `200`/`403`/`422` — proving they issue no query and so can produce none of the three cross-cutting responses. **The image wiring is asserted so that missing it goes red, which the obvious version of this test does not**: with `config.STOREFRONT_DIR` monkeypatched to a **different, also-populated** directory, `create_app(storefront=True, storefront_dir=<tmp>)` must serve `GET /shop/api/catalog` carrying **`<tmp>`'s** `imageUrl`s and `GET /shop/products/<id>.<ext>` must `200` out of that same tree — so a `create_app` that reads config instead of forwarding the parameter fails with *wrong* URLs and one that forwards only to the mount fails with `null` ones (a single populated directory against an unset config catches only the second, which is why the config default is pointed somewhere real). Preflight refuses to start on a missing `Agent`, missing snapshot or empty catalog, naming the fix command | `coder` | **after S7; owns `app.py` after S3** |
| **S9** | **Concurrency layer** (§4.4). **Decided here rather than left to the implementer, because it decides what this step can even fail with: the trigger call runs *inside the turn-queue worker*, never on the request thread.** `enqueue_turn` **submits** — the request thread **reserves the turn slot (the `409` single-flight check and the turn-map booking as one atomic step, §4.4 measure 1a)**, then does `services.post_message` and `executor.submit(...)`, **releasing the reservation only on the failures that queued no work — which is decided *before* `submit`, never inside its `except` (spelled out below)**, and then answers. `trigger.maybe_trigger` → `services.start_workflow_run` runs on the worker. Three independent reasons, none of them a preference: **(i)** `start_workflow_run` is **synchronous and drives the whole run** — it "starts the run, then hands off to the injected executor which drives the §2.1 loop" (`falkor-chat/server/falkorchat/services.py`), i.e. up to eight chat completions against a 180 s agent timeout. On the request thread that *is* the turn, and `POST /shop/api/messages` becomes the slowest route in the system — the exact outcome §4.4 measure 1 exists to prevent, since measure 1 swaps **which scheduler** runs the turn, not whether it is scheduled. **(ii)** It is the platform's delivered posture on both existing transports: `background._safe_run_workflow` is failure-isolated and off-band — "*its latency never blocks the poster*" — scheduled on `BackgroundTasks` by `api.py` and on a thread by `mcp.py`; the storefront replaces `BackgroundTasks` with its own bounded executor and inherits the rest. **(iii)** This row already assumes it: `enqueue_turn` **receives** the `ParticipantRecord` from the request thread precisely so the worker never resolves one. **The consequence that matters for the gate:** `WorkflowEngineDisabledError`, `WorkflowInputRejectedError` and `WorkflowRunNotFoundError` (`services.py:1996` `_require_executor`; the `run_ctx` size bound, `:2056`; the absent start anchor — the snapshot has no START, or the trigger message is missing — `:2085`) are raised **after the `200` has been sent**, so none of them is a `(route, response)` pair and none earns a §5.2/§5.3 row — they get a **measured exemption** instead, on the standard §5.3's third narrowing sets (`docs/reviews/salesperson-ui-impl.md` `## Pass 11`, P11-1). **The consequence that is a real gap, and is not created by this decision but inherited with it:** a turn that dies on the worker is failure-isolated by that same contract, so unless S9 says otherwise the participant sees their message, no reply, and a composer that quietly re-enables — indistinguishable from a completed turn on `GET /shop/api/state`. The dominant instance is not those three exceptions at all but **any LLM failure** (endpoint down, 180 s timeout), which is ordinary. So a bare `finally: clear_turn(...)` is **not** an acceptable whole answer. **Decided by the stakeholder and specified in §5.2 *The dead-turn signal*: `turn.lastTurn: 'failed' \| null`, alongside `state`, never a fourth `state` value** — `TurnState.in_flight` stays `state != idle` on the **delivered** `storefront.py`, so this step does not inherit a change to the very gate it is building, and a participant whose last turn failed is postable-to, which is the recovery. The latch therefore **cannot live in the `_turns` entry**: `set_turn_state(idle)` deletes that entry by delivered design, which would wipe the signal at the instant it is earned — it is a separate per-participant latch, composed into `get_state`'s `turn` payload. S9 sets it on the worker, in the same isolation block that logs the failure; clears it when that participant's next turn is **accepted** (the enqueue, not the `409`); and clears it wherever a reset clears turns (`clear_all_turns` and reset-mine's own clear), because the transcript the notice refers to is gone. §5.3 **C6a** carries the client half and the argument for why this takes no completeness-table row. The bounded `ThreadPoolExecutor` turn queue **keyed by `participantId`**, the queue position §5.2 *The queue position* defines — **derived on read from the map's arrival ordinals, never a number stored at booking**, so a `_turns` entry carries its booking and `set_turn_state`'s `queue_position=` parameter goes with the field it fed, **and the two delivered prose blocks this redefinition falsifies are carried in the same change**: `config.py`'s `STOREFRONT_TURN_WORKERS` comment and `falkor-chat/docs/SERVER.md` §1.3's matching row both still state the *old* definition verbatim (*a position is how many accepted turns were unfinished when this one arrived*), which no sweep of this plan can reach (`docs/reviews/salesperson-ui-impl.md` `## Pass 18`, P18-6) — and §4.4 measure 1a's `409 TurnInProgress` refusal *before* the message write; the storefront post path (`services.post_message` + enqueue trigger with `run_ctx={"language": …}`, **no `_safe_embed`**); raise the anyio limiter **inside `_lifespan` before `yield`**; graceful executor shutdown. **The single-flight gate is a *reservation*, and that is one clause of sequence rather than a different route shape.** *Why* it must be atomic is §4.4 measure 1a; what the request thread does is here. `Storefront.reserve_turn(participant_id)` performs the check and the booking as one test-and-set under the turn lock, answering the new booking or `None` when that participant already has a turn in flight — that `None` **is** the `409`, raised by the route as it is today and still before the write; the route then hands the booking to `enqueue_turn`, which carries it onto the worker. **`services.post_message` raising releases the reservation, unconditionally** — nothing has been submitted at that point — and that release is first of all a **repair for a property the reservation would otherwise break**: without it §5.3's `504 post_state_unknown` reconciliation does not lose information, it decides **wrongly** — reading `turn.state !== 'idle'` and parking that client in *wait, as normal* forever for a turn nobody will run (`## Pass 18`, question 2; C6b decides correctly today, with no reservation at all, which is why the two must land in one change) — and secondly what stops a leaked booking `409`-locking that participant for the life of the process and turning every reset-mine of theirs into a `503` (`docs/reviews/salesperson-ui-impl.md` `## Pass 17`, P17-3). **A refusal out of `executor.submit(...)` is the other case, and the rule there is: release only where this plan can say the pool queued nothing — which is a place in the sequence, not a property of the exception.** So `enqueue_turn` reads a `_turns_shutdown` flag **before** it calls `submit`; on a set flag it **releases the booking and raises `RuntimeError`, having submitted nothing**. The flag is set by `shutdown_turns()` *before* that method calls `executor.shutdown(wait=True)` — monotonic `False → True`, never cleared, a plain attribute under no lock, because the only way to read it wrongly is to read a stale `False`, which lands on the safe side. Past that check the `except` around `submit` **logs at `ERROR` naming the participant and the booking ordinal, and re-raises, releasing nothing.** On the exhaustion shape that is *usually* not even a leak: the queued item runs and `_run_turn`'s `finally` clears the booking under its own ownership check, so the standing booking is what keeps `GET /shop/api/state` truthful for exactly as long as that turn is live. **"Usually" is load-bearing, and the exception is measured rather than assumed.** That item runs as soon as *some* worker exists; on a pool with none started yet, none does. Measured on the pinned 3.12.3 while writing this revision — fresh `ThreadPoolExecutor(max_workers=4)`, no prior submit, `threading.Thread.start` patched to raise: the refusal leaves `_work_queue.qsize()` at **1** and `len(_threads)` at **0**, nothing runs, and `shutdown(wait=True)` returns immediately having drained nothing, because its wait is `for t in self._threads: t.join()` (`:236-238`) over an empty set. It self-heals on the next successful `submit` (measured: that submit ran both items). The same refusal on a *warm* pool did run the item (measured), which is what bounds this to the cold case. **And the cold case is reachable rather than theoretical:** the executor is constructed eagerly precisely because `ThreadPoolExecutor` "starts no thread until the first `submit`" — the delivered comment standing over `Storefront.__init__`'s `self._executor = ThreadPoolExecutor(...)` — so a cold pool is the process's **first** turn. What it costs there is the booking standing until a later successful `submit`, plus that participant's message sitting in the transcript with no reply. **Documented rather than coded around, and that is a ruling, not an omission:** the only discriminator available at the `except` is `len(self._executor._threads)`, a private attribute, and reading it would put this design back on exactly the implementation detail v1.30 moved off (`docs/reviews/salesperson-ui-impl.md` `## Pass 21`, P21-4 and its open question 2). **Mechanism — read and re-run while writing this revision rather than carried from the review.** On the pinned CPython 3.12.3, `ThreadPoolExecutor.submit` (`/usr/lib/python3.12/concurrent/futures/thread.py:164-180`) does `self._work_queue.put(w)` at `:178` and only *then* `self._adjust_thread_count()` at `:179`, whose `t.start()` (`:202`) is where `RuntimeError("can't start new thread")` originates; `_worker`'s loop (`:69-95`) pulls from that same shared queue and never checks who put an item on it. So an exhaustion refusal raises with the work item **already queued**, and any worker — one that exists now, or one a *later* `submit` starts — will run it. Reproduced here in a throwaway script: one worker busy, `threading.Thread.start` patched to raise, `submit` raised `can't start new thread`, `_work_queue.qsize()` read **1** immediately after, and the "refused" job ran. Releasing the booking on that shape manufactures a live turn that `turn_in_flight` reports as `False` — §4.4 measure 1a's corrupted invariant, the one `_await_quiesce` exists to make impossible, through a third door, and reproduced end to end against the real `Storefront` (`docs/reviews/salesperson-ui-impl.md` `## Pass 20`, P20-1). *(Tombstone, 2026-09-08 — v1.30: through v1.29 this clause released the reservation on **any** exception out of `submit`, and named thread exhaustion as one of the two shapes it was repairing. The rule was and is right — a reservation that reaches no worker must not survive — and the mechanism printed beside it was false on that second shape, which queues before it fails. The mechanism was replaced rather than the rule weakened: the release moved ahead of `submit`, where "nothing was queued" is knowable. Re-read at `thread.py:164-180` and re-run in this revision, not inherited from the review that found it. Do not restore the unconditional form on rediscovering that a leaked booking is bad — it is, and it is the lesser of the two.)* **Why not discriminate on the exception, and why not the flag *inside* the `except`.** Type cannot tell the shapes apart — both are a bare `RuntimeError` (`:170` and `:202`) — and the only thing that differs is CPython's message string, which this plan will not pin. Pass 20's own suggestion, the same flag read inside the `except`, gets the direction right but is **strictly weaker** than reading it before the call, and the gap is not cosmetic: with the read inside the `except`, a `shutdown_turns()` landing between the flag write and the executor actually shutting down leaves a submit that reaches `_adjust_thread_count`, queues its item, fails, sees a set flag and releases — P20-1 reintroduced in a narrow window. Reading before the call closes that window instead of narrowing it, because such a refusal never enters `submit` at all; and it keeps `enqueue_turn`'s signature and return type untouched, the shutdown path raising exactly as it does today. **What the early read costs, so the next reader weighing the two placements sees both columns.** In that same window — flag set, executor not yet stopped — a post that under the previous shape (no flag read ahead of `submit`; the superseded file re-read in this revision) would have been submitted and then drained by `shutdown(wait=True)` is now refused *before* `submit`, so it loses its turn and leaves the participant's message in the transcript with no reply: the message is written first (the `POST /shop/api/messages` route body calls `services.post_message` and *then* `shop.enqueue_turn`, with nothing between them catching the `RuntimeError` — read in this revision), and the drain it loses is real rather than hypothetical — measured in this revision, `shutdown(wait=True)` on a live pool ran a just-submitted item before returning. **Accepted, and the reason is the asymmetry of what is lost, not the width of the window.** The window is bounded by `shutdown_turns()`'s own two statements — the `_turns_shutdown = True` write, then `self._executor.shutdown(wait=True)` — so it opens only after the server has decided to stop: the price is one turn on a process that is going away, and re-posting after a restart recovers it. The `except` placement's price is measure 1a's invariant corrupted for `_await_quiesce` on a process that is *staying up*, which nothing recovers. (`## Pass 21`'s P21-5 adds that the window is narrower still because uvicorn drains in-flight requests before running lifespan shutdown — taken from the review, not verified here; the acceptance rests on the sentence before it, which does not need it.) **What this accepts, derived from `submit` rather than listed.** Read `:164-180`: everything before the queue put at `:178` is three guarded raises — `BrokenThreadPool` (`:167`), the executor's own `_shutdown` (`:170`), the interpreter's global `_shutdown` (`:172-173`) — plus the two object constructions at `:175-176`, which fail only on `MemoryError`. Those are the refusals that genuinely queue nothing, and whenever the flag was not already set this design **leaks a booking** on them instead of releasing it. That is the intended trade: a leaked booking is P17-3 — minor, one participant, self-limited — while an orphaned live turn breaks measure 1a's invariant for everyone `_await_quiesce` serves. **On the three guarded raises the leak is smaller than P17-3's own statement of it**, which is derived rather than assumed: `_broken` is written in exactly one place, `_initializer_failed` (`:206-208`), reached only from `_worker` (`:77`) when an `initializer` raises, and this executor is constructed with `max_workers` and `thread_name_prefix` only (`Storefront.__init__`'s `ThreadPoolExecutor(...)` call, re-read in this revision — **cited by symbol, because v1.30's line-numbered citation of this same construction was already stale by the time Pass 21 read it**; line numbers into `falkorchat/storefront.py` rot under every commit that touches the file, while those into the pinned CPython `thread.py` do not), so `BrokenThreadPool` is unreachable here; and the two `_shutdown` raises (`:170`, `:172-173`) mean the pool is stopped or the interpreter is exiting, so on those two "`409`-locked for the life of the process" is a lock that outlives nothing. **The two allocations are the exception, and they are the expensive shape: they must not be rounded into the raises.** `:175-176` fails only on `MemoryError`, which leaves the pool **healthy** — measured on 3.12.3 in this revision by making `_WorkItem`'s construction raise on a warmed pool: the refusal put nothing on the queue, `_shutdown` and `_broken` both stayed `False`, the running worker stayed alive and a later submit completed. So that booking leaks on a process that goes on serving, and P17-3's full statement does apply to it — that participant `409`-refused for the life of the process, every reset-mine of theirs a `503 quiesce_timeout`. Accepted anyway, because it does not move the trade: at the instant the interpreter cannot allocate a `Future`, manufacturing an orphaned live turn instead is not the better failure, and the shape that would have to pay for the fix is the common one. **Marked as inference rather than observation:** I read `submit`'s three raise sites but did not enumerate every writer of the module-global `_shutdown` beyond `_python_exit`, so treat *the interpreter is exiting* as the reading of `:172-173` rather than a proved exhaustive one — it changes the size of the residue, never the direction of the trade. **`_turns_shutdown` is not S10's stop-intake flag and the two must not be merged into one attribute.** This one is executor lifecycle — *is the pool still accepting work* — set once, at shutdown, forever; S10's is a reset-all intake gate that goes up and comes back down. One attribute doing both makes reset-all refuse turns permanently. **What that refusal answers the client — ruled here because it was ruled nowhere and ships as a bare `500`.** Delivered, the flag guard raises a plain `RuntimeError` that no handler maps, so the participant receives `500`, `content-type: text/plain`, `Internal Server Error`: no envelope, no token, produced by no handler — which is why S8's gate cannot see it (its mechanism arms repository faults) and why C13 would have to render a response this plan *knows about* as an unhandled one (`falkor-chat/docs/test-reports/salesperson-ui-s9-report.md` D-2, TP-012b). **The lost turn is not reopened** — it stays the accepted price argued above. **The response shape is the defect, and it cannot be repaired by a completeness-table row**, which is the smaller change and loses on the key: the table is keyed on `(route, response)`, and `(POST /shop/api/messages, 5xx)` already has a second, genuinely different producer — any unmapped exception out of `services.post_message`, which is a bug and must keep surfacing as one (TP-011c). One row would span two meanings, which §5.3's licence forbids outright; and in that section a `5xx` row means *no handler by construction*, so a mapped refusal must not be given one. **Decided: the refusal is typed.** `enqueue_turn` raises a `RuntimeError` **subclass** — so the delivered `test_a_submit_refused_after_shutdown_releases_the_reservation` keeps asserting exactly what it asserts today — and the route maps it to **`503 turn_not_scheduled`** with the storefront envelope, declared in that route's own `responses={…}` for S8's declaration half. **The delivered code already assumed this mapping existed**: that test's own docstring says the `RuntimeError` propagates *"so the route's own `except` is what turns it into a response"*, and there is no such `except` (read in this revision at `falkor-chat/server/tests/test_storefront.py`, and the route body calls `shop.enqueue_turn` outside any `try`). **The report's own preference is accepted in substance and rejected in placement**: it recommends a typed `503`, which this row takes, but reads it as something *C9 can render*, and C9 must not — C9's one meaning is *nothing changed* and its action is a retry that is safe **because** nothing changed, both false here, the message being written already and a retry posting a second line. So the token takes its own rule, §5.3 **C14**, and C9 keeps the code with one carve-out keyed on the token. **Also considered and rejected: moving the flag read ahead of `services.post_message`**, which would make the refusal mean *nothing changed* honestly. It closes nothing on its own — the flag can still flip between that check and `enqueue_turn`, leaving the identical undeclared response on a narrower window, so the typing is owed either way — and it costs `reserve_turn` a second refusal signal (its `None` already means the `409`) plus this surface a second token, to improve a case v1.31 priced and accepted. **What the typing covers is one of `enqueue_turn`'s three refusal shapes, and the other two stay untyped — derived from `submit`, not asserted.** The subclass is raised only at the pre-`submit` flag read, so: **(1)** the flag already set when `enqueue_turn` read it ⇒ `503 turn_not_scheduled`, and the token is true because nothing was submitted; **(2)** `submit` itself refusing **before** the queue put — the executor's own `_shutdown` check precedes that put (`/usr/lib/python3.12/concurrent/futures/thread.py:164-180`, re-read in this revision), so a `shutdown_turns()` landing between this row's flag read and the very next statement raises a **plain** `RuntimeError`, executed rather than argued (`docs/reviews/salesperson-ui-impl.md` `## Pass 23`, Appendix S §3) and not caught by a subclass mapping; **(3)** `submit` refusing **after** the queue put — thread exhaustion — where the item runs anyway. **(2) and (3) reach the client as the same bare `5xx` and are C13's**, recorded in its residual paragraph. **Widening the mapping is the worse answer, and this row's own derivation is what says so**: mapping any `RuntimeError` out of `enqueue_turn` would put the token on shape (3), where the work item *is* queued and the turn *does* run, so the token would be a lie. **Also considered and rejected: reading the flag inside the `except` to type shape (2).** It is *semantically* sound — in that window `submit` raises before queueing, so *not scheduled* is true — but it reintroduces the flag-read-inside-`except` spelling two passes prohibited for the neighbouring decision (P20-1, the release), where the identical read is **wrong** for shape (3); a reader who carries the pattern one decision over reinstates that defect. **And the window it buys is two adjacent statements wide**: the flag read and the `submit` call sit next to each other, so the race needs the whole of `shutdown_turns()` — including an `executor.shutdown(wait=True)` that drains every running turn — to complete between them, on a process already stopping. One bare `5xx` rendered loudly by C13 is the proportionate answer. **§5.2's row and C14 are qualified to match**: stating the condition unqualified was P23-2, and it was this row's own argument against the alternative it rejected, left unapplied to the one it chose. **Carried by S9e**, the sub-unit already working at the response boundary on turn-layer raises; it lands with the reason strings rather than as a step of its own. **v1.32's obligation list named three sites and the delivered suite encodes this response in more, so S9e *derives* the list rather than inheriting a count** (P23-3, P23-5). The three that were named: the route's mapping, that route's `responses={…}` declaration, and clearing §5.3's pending-producer row. **Two commands find the rest, and it takes two because the sites use disjoint vocabularies** — `grep -n 'enqueue_turn' falkor-chat/server/tests/test_storefront_api.py` reaches every site that names the *raiser* in prose (39 hits when this was written, most of them the AST reader's own fixtures rather than this response: read them, do not count them), while the response's row is keyed on route and status in `TABLE` and that grep never touches it. What those two returned here, as the starting point rather than the answer — in `falkor-chat/server/tests/test_storefront_api.py`, **`TABLE`** (`:131`, keyed `("POST", f"{API_PREFIX}/messages")` → `(status, token)` pairs, so the edit is one tuple) and **`STOREFRONT_RAISES_TODAY`** (`:3939`, a frozenset of raised class *names* that today carries `"RuntimeError"`), both **self-enforcing** — their guards assert a name-set equality, so introducing the subclass reddens them and cannot be forgotten; **`NON_FAMILY_RAISES["RuntimeError"]`** (`:3995`), which is **not** — its key stays alive on the `services._dispatch_write` leg, so the entry's `enqueue_turn` half, which already concedes that refusal *"**is** request-reachable"* and records the bare `500` as measured, stays green while going false; S9e rewrites that half rather than deleting the key. And one that is not about this response at all: `presenter_reset_all`'s `except` comment in `falkor-chat/server/falkorchat/storefront_api.py` (~`:1494`) still describes the second-timeout `504` as carrying no roster, four lines above the `participants=unresolved` that makes the key present with `null` — the same false-absence sentence §4.8 was corrected for, one route over, and the copy an S10 implementer reads while moving this body. **Each booking carries its own token, and every map write names its booking and takes effect only if that booking still owns the slot** — the reserve's test-and-set, the **release**, the `thinking` flip and the `finally` clear alike, the release included because between a reservation and a failed write a `clear_all_turns()` plus a second-tab post can install a different booking in that slot and an unconditional release would delete it, which is the very defect the token exists to prevent on the one path a worker-only rule does not cover (`docs/reviews/salesperson-ui-impl.md` `## Pass 18`, P18-3). This matters because the map holds one slot per participant and `clear_all_turns()` empties it under workers that are still running (the intake window it opens is S10's, `docs/reviews/salesperson-ui-impl.md` `## Pass 17` Ruling 2), so an unconditional `finally` deletes a *later* booking's entry. **The two halves are independent and neither subsumes the other**: the reservation closes the admission window, the token closes the ownership one. That token is also the handle S9b's cancellation hangs its `Future` on — one map, not two (Pass 17, P17-9) — and, being the booking's arrival ordinal, it is what §5.2's queue position is derived from. **Because it does both jobs, the ordinal is a per-`Storefront`, strictly monotonic counter — never reset, never reused** (an `itertools.count()` advanced under the turn lock; per-`Storefront` and **not** module-level, because every other piece of this object's mutable state is per-instance and its class docstring says why — a second `Storefront` must share nothing with the first but the graph, which is what makes the restart-survival test mean anything): it orders the line *and* identifies the booking, and a collision corrupts both at once, mis-placing the line and letting an ownership check pass against a foreign booking, which is P17-1 again in a new spelling. **`len(self._turns)` is a plausible reading of *arrival ordinal* and is wrong for both jobs** — it restarts after `clear_all_turns()`, so two live bookings can hold the same value (`docs/reviews/salesperson-ui-impl.md` `## Pass 18`, P18-2). **Do not hold the turn lock across `executor.submit(...)`** in order to force booking order to equal submit order. **The rule rests on there being nothing to buy** — §5.2 defines the line by booking order, so an ordering nobody reads is not worth widening a lock over a call into a third-party executor. **It is deliberately not a deadlock claim** — an earlier spelling of this row said it was, and that was false: `_python_exit` takes `_global_shutdown_lock` only to set `_shutdown`, and joins every worker *outside* it (`/usr/lib/python3.12/concurrent/futures/thread.py:23–31`, pinned 3.12.3), as does `shutdown()` with its own lock, so no cycle can form — staged and measured as exiting cleanly (`docs/reviews/salesperson-ui-impl.md` `## Pass 18`, P18-1). **The exit-time fact that is true is a cost, not a hang:** `_python_exit` joins every worker unlocked, so a worker blocked on the turn lock delays process exit for exactly as long as the lock is held — an argument for holding it briefly. Beneath that, an application lock held across two `concurrent.futures` internals is a lock-ordering hazard whose present benignity is an implementation detail rather than a contract. **Do not delete this rule on finding a mechanism that does not hold — that check has been run.** Book, release the lock, submit; two turns booked microseconds apart may then reach workers in the other order, which §5.2's definition accommodates by construction. **Reversal trigger — narrow, because only one of this rule's legs is contingent:** the *nothing to buy* leg holds only while §5.2 defines the line by booking order, so if that ever changes — if the position must track the order in which turns actually **start** — revisit this rule first. **The answer will still not be holding the lock across `submit`**, which buys start-order fidelity only by putting an application lock underneath `concurrent.futures`' internals; it will be a change to *how work reaches the pool* — a single ordered dispatcher, or a pool whose queue is keyed on the ordinal — both larger than this row and to be priced then rather than assumed now. Nothing else reopens it: the exit-cost and lock-ordering legs do not depend on §5.2 at all. Neither `reserve_turn` nor `release_turn` reaches `Services`, so nothing in this clause moves the reach the guards' statement pins (cited below). **And the half §4.8 asked for that S7 could not build: cancellation of a *queued* turn runs in front of `_await_quiesce`, never in place of it.** S7 shipped the wait alone and that is correct for the *result* — a queued turn reaches a worker, completes and clears its own entry, so waiting subsumes cancelling and differs only in latency — but not for **availability**: `FALKORCHAT_STOREFRONT_QUIESCE_S` is 30 s against a 180 s agent timeout, so a turn that outlives the budget turns reset-mine into a `503` with nothing reset, exactly where dropping the queued work would have let it succeed (§4.8; `docs/reviews/salesperson-ui-impl.md` `## Pass 7`, Ruling 3). Cancel means the queued `Future` is actually cancelled and *then* its map entry cleared; a future already running cannot be cancelled and **falls through to the existing wait** — S7's refusal to clear the turn-map entry as a stand-in stands, because that reports idle while the job is still queued and lets the delete race the turn quiesce exists to prevent. **And one decision this row makes rather than leaving to the dispatch brief: the per-participant record cache goes.** S6 built `Storefront._records` as a read-through cache with `lookup` as its reader, and the S7 gate found **zero production callers** — all eight `.lookup(` call sites are in `tests/` (Appendix K §6) — while `cached_ids()` is diagnostics. Deleting `lookup` alone would leave `resolve_token`'s and `reset_participant`'s `_cache_put`s writing into a map nobody reads, which is worse than either end state, so this is **one decision, not two**. **Decided: remove.** S8's tripwire forbids the router reading it (`storefront_api.py` never calls `.lookup(` — the router authenticates against the graph, not the cache), and this row's own `enqueue_turn` **receives** the `ParticipantRecord` from the request thread that already called `resolve_token`, so the worker never needs to resolve one either. With no reader available to any caller, the cache is dead code: S9 deletes `lookup`, `_records`, `cached_ids()` and every `_cache_put`/`_cache_drop` in one change, together with the tests that exist only to exercise them. **Consequence to bank rather than re-solve:** the S7 gate's **S7-2** (the two unpinned error-path `_cache_drop`s) is *dissolved* by this, not fixed — spend no tests on it. **Reversal trigger:** if the implementation turns up a real reader off the request thread — any path needing a `ParticipantRecord` it was not handed — keep `_records` **and** `lookup` together and pin both error-path evictions with tests (S7-2 names them); never keep one without the other. | `falkor-chat/server/falkorchat/storefront.py`, `.../storefront_api.py`, `.../app.py`, `falkor-chat/server/tests/test_storefront.py`, `.../test_storefront_api.py` | `Storefront.reserve_turn(participant_id)` → the booking, or `None` (which **is** the `409`); `Storefront.release_turn(participant_id, booking)`; `Storefront.enqueue_turn(ctx, participant, posted, booking)`; `turn: {state, queuePosition, lastTurn}` on `GET /shop/api/state`, `queuePosition` **composed in `get_state`** (§5.2 *The queue position*) | With `turn_workers=1` and a stub 2 s LLM, three *different* participants' posts complete in order and — sampled once the first is observed `thinking`, which the gated stub makes deterministic — report `thinking`/`0`, `queued`/`0`, `queued`/`1`, **the third falling to `0` when the first completes** — that countdown is the half a position fixed at booking cannot pass; at `turn_workers=4` a fifth arrival behind four *running* turns reports `queued`/**`0`**, not `4` (§5.2 *The queue position*); **two posts 100 ms apart from one participant produce exactly one `WorkflowRun` on that thread**, the second returning `409` with no `Message` written, **and so do two posts held concurrently *inside* the write** — two threads driving one `TestClient`, the second request entering while the first is blocked in `services.post_message` on a gate the test controls (the shape is verified as implementable, `docs/reviews/salesperson-ui-impl.md` `## Pass 18` Appendix O §1, and the suite has no precedent for it), which is the only spelling that tells a reservation from a check-then-act (a sleep-timed pair passes on both, and the check-then-act answers that second post `200`); **a booking is cleared only by its own worker** — with a turn live, a `clear_all_turns()` followed by a fresh accepted post leaves the new entry surviving the old worker's `finally`, `turn_in_flight` still `True`, **and that same case pins the ordinal**: the fresh booking's ordinal is strictly greater than the wiped one's, which a counter derived from the map's size fails and which is what makes the ownership check above meaningful rather than accidental; **the slot is released exactly where nothing was queued, and held where something may have been** — `services.post_message` raising leaves that participant postable (no `409`) with `turn.state` `idle`, which is what §5.3's `504 post_state_unknown` reconciliation reads; a post after `shutdown_turns()` leaves no entry behind and the `RuntimeError` still propagates (the delivered `test_a_submit_refused_after_shutdown_releases_the_reservation` already spells this, and under the pre-submit flag it passes without reaching `submit`) **— and the same post driven through `TestClient` answers `503` with `{"error": "turn_not_scheduled"}` and a JSON content type, never a bare `500`**, the `content-type` being asserted alongside the status because it is the `text/plain` body that identifies the un-typed escape rather than the code (S9e; §5.3 C14); **and a `submit` that raises *after* it was entered leaves the booking standing** — with `threading.Thread.start` patched to raise while a worker is busy, `enqueue_turn` propagates the `RuntimeError`, `turn_in_flight` is still **`True`** at that instant, and the item that got queued anyway runs and clears its own booking in `_run_turn`'s `finally` (poll until `turn_in_flight` is `False`, so the case pins both halves rather than only the non-release). **That case is this row's own** — it is neither S10's nor the dead-turn latch's, and it is the assertion that goes red on any implementation releasing from a bare `except` around `submit` (`docs/reviews/salesperson-ui-impl.md` `## Pass 20`, P20-1 and its open question 2); **the turn runs on the worker and never on the request thread** — `POST /shop/api/messages` against a 2.0 s stub turn answers `200` in the low milliseconds, two orders of magnitude under the turn, and the turn is observed running on a `storefront-turn_*` thread rather than the request's (QA drove exactly this shape at TP-028's neighbour TP-026; it is named here because it is this row's central decision and, until v1.33, **nothing in this done-condition asserted it directly** — the queue-position cases require it as a precondition and prove it only by implication); **poll latency under saturation is bounded on two statistics, and each names the design it can redden** — with the pool saturated (every one of `turn_workers` workers running a stub turn *and* at least `2 × turn_workers` further turns queued behind them, **all of them still unfinished when the last sample is taken**, which the test asserts rather than assumes, so a run whose turns drained mid-sample fails as *not measured* instead of passing on a laundered median), over at least 20 sequential `GET /shop/api/state` samples from one client: the **median** must be **no more than 3× the idle median** taken in the same run and **under 50 ms**, **and no single sample may exceed 100 ms** — all three figures printed by the test. *The median pair reddens **proportional** coupling: work on the poll path whose cost grows with queue depth, which is not hypothetical here — §5.2's queue position is **derived on every read** by a scan of the turn map, the one thing on this path whose cost varies with the queue. The ratio catches that on a fast box, the absolute on a slow one. The **max** reddens the shape a median structurally cannot see: a poll blocking on the turn lock while a worker holds it — measured on a harness of exactly this shape at a median of 4.18 ms, inside every median clause, against a single sample of **14 900 ms** (`docs/reviews/salesperson-ui-impl.md` `## Pass 23`, P23-1, Appendix S §1).* **The max is absolute rather than relative to the idle median, and that is a rejection of half of Pass 23's proposed clause.** It proposed no sample above 3× idle **or** above 100 ms; at an idle median of 3–4 ms the relative half bounds one sample at ~12 ms, which ordinary GC and scheduler noise reaches on a shared box, so it would redden healthy runs — and a flaky gate is a gate somebody switches off. Nothing is lost by dropping it: 100 ms is ~15× the healthy maximum that harness measured (6.6 ms) and ~1/150 of the broken one. **What this bound does not redden, stated rather than claimed away — the turn on the request thread.** v1.32 named that design as a target and was wrong: measured at the prescribed saturation it reads ratio **1.00×** at 4.16 ms, and still passes at twenty times that saturation, because sync `def` routes ride anyio's thread limiter at `THREAD_LIMIT` (100) and `3 × turn_workers` never approaches it (P23-1, Appendix S §1). The assertion that forbids that design is the first clause of this done-condition, added in the same revision precisely so this narrowing does not point at a hole. *A bound naming a target its statistic cannot hit is the reach-exceeds-mechanism defect this plan narrowed S8's assertion to avoid — committed one row over, in the fix for it, which is why the clause now says what each statistic catches and what none of them does.* Headroom as measured on the delivered system: **3.3 ms idle against 3.3 ms saturated**, max 3.6 ms, at `turn_workers=2` with 2 running and 6 queued (`falkor-chat/docs/test-reports/salesperson-ui-s9-report.md` TP-028). §6.4's p95 under 300 ms at 50 participants is a different instrument at a different altitude — S15's load harness, not this row — and the two bounds do not compete; executor drains on shutdown; **cancellation is asserted by the case that separates it from waiting** — with `turn_workers=1`, A's stub turn in flight for 2 s and B's turn queued behind it, B's reset-mine at `quiesce_s=0.5` returns **`200`** and B's queued turn never runs (no `WorkflowRun` on B's fresh thread, no reply written), where the wait-only implementation returns `503`; **and no record cache survives** — `Storefront` defines no `lookup`, `_records` or `cached_ids`, `resolve_token` is its sole participant-resolution path, and the eight `tests/` call sites go with them, the suite green without replacements — **S8's `never calls .lookup(` tripwire goes vacuous with the symbol, and that is the correct end state**: it did its work at S8's gate, and with no cache to read the rule it enforced (*the router authenticates against the graph*) holds structurally rather than by assertion; **S8c's `services.` access assertion stays *green* on this step, and staying green is a done-condition rather than a formality — green at the reach that guard actually has.** That reach is stated authoritatively, and finally, in the code itself: the comment block above `INHERITED_HANDLERS` in `falkor-chat/server/falkorchat/storefront_api.py`, and in `falkor-chat/server/tests/test_storefront_api.py` the comment above `SERVICE_LAYER_REACH_TODAY` plus the docstrings of the two guards it governs — `test_the_routers_service_layer_reach_is_exactly_what_the_exemptions_assume` and `test_the_raises_a_route_can_reach_are_exactly_what_the_exemptions_assume` — and of the AST readers beneath them (`_alias_prefixes`, `_bind_pairs`, `_service_layer_reach`, `_reached_methods`, `_raises_of`). **Read that statement; do not re-derive it here** — it names the four scopes it walks and every shape at which it stops, and it settles this step: under the placement decided above the reach does not move, `trigger.maybe_trigger`'s own `start_workflow_run` call site (`trigger.py:82`) being outside all four scopes, so re-deriving `SERVICE_LAYER_REACH_TODAY` is a **no-op at S9**. What the assertion pins is the storefront's **service surface**: red means a `Services` call was acquired through the storefront's own `self._services` rather than through the trigger — a **stop-and-re-decide, not a mechanical bump**. It is **not** evidence of *placement*; placement is evidenced by the armed-fault `200` and the queue-position assertions in this row's done-condition. **Do not restore a reddens-at-S9 claim** (this row carried one until v1.25, §5.3's preamble until v1.26): the reach genuinely does not move, the cited statement says so in both files, and a done-condition asserting otherwise is unmeetable. **What does move is the truth of three excuses, and no walk in either guard can see any of it** (same citation). After this step a storefront route does reach the workflow layer, so the `INHERITED_HANDLERS` reason strings of `WorkflowEngineDisabledError`, `WorkflowInputRejectedError` and `WorkflowRunNotFoundError` — form: *"no storefront route calls that layer"* — are **false as written** while remaining invisible to both guards. So all three take the measured exemption this row already promises above, measured at the **response boundary** instead of at the raise, and **S9 replaces each one's reason string** with the reason that is true of it (*reached only on the turn worker, after the `200` has been sent*) **and cites it to the armed-fault test below**. A reason string re-worded with nothing behind it is P11-1 a second time; that standard is unchanged — only its mechanism moves. **The per-exception counterfactual is deliberately not carried here:** which of the three a walk would have reported under the *rejected* spelling is a fact about a spelling this row rejects, the guard files decline to state it on purpose, and v1.24's version of it has since been falsified — P14-1's `Services`-sibling closure landed, so `_reached_methods` now closes a seeded `start_workflow_run` over `self._require_executor` and that walk would report all three, not the two v1.24 named. **`WorkflowDefNotFoundError` is not one of the three and takes no re-derivation:** its only raise sites are `materialize_def` (`services.py:1752`), `get_workflow_def_structure` (`:1814`) and `diff_def_snapshot` (`:1869`), none of them reachable from the trigger's calls, so its excuse (*no storefront route reads a def*) stays true as written. **And because a behavioural test enumerates where the walk closed, S9 derives that enumeration rather than copying it:** the worker's whole service surface is `trigger.maybe_trigger`'s own calls — `find_waiting_run_for_thread` (`trigger.py:76`, raises nothing), `resume_workflow_run` (`:78`) and `start_workflow_run` (`:82`) — plus the responder fall-through, and the workflow classes reachable through them are exactly the three above (`_require_executor` on **both** run paths, `services.py:1996`; the `run_ctx` size bound, `:2056`; the absent start anchor, `:2085`). If the trigger's call set has moved by the time S9 runs, the enumeration has moved with it. **`executor.run`/`executor.resume` raise `WorkflowRunNotFoundError` of their own** (`executor.py:503`/`:531`/`:595`/`:1366`), past `_drive_or_fault`'s four-type net (`services.py:2287`) and outside every scope either guard walks. They earn **no** separate exemption and no fourth armed fault: they are the same placement claim one layer down, covered by the same `200` and made observable by the dead-turn latch — which is the half that matters, because `lastTurn: 'failed'` is the participant's only evidence that an executor-side death happened at all. Under this row's placement decision the expected outcome is *exemption*, and the measurement is what proves it: with the fault armed (executor unwired; a `run_ctx` over `MAX_CONFIG_LEN`; the snapshot absent) `POST /shop/api/messages` still answers its ordinary `200` and the failure appears only in the log and the turn state — the same shape as `test_only_post_messages_can_raise_a_service_error`, and red if any of the three ever reaches the response; **and a turn that dies on the worker is observably distinct from one that completed** — asserted with a stub trigger that raises, driving the full post → poll cycle so the assertion is on **what the participant would see**, not merely that nothing hangs: `GET /shop/api/state` reports `turn.state == "idle"` **and** `turn.lastTurn == "failed"` in the same body, the silent `finally: clear_turn` implementation therefore failing on the second half. **Its lifecycle is asserted, not just its latch** — a participant whose latch is set can still post (`in_flight` untouched, no `409`), the next **accepted** post clears it while a `409`-refused post does not, and either reset clears it; **and the latch survives `set_turn_state(idle)`**, which is the delivered call that deletes the `_turns` entry and would otherwise take the signal with it (the test that goes red if the latch is stored in that entry) | `coder` | **after S8** |
| **S10** | **Presenter surface — a *move*, not a build: S8 already delivered all three routes.** *Whoever picks this step up will find `presenter/session`, `presenter/participants` and `presenter/reset-all` already answering in `storefront_api.py`, and must not re-implement them. S8's `{handlers} × {routes}` gate is evaluated over **all eleven** routes and cannot be run on a partial surface, **and as of v1.32 it cannot be run clean at all until S9e lands**: §5.3's completeness table carries one row — `POST /shop/api/messages` · `503 turn_not_scheduled` — whose producer is deliberately pending, so transcribing the table into the suite's `TABLE` ahead of S9e reddens the gate's symmetric half (*a row with no producer*) on exactly that row. S10 is the next step that re-runs the gate, which is why the warning is here as well as on the row (`docs/reviews/salesperson-ui-impl.md` `## Pass 23`, P23-7), and S8's own done-condition names `POST /shop/api/presenter/session` answering its `200`/`403`/`422` by name — so the three were built there, against the delivered core and **without touching `storefront.py`**, which is byte-identical through S8b: that untouched file is the line the scope fork was held at (`docs/reviews/salesperson-ui-impl.md` `## Pass 10`, ruling 1). **The inventory of exactly what S10 takes with it lives in the source, as `_STEP_10_INTERIM` (`falkor-chat/server/falkorchat/storefront_api.py`), and is the authority for the move**: `_PresenterSessions` + the `presenter/session` body → `Storefront.presenter_login`; `presenter/participants`' `repo.list_participants` call → `Storefront.list_participants`; the `presenter/reset-all` body → `Storefront.reset_all`; and **two of the four** private reads in `build_storefront_router` go with them — `repo` and `presenter_key` — while **`services` and `agent_id` stay**, because neither S9's row nor this one moves `read_messages` or `get_current_order` onto `Storefront` (same pass, P10-6). **What S8 deliberately did not build, and this row therefore still owns:** the login's fixed per-attempt delay, the observational attempt counter, and reset-everyone's **stop-intake** flag — which is `Storefront` state, i.e. not S8's file. This is the exact parallel of S7 shipping reset-mine's wait while S9 ships its cancellation. **Several done-conditions below are consequently already green against S8's code** — `presenter_configured`, the `presenter_login` constant-time tripwire pair, F8's both orderings (including the re-read that also times out), and the roster/`incomplete` assertions: after the move they are **re-pointed at `Storefront`, not re-derived**, and one going red means the move lost something.* The three routes and their contracts: `POST /shop/api/presenter/session` (key → token, **entirely in-process: it reads `config.STOREFRONT_PRESENTER_KEY` (`FALKORCHAT_STOREFRONT_PRESENTER_KEY` — S6's delivered spelling, §4.3) and mints a token without touching the graph, which is what puts it in §5.3's `no graph access` class**; **an unset key must never authenticate** — `hmac.compare_digest("", "")` is `True`, so the route checks S6's delivered `Storefront.presenter_configured` *before* comparing and answers `403` when no key is configured (`falkor-chat/docs/SERVER.md` §1.3); rate-limited: **a fixed per-attempt delay, plus an attempt counter that is deliberately *observational only*** — it is logged and exposed to the operator so a brute-force attempt is visible, and it **never changes the response**, so the codes this route can answer stay `200` / `403` / `422` / the cross-cutting three. **Decided rather than left open (Pass 7, P7-5):** the alternative — a lockout after N attempts — is **rejected**, because there is exactly one shared presenter key (§4.3), so a lockout is a self-DoS: anyone on the LAN could lock the presenter out of their own demo mid-show, which is worse than the brute-force risk it mitigates, given that the fixed delay already bounds the attempt rate and R6 already accepts the standing key as a residual. It would also add an unlisted response (`429`/`423`, or a `403` that silently changes meaning from *wrong key* to *throttled*) that C2's second half would render as "your key is wrong" when the key may be right. **Reversal trigger:** if this is ever exposed beyond a controlled LAN, the answer is not a lockout but K-016), `GET /shop/api/presenter/participants` (roster filtered on `User.tokenHash IS NOT NULL`, so `config.USER_ID`'s lifespan node never appears; **it returns §5.2's four keys and nothing more** — the delivered `list_participants` (`falkor-chat/docs/QUERIES.md` §18.3) projects no activity data, and composing `messageCount`/`cartTotal`/`orderStatus` per participant would be ~150 extra graph queries per presenter poll at 50 participants, outside R10's budget and unprofiled by S0's §8. **Reversal trigger:** if the demo genuinely needs activity stats, they are **one aggregate roster query** designed by `graph-dba` as a §18.3 append, never a per-participant fan-out), `POST /shop/api/presenter/reset-all` with §4.8's stop-intake-then-drain quiesce. | `falkor-chat/server/falkorchat/storefront.py`, `.../storefront_api.py`, `falkor-chat/server/tests/test_storefront_api.py` | `presenter_login(key) -> token`, `list_participants()`, `reset_all()` | A participant token is refused on every presenter route; a wrong key is refused and counted; **an *unset* presenter key authenticates nobody: `presenter_configured` is checked *before* any `compare_digest`, asserted with a `presenter_key=""` storefront answering `403` to an empty submitted key** (`docs/reviews/salesperson-ui-impl.md` `## Pass 6`, S6-2 — the guard S6 delivered and tested is useless unless this row tells S10 to call it); **S6's constant-time tripwire pair is extended to `presenter_login`** — a `hmac.compare_digest` spy asserting exactly one call with the expected arguments, plus `assert "==" not in body` (same pass, S6-4: the shipped tripwire inspects `resolve_token` only, and `presenter_login` is the other `compare_digest` site); `reset-all` invalidates every participant token **but not the presenter's**, and clears the presenter's own conversation too; **the same §7 (a)–(d) quiesce conditions as S7**, applied to `reset_all`'s stop-intake-then-drain path; roster excludes non-participant `User`s **and its field set is exactly §5.2's four keys** — asserted, not assumed; **`unscopedCount == 0` returns no `incomplete` field at all**, not `incomplete: false`; **F8, both orderings** (§4.8): a FalkorDB socket `TimeoutError` returns **`504 reset_state_unknown`** after re-reading state, never the quiesce `503` — **and a stub whose re-read *also* raises `TimeoutError` still returns `504` carrying **`"participants": null`** — this route's evidence block is the re-read roster, not `state`, and the key is present with a null value (§4.8 F8, §5.2 *Absent versus null on the wire*) — never a `500`**; **§5.2's anomaly contract holds in full** | `coder` | **after S9** |
...
**A fourth omission, licensed here rather than repeated per route: the credential refusals.** A
`401` on any of the six routes carrying a participant credential (`/state`, `GET` and
`POST /messages`, `/catalog`, `/order/advance`, `/reset`), and a `401` or
`403 wrong_credential_type` on the two authenticated presenter routes, are produced by the
`get_participant`/`get_presenter` **dependencies** — not by any route's body — so they are uniform
across every route that carries a credential and are ruled once, per credential, by §5.3 **C3** and
**C2**. §5.3's per-route table carries each of them as its own row; repeating them here would be
ten prose copies of two dependencies' uniform behaviour, across eight rows, with ten ways to drift (P11-5's lesser half — the
choice recorded rather than the omission left implicit). **Three routes are outside the licence
because they carry no credential**: `GET /shop/api/health`, `POST /shop/api/session` and
`POST /shop/api/presenter/session` — the last of which *mints* the presenter credential, so its
`403 bad key` is its own body's answer and **is** listed in its row. And the `401` named in
`POST /shop/api/reset`'s row is **not** a duplicate of the licensed one: it is the zero-rows /
already-deleted-participant case, which is content rather than the dependency's uniform refusal.

| Route | Body / query | Returns |
|---|---|---|
| `GET /shop/api/health` | — | `{status, storefrontEnabled, locales}` |
| `POST /shop/api/session` | `{displayName ≤ 60, language ∈ locales}` | `{participantId, token, displayName, language, welcome}` — **`welcome` is specified under *The join greeting* below** · **`422`** on a bounds/enum violation (§5.3 C11) · **`503 demo_not_seeded`** when the demo `Agent` is absent from the workspace — `ensure_participant` wrote **nothing at all**, so this is a *nothing changed* `503` (§5.3 C9) naming `./scripts/seed_demo.sh`, and **join is the only route that raises `DemoNotSeededError`**: it comes from `Storefront.join` and from nowhere else (`falkor-chat/server/falkorchat/storefront.py`), which no other route calls. **The *token* is not join's alone, and v1.20's "the only route that can produce it" was wrong about that** — `POST /shop/api/messages` answers the same `503 demo_not_seeded` from a different exception (`UnknownMemberError`: the same absent demo `Agent`, met one route over as an unresolvable mention), re-shaped by S8b's `ServiceError` handler. Same condition, same token, same C9 rule; see §5.3's row. **It is not unreachable and must not be left to escape as a bare `500`** — §4.9's readiness preflight asks the same question at *boot*, which bounds the mis-seeded-deployment case and not the graph being altered out of band afterwards (the same posture as `409 unscoped_participant`: a graph can be unhealthy in ways the storefront did not cause) · **not idempotent** — a lost response can leave a ghost participant whose token nobody holds (§5.3 C4's join case, R12) |
| `GET /shop/api/state` | — | `{profile:{name,deliveryAddress}, cart:{items[],total}, order:{orderId,status,lines[],total}\|null, turn:{state,queuePosition,lastTurn}}` — **`queuePosition` is specified under *The queue position* and `lastTurn` under *The dead-turn signal*, both below** |
...
| `POST /shop/api/messages` | `{text ≤ 2000}` | the posted row · **`409 TurnInProgress`, nothing written**, when that participant already has a turn in flight · **`422`** when `text` exceeds the bound (§5.3 C11) · **`503 demo_not_seeded`** when the demo `Agent` named in `mentions=[agentId]` is gone — `services._validate_and_derive_role` raises `UnknownMemberError` **before any write**, so this is the same *nothing changed* `503` as join's row, met one route over and re-shaped from the `ServiceError` family by S8b's handler rather than raised by this body (§5.3 C9) · **`503 turn_not_scheduled`** when the post lands inside the shutdown window **with the flag already set as `enqueue_turn` reads it** — the qualification is load-bearing and §5.1's S9 row derives it: `shutdown_turns()` has set the pool's lifecycle flag, so `enqueue_turn` refuses **after** `services.post_message` has written the message — the line is in the transcript, no reply will ever come, and this is the one `503` on this surface that does **not** mean *nothing changed* (§5.3 **C14**; §5.1's S9 row owns the trade and the mapping obligation) |
...
**The queue position (`turn.queuePosition`).** A non-negative integer, **always present**,
alongside `state` and `lastTurn`. It means one thing: **the participant's 0-based index in the
waiting line** — equivalently, how many *other* accepted turns are ahead of theirs in that line. It
is **`0` whenever `state` is `idle` or `thinking`**, where it is a constant rather than a position:
an idle participant is in no line, and a running turn has already left it. **`0` on a `queued` turn
is ordinary and load-bearing** — *first in line, waiting for a worker to free up* — so S13 renders
it as a queue, never as the absence of one. **The line is ordered by booking, and that is the
definition rather than a tie-break**: the executor may start two turns booked microseconds apart in
the other order (§5.1's S9 row concedes exactly that), which is not a divergence here, because this
contract is about the line and not about start order.

**Derived on every read, never stored.** `GET /shop/api/state` composes it from the turn map: the
count of entries that are still `queued` and were accepted **earlier** than this one, the map
holding each booking's arrival ordinal (§5.1's S9 row) rather than a number. Two properties follow,
and both are the point. It is **correct at every `turn_workers`** — a `thinking` turn occupies a
worker rather than a place in line, so it is not counted — and it **counts down** as the queue
drains, which is the whole difference between a queue position and an indefinite spinner wearing a
number (§4.4 measure 1). *A position taken once at booking has neither property*: S9a stored
`len(self._turns)` at booking, which at the **delivered default** `turn_workers=4` told a fifth
arrival `queuePosition: 4` while it was first in line, and never moved until that turn started
(`docs/reviews/salesperson-ui-impl.md` `## Pass 17`, P17-2, reproduced). Deriving it costs one scan
of a map bounded by the participant count, once per poll — nothing at this scale, and the only
spelling in which the number cannot go stale.

**What it deliberately is not.** Not an ETA: turn duration is R1's variable and the server has no
honest estimate to publish, so the API gives a place in line and S12c's bundles turn it into words.
Not a count of everything in flight: running turns are excluded, which is exactly why `turn_workers`
need not appear on the wire — a deployment-sizing number is not a shopper's business.

**One bound, stated rather than discovered.** `presenter_reset_all`'s `clear_all_turns()` empties
the map while its workers are still running (S10 owns the stop-intake flag that narrows this;
`docs/reviews/salesperson-ui-impl.md` `## Pass 17`, Ruling 2), so for at most one turn's duration
afterwards a fresh arrival can read `queued`/`0` while every worker is still busy. That is an
under-count, never an over-count, and it self-corrects.

**The same window has a second consequence, and it is accepted rather than overlooked.** Those
running turns have had their bookings wiped, so `turn_in_flight` under-reports them too and
`_await_quiesce` can return `True` with one of them live — P17-1's invariant, reached through
reset-all instead of through a double post (`docs/reviews/salesperson-ui-impl.md` `## Pass 18`,
P18-5). It is accepted here because reset-all has just deleted the graph those turns would write
into, so their remaining writes are silent no-ops by the quiesce ordering
(`docs/plans/salesperson-ui-graph.md` §7.3), and because S10's stop-intake flag narrows the intake
half of it. §5.1's per-booking token is what keeps both consequences merely optimistic rather than
corrupting: those workers clear nothing that is not theirs.

**`turn_in_flight` can deviate in two directions, and this plan is deliberately not neutral
between them.** The window above is an **under**-report — a turn is live and the map does not say
so — accepted only because reset-all has just deleted the graph those turns write into. An
**over**-report is the opposite: a booking standing with no turn behind it, which costs that one
participant a `409` until it clears and costs everyone else a place in the line. **Wherever the
two cannot be told apart, §5.1's S9 row takes the over-report**, and its one instance is a
`submit` that refuses *after* it has queued its work item — releasing the booking there would
manufacture exactly the under-report `_await_quiesce` exists to rule out
(`docs/reviews/salesperson-ui-impl.md` `## Pass 20`, P20-1). **What the asymmetry costs is derived
once, in the S9 row, and is deliberately not summarised here** — §5.1's *cite, don't re-list* rule
(v1.7), applied to a cost rather than to a list. All this section needs from it is the direction,
above; the row is where to read *which* refusals leak a booking, on what kind of pool, and for how
long. *(Tombstone, 2026-09-08 — v1.31: v1.30 closed this paragraph with a one-sentence précis of
that residue, to the effect that in normal operation the asymmetry costs nothing. The direction it
argues for is unchanged and correct; the précis was not — it generalised over refusal shapes and
over pool states that the S9 row's own derivation excludes, and both exclusions are now stated
explicitly there (`docs/reviews/salesperson-ui-impl.md` `## Pass 21`, P21-4). The repair is
structural rather than a better sentence: a summary that has to stay true to a derivation one
section away will drift again, so this section now keeps no second copy of it to keep true.)*

**The dead-turn signal (`turn.lastTurn`).** `'failed' | null`, **always present**, alongside
`state` and `queuePosition`. It exists because S9's trigger call runs on the turn-queue worker
(§5.1 S9) under the platform's failure-isolated contract — logged, never propagated — so a turn
that dies produces no response at all: without this field the participant sees their message, no
reply, and a composer that quietly re-enables, which is indistinguishable from a completed turn.
The dominant cause is not exotic; it is **any LLM failure** (endpoint down, the 180 s agent
timeout), which R1 says to expect.

**It is not a fourth `state`, and this is the whole point of the shape.** `state` keeps
`idle`/`queued`/`thinking`; `Storefront.turn_state`'s `in_flight` stays `state != idle`
(`falkor-chat/server/falkorchat/storefront.py`, delivered), so §4.4 measure 1a's `409` gate is
untouched and a participant whose last turn failed is **postable-to** — which is exactly the
...
transcript the notice refers to is gone (§4.8). *Process-local*, like the turn map itself and for
the same delivered reason: a restart drops it, which is correct, since the worker that was driving
the turn is gone too.

**One field, not a report.** It says *the last turn did not produce a reply*, and nothing about
why — the reason is a log line for the operator, not a participant-facing string, and widening it
to carry an error class would put a Python-shaped value in front of a business audience for no
recovery benefit. The recovery is identical in every case: send again.

**Absent versus null on the wire — one rule, derived rather than decided field by field.** Both
shapes are in use above and the difference between them is contractual on this surface, so the
discriminator is stated once, here:

> **A key is omitted only where its absence — alone, or as one member of a group of keys that
> appear and disappear together — distinguishes two otherwise identical responses. Everywhere else
> an optional value is carried as the key, with `null`. The co-varying group is the unit: its keys
> are omitted together and present together, never mixed.**

The first case is reset-all's `incomplete`/`unresolved`, and it is a **pair** — which is why the
rule is stated over groups and not over keys. Both are written under one condition, and the two
keys' joint absence is the shape difference between a clean `200` and a partial one; the argument is
§5.3 **C6a**'s, the delivered pair is stated verbatim in §5.2's reset-all row above, and that shape
difference is why the pair takes a completeness-table row of its own. Read per key — the shape
difference attributed to one of them rather than to the pair — this rule would have made
`unresolved` `null` beside `incomplete`'s absence — a third shape nothing rules and the response does not produce
(`docs/reviews/salesperson-ui-impl.md` `## Pass 23`, P23-6: the fifth optional field, found inside
this rule's own first case). Everything else optional on this surface is the second case — `order` on
`GET /shop/api/state`, `turn.lastTurn`, `profile.deliveryAddress`, a catalog row's `imageUrl` and
F8's per-route evidence block on the `504` (§4.8) among them — and the shared reason is that none
of them distinguishes a *response*: they are content, and content whose shape varies forces every
client into a presence check no rule covers. **Apply the test, do not consult that list**, which is
illustrative and will age: ask whether the field's absence — or its group's — picks out a
different response. Yes ⇒ omit the whole group, and it earns a completeness-table row, because it
*is* a second response. No ⇒ `null`.
...
**The rules — C1…C14.** S12a's done-conditions cite these by number. **C13 is the one that does not enumerate** — it is the runtime guard against a `(route, response)` no rule covers. It is *not* a guard on whether the other thirteen are **right**: see C13's own residual paragraph.
...
- **C4 — after a `504`, re-read from the endpoint the failed path's *surviving* credential can
  ...
  **And the re-read is the report — never the `504`'s own body.** Both reset `504`s carry an
  evidence block (§4.8: `state` on reset-mine, `participants` on reset-all) that is **present with
  a `null` value** when the server's own re-read failed too, so a client branching on the key's
  presence renders an absent block as a real one. Branch on the value, or ignore the block and use
  the re-read this rule already asks for.
  ...
  *domain* — the cross-cutting `504` row, which said "every route that writes" — without extending
  ...
  - after **`POST /shop/api/messages`** → **`GET /shop/api/messages` *and* `GET /shop/api/state`, and
    ...
    state is **message written, turn never scheduled** — §5.1's S9 row reserves the turn slot
    *ahead* of the write and **releases it on exactly this failure**, which is what leaves
    `turn.state` `idle` here rather than a `queued` turn nobody will ever run. **That release is a
    repair, not an improvement:** the rule below decides correctly today, with no reservation at
    all, and a reservation that survived its failed write would not blur it but **invert** it —
    `turn.state !== 'idle'` sends this client into *wait, as normal* forever
    (`docs/reviews/salesperson-ui-impl.md` `## Pass 18`, question 2), which is why reserve and
    release are one change. Re-reading only `/messages` shows the message
...
- **C6a — `409 TurnInProgress` retains the composer text** and re-enables send when `turn.state`
  ...
  **And `turn.lastTurn === 'failed'` shows a notice — it does *not* gate the composer.** The
  composer re-enables on `state === 'idle'` exactly as it already would; `lastTurn` is read
  independently and renders "the reply never arrived — send again". Two things it must not do:
  **not** hold send disabled (the participant would be locked out of the only recovery the API
  offers), and **not** auto-resend (C12's no-retry rule covers the browser layer, and a silent
  resend duplicates the transcript line for a participant who never asked).
  **The neighbour to stay consistent with is C4**, which already rules that a lost turn must *say
  so* rather than leave the participant waiting — in the `504` case, reconciled from `/messages`
  plus `/state`. This is the same obligation on the other path: there, the turn was lost with a
  response to key on; here, it was lost after a `200` and the poll is the only carrier. A client
  that implements C4's reconciliation and ignores `lastTurn` has covered the rarer half.
  **`lastTurn` takes no row in the completeness table, and that is a decision rather than an
  oversight.** The table keys on `(route, response)`; `lastTurn` is always present on a `200`
  `/state` body, so it distinguishes no response — it is state content, exactly like `state`'s own
  `queued`/`thinking` values, which have never had rows either. It is **not** the `incomplete`
  case: reset-all's `incomplete` earns a row because its *absence* is load-bearing (§5.2:
  `unscopedCount == 0` returns no `incomplete` field **at all**, never `incomplete: false`), which
  is a shape difference between two `200`s. `lastTurn: null` is not a shape difference. Adding a
  row would also have put a pair in the table that today's app cannot produce, which is precisely
  what the gate's symmetric half ("a row with no producer fails the step") exists to catch.
...
- **C8 — polling is 2 s for `GET /shop/api/state` and `GET /shop/api/messages`; the catalog is
  ...
  **Poll the two on one shared interval so their ticks coincide** — this aligns them, it does **not**
  ...
  close.
- **C9 — a `503` means *nothing changed*, and the client says so — with exactly one carve-out,
  keyed on the token.** `turn_not_scheduled` (**C14**) is the single `503` this surface produces
  *after* a write has committed, and it is the only response excluded here; **C9 is the default for
  the code and C14 the one exception**, so an untokened `503` — §3's reverse proxy answering for a
  process that has gone away — stays C9's, correctly: a request that never reached the app changed
  nothing. **One meaning, but two actions, and
  ...
  **`graph_unavailable`** on any route that touches the graph (it could not be reached at all, so
  ...
  that times out changes nothing by definition); and **`demo_not_seeded`** on **join and on
  `POST /shop/api/messages`** — the same absent demo `Agent`, refusing the write before anything is
  written, met once as `DemoNotSeededError` from `Storefront.join` and once as `UnknownMemberError`
  from the mention `POST /messages` carries (§5.2). Common to all four: keep the credential, keep the
...
- **C13 — anything with no rule fails *loudly*.** Any `(route, response)` the client receives that matches no rule above renders an explicit
  ...
  **absence** of a matching rule. It is silent whenever a rule matches and is **wrong** — the branch
  ...
  - **Mis-ruled — open, and *fed* by every generalisation.** Pass 8 found three more (C4 silent on
    ...
    **A fourth landed after those eight passes, and correcting how it was found matters more than
    the finding**: the shutdown window's bare `500` on `POST /shop/api/messages` (D-2, now **C14**).
    v1.32 filed it in the bucket above, as a residue only execution could reach. That is false. It
    was measured through the route and written into the delivered suite at `d776ca8` — present at
    `fc2b43b`, the commit the acceptance pass tested — inside `NON_FAMILY_RAISES["RuntimeError"]`,
    an **allowlist entry carrying a written reason**, whose own text concedes the refusal *"**is**
    request-reachable"*, records the bare `500` verbatim, and cites this plan's v1.30 wording as its
    licence. It was **seen, measured and excused**: a rule matched and was wrong, which is this
    bucket. What execution supplied was not discovery but **consequence** — the same fact arriving
    as a defect against a shipped contract instead of as a line in an allowlist nobody re-read.
    **Two things that does not weaken**: S8's cross-product still genuinely cannot see it, the
    response coming from no handler, so closing it at the producer rather than widening that gate
    stands (S8's row); and C13 would still have made it loud on the client, which is all C13 claims.
    **What it does weaken is the assumption that enumeration only costs you in the finding** — this
    one was found, written down, and then reasoned past. An excuse is an enumeration too, and it
    fails the same way: silently, in prose, while every mechanical guard around it stays green
    (`docs/reviews/salesperson-ui-impl.md` `## Pass 23`, P23-3).
  - **Deliberately unruled and *named*, which is not the same as unknown.** Two of
    `Storefront.enqueue_turn`'s three refusal shapes reach the client as a bare `5xx` and stay there
    by decision: a `shutdown_turns()` landing in the gap between the pre-`submit` flag read and the
    `submit` call, which raises a **plain** `RuntimeError` that C14's subclass mapping does not
    catch; and a thread-exhaustion refusal, which queues the work item *before* it fails, so the
    turn does run and `turn_not_scheduled` would be false of it. §5.1's S9 row derives all three
    shapes from `submit` and rejects both ways of typing these two. **C13 is their rule** — loud,
    naming route and status, clearing nothing — which is the only thing a client can do correctly
    about two refusals whose meanings differ and which it cannot tell apart. Naming a residue here
    is what keeps it out of the unruled count above.
...
- **C14 — `503 turn_not_scheduled` on `POST /shop/api/messages` is the one `503` that means *your
  message is in and no reply is coming*.** It is produced when the post lands in the shutdown
  window **and `enqueue_turn` found the lifecycle flag already set** (§5.2; §5.1's S9 row owns the
  mechanism, the accepted trade and the derivation of why the qualifier is there): the message was
  written, the turn was never scheduled, and the process is stopping. **The two neighbouring refusals
  that do *not* carry this token — a `shutdown_turns()` landing inside the read→submit gap, and a
  thread-exhaustion refusal that queues the item first — arrive untyped and are C13's**, which is
  where the residue is scored; a client must not read this token's absence as evidence the turn
  ran. **The action is C4's `/messages`
  reconciliation with the uncertainty removed** — say the turn was lost, keep the transcript line,
  re-enable send, and state the cost of the only recovery on offer, that re-sending posts a second
  line. There is **no re-read**: nothing is unknown, and there is shortly nothing to read against.
  It clears no credential and navigates nowhere.
  **Do not route it through C9**, whose action is a retry that is safe *because nothing changed* —
  here something did, and the retry duplicates the transcript line. **Nor through C6a**, whose
  composer rule waits for `turn.state` to return to `idle` on a poll that will not answer.
  **The three lost-turn paths converge, and that is deliberate**: C4's reconciled branch (inferred
  from a re-read), C6a's `lastTurn` notice (learned from a poll) and this rule (stated in the
  response) are one participant-facing outcome reached three ways, so S12a builds that notice once
  and renders all three through it.
  **Its enumeration is exactly one `(route, response)` pair, and that is checkable rather than
  asserted**: `Storefront.enqueue_turn` has one caller, the `POST /shop/api/messages` route body —
  which is what S12a's per-rule route enumeration comes to for this rule.
...
| `POST /shop/api/session` | `503 demo_not_seeded` — the demo `Agent` is absent, **nothing was written**; **producer is the route itself** (`except DemoNotSeededError`), not a handler (§5.2) | C9 · user-initiated branch, and the one source that does not clear on its own |
...
| `POST /shop/api/messages` | `503 demo_not_seeded` — the demo `Agent` named in `mentions=[agentId]` is gone, so `services._validate_and_derive_role` raises `UnknownMemberError` **before any write**. **Producer is S8b's path-scoped `ServiceError` re-shaper**, on the one route measured to reach it | C9 · same source, same token and same rule as join's row, one route over — the source that does not clear on its own |
| `POST /shop/api/messages` | `503 turn_not_scheduled` — the post landed in the shutdown window **and the flag was already set at `enqueue_turn`'s pre-`submit` read**, so `services.post_message` wrote the message and `enqueue_turn` then refused having submitted nothing (the two shapes that refuse *without* the flag set take no row here — they are C13's, §5.1's S9 row derives all three); **producer is the route itself once S9e lands** — mapping `enqueue_turn`'s refusal (§5.1's S9 row); until then it is this table's one row whose producer is deliberately pending, and re-running S8's gate ahead of S9e fails its symmetric half on exactly this row | **C14** — the one `503` on this surface that does not mean *nothing changed*, so it is C9's single carve-out and is keyed on the token |
...
**How S8's gate reads this table — the three places where the table's key and the gate's key differ.**
All three are *narrowings the gate is read through*, not changes to what the table says, and each is
...
- **The `5xx` rows sit outside the handler cross product, by construction.**
  ...
  *produces*; an unmapped error has no handler — being unmapped is what makes it a `5xx` — so it can
  ...
  is exactly what C13 exists for. **This asymmetry is not an omission the gate should fail on.** **v1.33 adds
  a third route with a known-producible bare `5xx` and still does not row it** — `POST /shop/api/messages`'s two untyped `enqueue_turn` refusals (C13's residual paragraph, §5.1's
  S9 row). The rule for a `5xx` row is unchanged and is not *how well the producer is understood*:
  a row exists where §5.2 makes a claim about a specific producer **and** C12's never-retry
  absolute is load-bearing, which is the two destructive routes. Knowing precisely which refusals
  produce this one is what let C13's residual name it; it is not a reason to row it.
- **The re-shaped `ServiceError` family is filtered by a *measured* route set, not by a route class.**
  S8b added a `/shop/api`-scoped `ServiceError` handler that re-shapes the family into plan tokens
  and delegates everything else to `app.py`'s incumbent (`docs/reviews/salesperson-ui-impl.md`
  `## Pass 10`, P10-1: the old gate enumerated a *delta* against a baseline app, so it saw 5 of 17
  handlers and missed the one that fires). It is a **fourth** handler kind — neither cross-cutting
  nor envelope — and **no route class predicts where it can fire**: whether a route reaches
  `services._validate_and_derive_role` is a property of which service call it makes, not of whether
  it writes. Its half of the cross product is therefore `{the family's mapped responses} × {the
  routes measured to reach it}`, the second factor **derived by execution** — each fault armed in
  turn while all eleven routes are driven, exactly one moving — rather than read off the class
  table. Its **whole** contribution to this table is two pairs, both on `POST /shop/api/messages`:
  `(401, invalid_token)`, which that route already carried — `ThreadNotFoundError` /
  `UnknownActorError` mean the participant's own `Thread` or `User` is gone, which is **C3**, and
  answering `401` converges the reset-window race with what `resolve_token` would answer on the very
  next request — and `(503, demo_not_seeded)`, the row added above.
  **The same standard governs the *inherited* handlers, and that is where it bites next.** The
  eleven workflow/framework handlers the storefront app carries but does not register are excused
  from the cross product by a **reason string**, and eleven reason strings are an enumeration —
  the thing this section has twice had to replace with a mechanism (`docs/reviews/salesperson-ui-impl.md`
  `## Pass 11`, P11-1: a `/shop/api` route raising `WorkflowEngineDisabledError` answers an unruled
  `503 {"error": "WorkflowEngineDisabledError"}`, colliding in status with the two tokens C9
  dispatches on, green through the whole gate). Their route set is empty **by placement, not by
  luck** — no storefront route calls the workflow layer at all today, and S9's trigger enqueue is
  the first step that reaches it. So the excuse gets the same treatment as everything else here:
  it is **measured**, by S8c's `services.` access set rather than by prose. That assertion does
  **not** redden at S9 — the trigger's call site is outside every scope the guards walk, so those
  three excuses go false where no walk can see it, and what measures them instead is S9's
  armed-fault test at the **response boundary**. The guards' reach is stated in the code
  (`falkor-chat/server/falkorchat/storefront_api.py` above `INHERITED_HANDLERS`;
  `falkor-chat/server/tests/test_storefront_api.py` above `SERVICE_LAYER_REACH_TODAY`); §5.1's S9
  row owns the re-derivation.
...
§4.3 is a left-hand row. That is the intended split, and C1–C14 place no security obligation on the
browser.
...
- **S9** — the bounded queue with `turn_workers=1` and a fake 2 s LLM: per-participant
  single-flight (**the concurrent pair held inside the write, not a sleep-timed one**), `409`
  before write, global ordering, **the queue position counting down as the queue drains, and
  correct at `turn_workers=4`** (§5.2 *The queue position*), drain on shutdown, **and reset-mine
  cancelling a queued turn rather than waiting it out** (the `200`-vs-`503` case in S9's
  done-condition, which is the only one that tells the two apart); **and the dead-turn latch** —
...
- **S12a** — the **client** tier's contract tests, which §6 named nowhere before v1.13 (the client's
  only appearance in the test strategy was `npm test` green inside four step rows): **§5.3's C1–C14,
  ...
  `(route, field)` cells, C14 over the one route it spans — a rule spanning a single pair states it
  and is checked like the rest, which is what keeps the guard mechanical rather than a judgement
  about which rules are worth enumerating). This is the plan's only mechanical guard against a rule whose domain was
...
- **Run B — live LM Studio.** The same harness, sweeping concurrency (1, 2, 4, 8, 16, 32, 50) and
  ...
  **Count `turn.lastTurn === 'failed'` and publish it beside the curve** — the harness already
  polls `/state` (it must, to honour the `409` contract rather than firing blind), so the count is
  nearly free, and without it Run B's curve is **biased by construction**: a turn that dies
  contributes no reply-latency sample, so exactly the runs that failed — the slowest, at the
  highest concurrencies — remove themselves from the population and *improve* the published number.
  A curve over completed turns only, with no failure count next to it, is the wrong evidence for
  R1, which is the risk this run exists to quantify. This is the measurement `lastTurn` makes
  possible; before it there was no wire signal to count.
