...
> **Status:** active · **Owner:** `architect` · **Tracks:** — (M<n> TBD) · **Version:** 1.18 · **Reviews:** `docs/reviews/salesperson-ui.md`, `docs/reviews/salesperson-ui-impl.md`
...
| `falkor-chat/server/falkorchat/storefront.py` | S6, S7, S8, S9, S10 | **S6 → S7 → S8 → S9 → S10** · S8's touch is one method — `_catalog_rows` loses its second read once `filter_products` projects `productId` (§5.1 S8, Ruling 1) |
| `falkor-chat/server/falkorchat/storefront_api.py` | S8, S9, S10 | **S8 → S9 → S10** |
| `falkor-chat/server/falkorchat/repository.py` | S4, S8 | **S4 → S8** · S8's touch is the additive `productId` projection on the delivered `filter_products` (§5.1 S8, Ruling 1) — the one place this plan reaches into a delivered step's file on purpose |
...
| `falkor-chat/server/tests/test_storefront.py` | S6, S7, S8, S9 | **S6 → S7 → S8 → S9** |
| `falkor-chat/server/tests/test_storefront_api.py` | S8, S9, S10 | **S8 → S9 → S10** |
| `falkor-chat/server/tests/test_repository.py` | S4, S8 | **S4 → S8** · S8 adds the `filter_products` row-key assertion only |
| `falkor-chat/server/tests/test_trigger.py` · `test_salesperson_scaffold.py` | S2 · S1 | — |
| `falkor-chat/server/tests/test_process_input.py` | S2 (**delivered**) | — · outside S2's §5.1 Files column, and accepted: that file already owns the real-graph chat-path harness the `run_ctx` merge needed, and **no other step touches it** (S4 takes `test_repository.py` + `test_services.py`, and S8 appends to the first) |
| `falkor-chat/docs/QUERIES.md` | S2 (§12.1/§12.12, **delivered**), S4 (new §18), S8 (§15.2's `RETURN` line) | **S2 → S4 → S8** — three disjoint sections, and S2 → S4 is already satisfied by the `services.py` ordering. §12.1/§12.12 are **already edited**, so S4's §18 is an append rather than a merge, and S8's §15.2 edit is a two-token change to one `RETURN` line in a section neither of the others touches |
...
| **Stored value** | `{participantId, token, displayName, language}` — the `POST /shop/api/session` response | `{token}` — the `POST /shop/api/presenter/session` response |
...
- **S8/S10** — `TestClient` contract tests over the whole router: the auth matrix (no token /
  ...
  and an order, every route called with B's token returns only B's data. Plus S8's two wiring
  tripwires, both built to go red rather than null: the `storefront_dir` `create_app` forwards is
  asserted against a *different* populated `config.STOREFRONT_DIR`, and the catalog route is
  asserted with `services.lookup_product` patched to raise (§5.1 S8).
