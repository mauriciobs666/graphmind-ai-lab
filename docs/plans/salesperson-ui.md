...
> **Status:** active · **Owner:** `architect` · **Tracks:** — (M<n> TBD) · **Version:** 1.32 · **Reviews:** `docs/reviews/salesperson-ui.md`, `docs/reviews/salesperson-ui-impl.md`
...
> **A key is omitted only where its absence distinguishes two otherwise identical responses.
> Everywhere else an optional value is carried as the key, with `null`.**

The first case is `incomplete` on reset-all, and the reason it is the first case is already argued
in §5.3 **C6a**: absence is the *only* thing separating a clean `200` from a partial one, which is
a shape difference between two responses and is why that pair takes a completeness-table row of its
own. Everything else optional on this surface is the second case — `order` on
...
illustrative and will age: ask whether the field's absence picks out a different response. Yes ⇒
omit it, and it earns a completeness-table row, because it *is* a second response. No ⇒ `null`.
...
- **C13 — anything with no rule fails *loudly*.** Any `(route, response)` the client receives that matches no rule above renders an explicit
  ...
  **absence** of a matching rule. It is silent whenever a rule matches and is **wrong** — the branch
  ...
  - **Unruled — closed.** Seven instances were found, five of them (1, 3, 4, 5, 7) responses the plan
    ...
    **An eighth arrived after those eight passes, and how it arrived is the point**: the shutdown
    window's bare `500` on `POST /shop/api/messages` (D-2, now **C14**) was found by *executing*
    the system, not by reading it — invisible to every static pass and to S8's gate alike, the
    latter by construction, since the response comes from no handler. Both closure mechanisms
    behaved as designed: the map stayed total over what it maps, and C13 would have made this loud
    in front of the presenter. What it adds to the argument above is that the residue is reached by
    running the thing, which is why the acceptance gate is not redundant with the review gates.
...
- **C14 — `503 turn_not_scheduled` on `POST /shop/api/messages` is the one `503` that means *your
  ...
  window (§5.2; §5.1's S9 row owns the mechanism and the accepted trade): the message was written,
  the turn was never scheduled, and the process is stopping. **The action is C4's `/messages`
