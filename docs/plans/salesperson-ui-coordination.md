...
| **S8** — the `/shop/api` router, the total-by-type error map, the mounts, the preflight | `coder` (**fresh**) | `a1a140f443d3ddfa2` | in-flight (**holds the live DB**) | `storefront_api.py` (new), `schemas.py`, `app.py`, `test_storefront_api.py` (new), `test_app.py` | `analyst` — **the gate Pass 8's stopping rule named** → — | — |
