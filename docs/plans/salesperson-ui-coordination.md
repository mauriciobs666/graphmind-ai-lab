...
**S9c closed.** **Pass 25** (`analyst`, `a22da77c5ce8adf0d`) came back **approve** — 0
blockers/majors, 1 minor + 1 nit, both documentation citation polish (an under-cited but factually
correct cross-reference; a docstring naming ambiguity), neither a code defect, neither blocking.
The reviewer independently constructed and ran all four load-bearing mutations by hand rather than
re-reading the delivery's report — including reproducing the implementer's claimed same-turn race
**deterministically**, via an `Event`-gated monkeypatch, rather than trusting its "8/200" figure —
and all four restored to the delivered md5. Suite/lint independently reproduced again, matching
exactly, and the 18-site exact-dict sweep recounted from source rather than taken from
`HISTORY.md`. Both cosmetic findings left as-is (reviewer itself called them optional — "take or
leave" / "I'd weigh this as a nit if someone else owned the call") rather than spending a
fix-and-re-gate cycle on doc-citation polish after a clean approve. **Accepted — committed
`60b18d9`** (S9c) **and `6c463a6`** (Pass 25).

**S9d is next, unblocked** (behind S9c on `storefront.py`): remove the per-participant record
cache whole (`lookup`, `_records`, `cached_ids`, every `_cache_put`/`_cache_drop`) plus the tests
that exist only to exercise them. S9e remains queued behind S9d.
...
| **S9c** — the dead-turn latch `turn.lastTurn` and its lifecycle | `coder` | `a55c29ae54289d9dd` | **accepted — committed `60b18d9`**. Self-found and self-fixed a same-turn race (clear-after-`submit` could lose to the worker's own failure-mark); teco-verified the fix and independently re-authored the clear-placement mutation, both reddening as expected | `storefront.py`, tests, `HISTORY.md`, `SERVER.md` | `analyst` Pass 25 (`a22da77c5ce8adf0d`) → **approve**, 1 minor + 1 nit (docs polish, not code) — committed `6c463a6` | 266k tok / 145 tools + 160k tok / 75 tools |
| **S9d** — remove the per-participant record cache whole | `coder` | `afa92fda83f4aaf63` | **accepted — committed `a9981c8`** (5 files, +108/−354; suite 2653→2643). Two rounds of teco-verified findings before acceptance: Pass 25's own recurring defect class ("aspirational docstring, unwired mechanism") re-confirmed by independently tracing `enqueue_turn`'s `executor.submit` to prove S9's turn worker never used the removed `lookup()`; Pass 26 (1 major + 2 minors, all fixed) caught stale cache-existence prose the removal itself had missed; teco's own follow-up caught `ParticipantRecord.without_token()` left with zero callers, same category as the named cache methods, removed too | `storefront.py`, `storefront_api.py`, both test files, `HISTORY.md` | `analyst` Pass 26 (`a34c984fab187c447`) → **approve with suggestions**, all findings fixed and re-verified — committed `bfda9d4` | 440k tok / 99 tools (coder, 3 legs) + 106k tok / 32 tools (analyst) |
| **S9e** — the three `INHERITED_HANDLERS` reason strings + armed-fault measurements | `coder` | `a85a9a400fecb4720` | delivered (revert confirmed) — dispatched as **two clusters**: (A) C14's `503 turn_not_scheduled` typed mapping, scoped narrow per Pass 23's P23-2 (pre-submit `_turns_shutdown` branch only, not the post-`submit` thread-exhaustion shape), plus P23-3's three follow-on sites (`§5.3` table declaration, `STOREFRONT_RAISES_TODAY`, `NON_FAMILY_RAISES["RuntimeError"]`'s stale clause); (B) the three now-false `INHERITED_HANDLERS` workflow-reachability reason strings (Pass 15's finding) — teco pre-confirmed 2 of 3 at source and left the third + the `WorkflowEngineDisabledError`-stays-excused reasoning for coder to confirm or refute. **Delivered — both clusters correctly implemented, teco-verified against source**: my own `WorkflowEngineDisabledError`-stays-excused hypothesis was **wrong**, refuted directly from the plan's own S9-row text (all three classes explicitly named as taking the same measured exemption; `WorkflowDefNotFoundError` explicitly named as *not* one of the three) — missed pre-dispatch because `fold`+`grep` on this file's giant single-line rows silently missed the match; re-read with a direct Python string search instead. **`DocumentNotFoundError` hunk (out-of-scope, dependent on the concurrent `document-ingestion2` session's uncommitted `services.py`) confirmed genuinely not `coder`'s own** — its own diff-disjointness check showed the hunk sat apart from every line it actually touched — and reverted; teco independently re-confirmed: zero `DocumentNotFoundError` hits left in the `storefront_api.py` diff, `services.py` (uncommitted, other session) does define the class, and the suite now shows exactly the expected single pre-existing failure (`test_every_service_error_subclass_is_mapped_or_declared_unreachable`, unrelated to S9e) — 145 passed / 1 failed on `test_storefront_api.py` alone, all 5 of S9e's own new tests green in isolation. **teco ran its own independent mutation probe** (not just trusting coder's report): disabled `_mark_turn_failed` in `_run_turn`, all three Cluster B armed-fault tests correctly reddened, restored by copy, `diff -q` clean, tests green again | `storefront.py`, `storefront_api.py`, both test files, `HISTORY.md` | `analyst` (`a960485f78f83cac6`, Pass 27) → **needs changes** (1 blocker, 1 minor) — see below | — |

**Pass 27 (`a960485f78f83cac6`) verdict: needs changes.** Both clusters independently confirmed correct (reviewer read the real diffs, traced real code paths, ran its own mutations beyond teco's, all restored byte-identical). **P27-1 (blocker): the `DocumentNotFoundError` contamination is back** — reviewer found it still present in `storefront_api.py`'s diff, contradicting `HISTORY.md`'s claim of a revert. **teco re-verified this live and found it worse than reported**: the import + `SERVICE_ERRORS_UNREACHABLE` entry are indeed back in `storefront_api.py` (the shared, actively-edited-by-`document-ingestion2` file was re-touched after teco's own confirmed-clean check and before/during Pass 27's review — a live recurrence, not a stale report). **teco additionally found contamination Pass 27 flagged but didn't fully diagnose**: `tests/test_storefront_api.py` carries its own parallel hunks on the *`ServiceError`* family (not `StorefrontError` — confirmed by reading the surrounding code: `family = _subclasses(ServiceError)`), bumping `== 10` → `== 11` twice and "six of the seven"/"seven of eight" unreachable-reason counts — these are `document-ingestion2`'s own test edits for its new `DocumentNotFoundError`, not S9e's (S9e's own family-count-shaped change is the *separate*, legitimate `STOREFRONT_RAISES_TODAY`/"all eight are `StorefrontError` subclasses" edit, confirmed correct). P27-2 (minor): `WorkflowDefNotFoundError`'s "stays true as written" claim is asserted, not pinned to a test like its three neighbors. **Sent back to `coder`** (same agentId, resumed) with all of the above plus a durability fix: produce a clean patch file on disk (immune to further tree churn) rather than relying on the shared working tree still being clean at commit time.
