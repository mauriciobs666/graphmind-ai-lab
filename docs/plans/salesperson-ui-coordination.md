...
| **S9d** — remove the per-participant record cache whole | `coder` | `afa92fda83f4aaf63` | in-flight — dispatched with full call-site inventory (`storefront.py` fields/methods/6 call sites, `storefront_api.py` module+route docstrings, 2 `forget_all()` calls, ~10 tests to remove/trim in both test files), baseline reproduced by teco pre-dispatch: **2653 passed, 14 deselected** | `storefront.py`, `storefront_api.py`, both test files, `HISTORY.md` | `analyst` | — |
