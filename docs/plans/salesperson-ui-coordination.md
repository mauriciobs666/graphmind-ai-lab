...
| **S9e** — the three `INHERITED_HANDLERS` reason strings + armed-fault measurements | `coder` | `a85a9a400fecb4720` | **committed `dcec3f2`** — two clusters: (A) C14's `503 turn_not_scheduled` typed mapping on `enqueue_turn`'s pre-submit shutdown refusal, scoped narrow per P23-2; (B) the three now-false `INHERITED_HANDLERS` workflow-reachability reason strings (Pass 15), each pinned to a real armed-fault test driving a live `WorkflowTrigger`/`Services` pair. teco's own pre-dispatch `WorkflowEngineDisabledError`-stays-excused hypothesis was refuted by `coder` directly from the plan's own S9-row text, independently re-confirmed by teco via direct Python string search (the file's giant single-line rows silently defeat `fold`+`grep`). **Recurring shared-tree collision, resolved durably**: a `DocumentNotFoundError` hunk belonging to the concurrent `document-ingestion2` session (its own `4a6186b` commit message defers this exact registration) kept reappearing in the live `storefront_api.py`/`test_storefront_api.py` working tree — reverted once, confirmed clean by teco, then found recurred **and expanded** (a second, previously-missed `ServiceError`-family-count contamination in the test file) at Pass 27. `coder` built and verified a self-contained patch of only its own hunks in an isolated `git worktree`; teco independently re-verified that same patch in a separate worktree of its own (fresh `HEAD`, zero contamination, full suite showing only the 2 pre-existing unrelated failures, all 6 of S9e's own tests green), then staged it directly into the git index via `git apply --cached` — bypassing the live tree's contents entirely — and committed, leaving the other session's uncommitted edit intact and undestroyed in the working tree | `storefront.py`, `storefront_api.py`, both test files, `HISTORY.md` | `analyst` (`a960485f78f83cac6`, Pass 27 → needs changes, then resumed for Pass 28 against `dcec3f2`) → — | — |

**Pass 27 verdict: needs changes** (1 blocker, 1 minor) — both real clusters independently confirmed correct via the reviewer's own mutation tests; P27-1 (the `DocumentNotFoundError` recurrence) and P27-2 (`WorkflowDefNotFoundError`'s unpinned claim) both resolved in the `dcec3f2` commit above — P27-2 closed by a new AST-based test (`test_the_trigger_never_reads_a_workflow_def_so_that_excuse_stays_pinned`) reading `trigger.py`'s actual call set with its own negative control. **Pass 28 re-gate dispatched against the commit, not the live tree**, to confirm.
