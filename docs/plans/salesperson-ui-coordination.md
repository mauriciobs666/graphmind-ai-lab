...
## RESUME HERE — state as of 2026-09-11, S9e in-flight (`a85a9a400fecb4720`), last of the five S9 sub-units
...
| **S9e** — the three `INHERITED_HANDLERS` reason strings + armed-fault measurements | `coder` | `a85a9a400fecb4720` | in-flight — dispatched as **two clusters**: (A) C14's `503 turn_not_scheduled` typed mapping, scoped narrow per Pass 23's P23-2 (pre-submit `_turns_shutdown` branch only, not the post-`submit` thread-exhaustion shape), plus P23-3's three follow-on sites (`§5.3` table declaration, `STOREFRONT_RAISES_TODAY`, `NON_FAMILY_RAISES["RuntimeError"]`'s stale clause); (B) the three now-false `INHERITED_HANDLERS` workflow-reachability reason strings (Pass 15's finding) — teco pre-confirmed 2 of 3 at source (`WorkflowRunNotFoundError`, `WorkflowInputRejectedError`, both documented raises of `services.start_workflow_run` per its own docstring, reachable via `trigger.maybe_trigger` on the turn worker) and left the third + the `WorkflowEngineDisabledError`-stays-excused reasoning for coder to confirm or refute, with an explicit stop-and-ask if the count doesn't resolve to three. Baseline reproduced: 2643/14. **Delivered — both clusters correctly implemented, teco-verified against source**: my own `WorkflowEngineDisabledError`-stays-excused hypothesis was **wrong**, refuted directly from the plan's own S9-row text (all three classes — `WorkflowEngineDisabledError`, `WorkflowInputRejectedError`, `WorkflowRunNotFoundError` — explicitly named as taking the same measured exemption; `WorkflowDefNotFoundError` explicitly named as *not* one of the three), which I'd missed pre-dispatch because `fold`+`grep` on this file's giant single-line rows silently missed the match — re-read with a direct Python string search instead. Final suite 2648/14, reproduced. **One real problem found before acceptance**: `storefront_api.py`'s diff imports `DocumentNotFoundError` and adds a `SERVICE_ERRORS_UNREACHABLE` entry for it — out of S9e's scope entirely, and dependent on a *different*, concurrent session's (`document-ingestion2`) uncommitted `services.py` change (confirmed absent at `HEAD` via `git show`). Sent back for a revert of just that hunk before this can be committed | `storefront.py`, `storefront_api.py`, both test files, `HISTORY.md` | `analyst` | — |
