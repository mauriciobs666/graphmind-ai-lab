...
| U139 | `tdd-engineer` | `aa2d47bd9295317ab` | in-flight — dispatched, parallel with U140 (disjoint files: `modelbench/results.py`, `modelbench/scoring/toolcalls.py` stub, `tests/test_results.py`, `tests/test_convo.py` vs. U140's `packs/tool-caller-shop-assistant/`). S5 spec §5 Step 0 — `FunnelCounts`/`HazardPoint` dataclasses, `ToolCallAggregates.determinismProbe`/`funnelCounts`, `hazard` type change, `named_metrics()` fix, `_encode`/`_decode` cases; `toolcalls.py` stub (two constants + union/disjointness assertion); tripwire's third leg added, old tripwire deleted | `modelbench/{results,scoring/toolcalls}.py` + tests | `tdd-engineer` (self-mutation-tested) → — | — |
| U140 | `tdd-engineer` | `a8cfef1fcbdeb86cb` | in-flight — dispatched, parallel with U139 (see above). S5 spec §5 Steps 1-2 — the storefront (`tools/sim.py`, `tools/schemas.json`, `catalog.json`, seven tools transcribed from `falkorchat/tools.py`, dispatch-totality contract, E1's eleven-case adversarial property test) plus `pack.json`/`prompts/system.md` (manifest-only read test) | `packs/tool-caller-shop-assistant/*` + tests | `tdd-engineer` (self-mutation-tested) → — | — |
