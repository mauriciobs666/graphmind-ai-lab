...
| U84 — **the `drive` rework, dispatched at last.** Given `coder` rather than `tdd-engineer` deliberately: this is *a detailed plan executed against a settled specification*, not a bug hunt, and §4 S2's *Done when* is the done-condition. **Told why it waited and what that buys** — sixteen gate passes closed on their own stopping rule, so it is the first implementation unit to build against a finished document, and **if it finds the plan wrong that is a finding to report loudly, not to work around**. Two runnable done-conditions: `grep -c 'turn\.expect\|expect\.get\|expect = ' modelbench/convo.py` **9 → 0**, and `historyReplay`'s fourth value. Fenced off the already-built guard with the reason: **do not derive either declaration from the other and do not touch the transcript to make something pass** — it was built in its own round precisely so this unit could not author it. **The judgement I flagged as most likely to fail silently**: `drive` must catch `LMStudioError` and *continue the script*, since a skipped turn destroys every later turn's denominator, and a reply-less turn still contributes to replayed history. Carries yesterday's `PYTHONDONTWRITEBYTECODE=1` mutation fix and the cite-by-symbol rule. Sizing escape hatch given explicitly | `coder` (fresh) | `a2157630dab532afe` | **in-flight** — dispatched at `3c795cc` | `convo.py` + tests | `analyst` → — | — |
