...
| U40 — Diff-scoped review of `8fc2341` + adjudicate the implementer's 6 reported items | `analyst` (fresh) | `ad216ed80e4e38da2` | in-flight | `docs/reviews/small-model-benchmarking-impl.md` | self → — | — |
| U41 — **S1 impl 2 of 3**: Tables **C + D + E + G** (the `stats.py` signature round, C→G mandated) | `coder` | — | **queued** — held until U40 clears | `model-bench/` source + tests | `analyst` diff-scoped → — | — |
| U42 — **S1 impl 3 of 3**: Table **F** (the continuous carrier, `results.py`/`report.py`) | `coder` | — | **queued** | `model-bench/` source + tests | `analyst` diff-scoped → — | — |
