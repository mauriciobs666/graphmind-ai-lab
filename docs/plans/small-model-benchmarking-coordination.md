...
| U59 — Re-baseline Table H to `93b0e42`. **Three stale facts, all found by my own verification rather than by a gate**, and all caused by U55 landing *while U57 was writing*: DC-12's *"byte-identical from `7f865e2` through HEAD"* clause is now false; enumerating command 2 reads **21 not 20** (`test_stats.py` **15 → 16**, the added line being inside the very test U55 turned into a six-id sweep); and P10-5's fresh re-derivation inherits it as *sixteen of twenty-one*. **Also a convention question I declined to settle myself** — should a table's stated baseline re-point to the tip each time an unrelated unit lands, or stay pinned to what its author measured with DC-12 carrying the delta? It owns §7 rule 5, so it decides and writes it down; Table F's unit will hit this again | `architect` (**resumed** `a6e6e78f99baf4ead` at **401k tok**. Fresh dispatch is what my own rule says at this size, and I overrode it a second time on a narrower ground: I supplied **every number**, so the work is three edits plus one judgment and needs no re-derivation — a fresh agent would pay a full plan re-read to do less. Stop signal restated) | `a6e6e78f99baf4ead` | **in-flight** | `docs/plans/small-model-benchmarking.md` **v1.21** | `analyst` → — | — |
