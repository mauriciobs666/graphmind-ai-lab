...
| U134 | `tdd-engineer` | `af969fb9c6eb7ca55` | in-flight | Step 4 (`docs/plans/small-model-benchmarking-s4-spec.md` §7 Step 4) — `nlq-structured-query`'s live half: write `tables.json`'s `knowledge_base` half from a fresh `teco`-captured `ws:nlq-eval` snapshot, `--check-tables-shape` + `PROVENANCE.md`, `--stamp-answerability` (expect 6/34), the live proof run against one model (`qwen/qwen3-4b-2507`, matching U132), report-rendering check. No longer blocked — U133b/U133d both landed (`dc4ce3d`, `acfe7a8`) | — | `analyst` (full-S4 code+methodology gate, after Step 4 closes) → — | — |
