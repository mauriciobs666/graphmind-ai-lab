...
| U141 | `tdd-engineer` | `a6d0362d6f73e35cf` | in-flight — dispatched. S5 spec §5 Steps 3-4 (sequenced after U139/U140, both now accepted) — Step 3: `scoring/toolcalls.py`'s per-turn pure functions (`turn_disposition_scores`, `emission_form`, `detect_prose_pseudo_call`/`prose_detector_precision_recall`, `right_tool_chosen`, `argument_correctness`, `spurious_and_duplicate`, `stopping_when_done`/`iteration_summary`, `reply_matches_tool`, `restraint`), each against the `-ml` note §4.2's exact denominators. Step 4: the ten named adversarial `ConversationTrace` fixtures + the two discriminating pairs (`cap-hit`-empty-dispatch, `timed-out`-vs-`no-response`) + `clean_through_turn`'s third state + `hazard_points`'s censoring (both mutation directions) + `outcome_vectors_differ`. Excludes `score_conversations` assembly (Step 5) and `report.py`/`runner.py` wiring (Step 6). Briefed with the E1-generator lesson from U140 (isolate one fixture's target field/rule, keep every other field baseline-valid) | `modelbench/scoring/toolcalls.py` + `tests/test_scoring_toolcalls.py` | `tdd-engineer` (self-mutation-tested) → — | — |
