...
| U141 | `tdd-engineer` | `a6d0362d6f73e35cf` | **gated — finding sent back, awaiting fix.** S5 spec §5 Steps 3-4 (sequenced after U139/U140, both accepted) — Step 3: `scoring/toolcalls.py`'s per-turn pure functions (`turn_disposition_scores`, `emission_form`, `detect_prose_pseudo_call`/`prose_detector_precision_recall`, `right_tool_chosen`, `argument_correctness`, `spurious_and_duplicate`, `stopping_when_done`/`iteration_summary`, `reply_matches_tool`, `restraint`), each against the `-ml` note §4.2's exact denominators. Step 4: the ten named adversarial `ConversationTrace` fixtures + the two discriminating pairs (`cap-hit`-empty-dispatch, `timed-out`-vs-`no-response`) + `clean_through_turn`'s third state + `hazard_points`'s censoring (both mutation directions) + `outcome_vectors_differ`. Excludes `score_conversations` assembly (Step 5) and `report.py`/`runner.py` wiring (Step 6). **Independently re-verified**: full suite reproduced (`1563 passed, 3 deselected` — 1483 baseline + 80 new), `ruff check .` clean; `_p95_rank`'s formula confirmed to literally match `stats.py:649`'s internal rank expression (never re-derived, as claimed); both discriminating-pair tests read in full and confirmed correct against the `-ml` note's five/two named consequences. **`teco`'s own independent mutation probe found a real gap**: `_scalar_equal`'s "never coerce across types" rule (`-ml` §4.2(d)'s explicit instruction) has zero test coverage — removing the cross-type refusal branch (byte-verified backup/restore) left all 80 tests green; no test anywhere calls `_scalar_equal`/`argument_correctness` with a numeric-vs-string pair that would only match via `str()`-coercion (e.g. `49.99` vs `"49.99"`). This is exactly the shape of defect FR-8(d) exists to catch. Finding relayed to the same delegate (agentId `a6d0362d6f73e35cf`) with the exact repro; awaiting fix and re-verification | `modelbench/scoring/toolcalls.py` + `tests/test_scoring_toolcalls.py` | `tdd-engineer` (self-mutation-tested, one gap found by `teco`) → gated | 314k tok / 99 tools |
