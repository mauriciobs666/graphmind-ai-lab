...
| U133d | `tdd-engineer` | `a5149e3a6c5ba1bea` | in-flight | Implement U133b's A3 contract in `modelbench/scoring/extraction.py`/`modelbench/results.py`: unanswerable-branch exploratory scoring, `luckyPass`/`luckyPassCount`, `aggregate()`'s shape-pooling change — then re-verify the real 40-item pack's reference specs still stamp the expected 6/34 split with zero lucky passes | — | `analyst` (deferred to full-S4 code gate) → — | — |
...
| U134 | — | — | queued | Step 4 — `nlq-structured-query`, live half, per the now-corrected 6/34 understanding: the official `tables.json`/`PROVENANCE.md` snapshot write, `--stamp-answerability`, the score-reporting code delta from U133b, then the live proof run (mirroring U132) — blocked on U133b/U133c landing | — → — | — |
