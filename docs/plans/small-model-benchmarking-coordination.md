...
| U133a | `data-scientist` | `a298b7b1368ba79c6` | in-flight | Methodology opinion: are `nlq-38`/`nlq-39` genuinely unanswerable-by-construction (matching the golden set's own rationale), should the unanswerable-bucket count/verdict-metric denominator move from 4/36 to 6/34, and what's the cleanest resolution path (reclassify vs. replace corpus items vs. other) | `docs/reviews/nlq-conflicting-facts-answerability-ml.md` | — → — | — |
| U134 | — | — | queued | Step 4 — `nlq-structured-query`, live half: the official `tables.json`/`PROVENANCE.md` snapshot write, `--stamp-answerability`, then the live proof run (mirroring U132) — **blocked on U133a's resolution**, since `--stamp-answerability`'s expected outcome (4 vs 6 unanswerable) depends on it | — → — | — |
