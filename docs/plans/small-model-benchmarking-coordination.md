...
| U133b | `architect` | — | queued | Apply review A1/A2/A3: correct plan §3.8.3's "4 unanswerable" to 6/34 citing the upstream ruling; state the answerability-stamp authoring rule (filter on the question's subject); specify the new score-reporting contract (unanswerable items still get a per-shape exploratory score via the full scorer, labelled "lucky pass" if it lands correct) as a concrete spec delta for the implementer. Also: the review's open question — does any other model-bench pack's plan text inherit a stale denominator/exclusion claim from an upstream falkor-chat note (one grep before S4 closes) | — | — → — | — |
| U133c | `data-scientist` | `a298b7b1368ba79c6` | queued | Apply review A4: correct `-ml` §7.2's `nlq-generator` row (n_eff=34, floor 17.6pp, MDD80 22.3pp) and §3's fact table (34 answerable / 6 unanswerable split), in place, per the doc's own same-kind/role revision convention | — | — → — | — |
