...
| U133 | — | — | queued | Step 3 — `nlq-structured-query`, offline half, in three passes (Pass A `tools/exec.py`, Pass B `extraction.py`'s pure scoring, Pass C wiring `score_item`/`aggregate`) per spec §7 Step 3 | `analyst` (deferred to full-S4 code gate) → — | — |
