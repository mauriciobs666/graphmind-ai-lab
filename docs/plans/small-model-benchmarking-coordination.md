...
| U137 | `coder` | `ac8bcb4b32cf0c542` | in-flight — dispatched (haiku). S4 stage-close doc sync — `AGENTS.md`/`README.md`'s "Current state" still say "Stage S3 is closed" (never updated during S4, unlike S2→S3's U119 precedent); rewrite to S4-closed, update "what S4+ owes" to S5+, one `HISTORY.md` entry. Prose-only, no code | `model-bench/{AGENTS.md,README.md,docs/HISTORY.md}` | — → — | — |
