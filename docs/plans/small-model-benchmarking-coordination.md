...
| U133 | `tdd-engineer` | `a8e9a0a3cf132de79` | in-flight | Step 3 — `nlq-structured-query`, offline half, in three passes (Pass A `tools/exec.py`, Pass B `extraction.py`'s pure scoring, Pass C wiring `score_item`/`aggregate`) + `refresh_golden.py`'s three new origins/`schema-literal` kind + the 40 hand-authored `reference_specs.json` entries, per spec §7 Step 3. **`teco` performed the §2.6 `ws:nlq-eval` live snapshot itself** (62 Entity/12 Document/12 Chunk rows, via `mcp__cypher__query`, saved to scratchpad, handed to the delegate by path for spec-authoring) — the official `tables.json` write + `PROVENANCE.md` row + `--check-tables-shape`/`--stamp-answerability` stay Step 4's, dispatched after this lands | `analyst` (deferred to full-S4 code gate) → — | — |
| U134 | — | — | queued | Step 4 — `nlq-structured-query`, live half: the official `tables.json`/`PROVENANCE.md` snapshot write, `--stamp-answerability`, then the live proof run (mirroring U132) | — → — | — |
