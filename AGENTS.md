...
|---|---|---|
| `teco` | Technical coordinator; decomposes a goal into a sequenced work breakdown and delegates each piece to the right specialist (architect, coder, tdd-engineer, analyst, qa-engineer, graph-dba, devops, cobb), then integrates. Hybrid: drives execution but returns to the user at decision points. Coordinates — doesn't design/code (Write/Edit = coordination doc only, enforced by a `PreToolUse` hook). | A multi-step / multi-specialty task, orchestration, or end-to-end feature delivery. |
